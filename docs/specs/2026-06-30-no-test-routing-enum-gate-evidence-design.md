...
**Status:** proposed — targets **v0.7.9** (doc/correctness sweep, landOrder 2). **Severity: MEDIUM.**
...
**Integration-base pin:** war this on the **post-#251 landed tip** (the land-phase prompt at [`workflow-template.js` land-phase prompt](../../skills/war/assets/workflow-template.js)). #236's edit is adjacent to #251's landed land-phase prompt string; baselining off main avoids a spurious adjacent-line conflict.
...
- `HARD_ESCALATION_REASONS` ([the inline array](../../skills/war/assets/workflow-template.js)) includes `'no-test'`, which is **load-bearing**: the merge-task escalation push (`escalated.push({ task, reason: 'no-test' })`) and `hardEscalation = escalated.some(e => …includes(e.reason))` both depend on it. But the same array is reused on the **land side** — [the `landResult.status` check](../../skills/war/assets/workflow-template.js) — where `'no-test'` is not a valid land-phase status. The land-phase prompt only ever returns `landed`/`land_stale`/`gate_failed`/`error`, so the membership is inert today, but logically a never-emitted `no-test` land result would spuriously hard-escalate. **(#236 — nit, latent-correctness.)**
...
- The `gate_output` population directive — [merge-task prompt gate_output sentence](../../skills/war/assets/workflow-template.js) (mirrored at the no-test retry prompt) and [`war-refiner.md` step 5](../../agents/war-refiner.md) — asks for *"the executed gate output (stdout+stderr)"* but never forbids curating it. The 12 tracked `*.test.sh` runners each emit a single aggregate PASS line, so a 3-of-12 excerpt leaves the downstream execution-evidence auditor ([gate-audit prompt consuming `gateOutput`](../../skills/war/assets/workflow-template.js)) unable to confirm all runners ran by direct evidence — a partial paste reads as an under-run. **(#269 — nit, reporting-completeness.)**
...
| 4 | #269 | Append one completeness clause to the `gate_output` population sentence at its canonical source ([`war-refiner.md` step 5](../../agents/war-refiner.md)) and its two mirror sites in `workflow-template.js`: *do NOT curate/excerpt; include the complete `*.test.sh` runner list or state the total count (e.g. "12 `*.test.sh`")*. Prompt-only. | A schema field for runner-count — speculative; the auditor reads `gate_output` as text, a prose directive closes the gap with no control-flow change. Editing `war-worker.md` — the worker self-report routes through the same `gate_output` field; no separate worker summary exists, so no edit needed. |
...
| [`agents/war-refiner.md`](../../agents/war-refiner.md) | #269: append the same completeness clause to step 5's `gate_output` sentence. (Step 4's exit-1-vs-2 split is already correct — #237 brings the template *to* it, no `.md` change for #237.) |
...
| `.claude-plugin/plugin.json`, `.claude-plugin/marketplace.json` (×2), `README.md` `## Status` | Version bump to `0.7.9` across the four canonical slots (release task). |
...
2. **#236** — enum-narrowing comment (land-side check). Adjacent to #251's land-phase prompt — pin the integration base here.
3. **#235** — reAuditFailed comment reword.
4. **#269** — `gate_output` completeness clause (template ×2 + `war-refiner.md` step 5).
...
4. **#269** — the `gate_output` population sentence in `war-refiner.md` step 5 and both merge-task prompt strings instruct the agent not to curate/excerpt and to include the full `*.test.sh` runner list or state the total count; a grep for the new clause finds it in all three sites.
...
6. **Full gate green** — `node --test "skills/**/*.test.mjs"` plus all 12 `*.test.sh` runners pass after the stack lands.
...
- **Version serialization:** `0.7.9` replaces the four canonical slots (`plugin.json`, `marketplace.json` ×2, `README` `## Status`) in one release commit at the tip of this stack; this stack lands serially on the post-#251 tip (live is `0.7.7`; landOrder 2 means a sibling `0.7.8` lands first).
