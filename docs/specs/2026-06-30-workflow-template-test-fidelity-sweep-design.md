...
**Status:** proposed — targets **v0.8.6** (test-hygiene / cosmetic sweep; re-baselined off master v0.8.0 after the two submodule increments). **Severity: LOW.**
...
Anchors re-grounded against current master HEAD `89e5968` (v0.8.0) after the two submodule-support increments (v0.7.8 + v0.8.0) added +316 lines of submodule tests to this same file. The suite is green. (Pre-update fork point was `88c64cc`, v0.7.7; the +316 churn pushed some construct line numbers down — anchor by construct, not line.)
...
2. **#267 — comment mispredicts deletion failure-mode + dead local.** In [test `L3 T2 Test 1 — blocked fix-worker escalates on round r`](../../skills/war/assets/workflow-template.test.mjs): (a) the "Load-bearing assertion" comment STILL predicts that deleting the fix-worker binding makes the loop **reach audit-blocked** — wrong. Tracing the `buildSeqImpl`-style impl: with the bind removed, the impl serves the first `fix:t1:r1` dispatch, so `fixDispatchCount` stays 0; the auditor returns `request_changes` only on the `fixDispatchCount === 0` branch and `approve` otherwise, so the next round re-audits and the task **approves + lands** — never audit-blocked. The three token-`'X'` assertions (`t1Esc.reason==='escalate'`, `t1Esc.blocked==='X'`, `logEntry.blocked==='X'`) still fail RED on bind-deletion, so the test is genuinely load-bearing; only the narration is wrong. (b) a `const t1Log = …` local is computed but never referenced. **RE-GROUNDING CORRECTION:** both halves are STILL un-fixed at current HEAD `89e5968` (and were un-fixed at the fork `88c64cc`) — `grep -c 't1Log'` returns **1** and the comment still reads "reach audit-blocked". The earlier "already self-corrected in this worktree" claim was never true; the +316 submodule churn did not touch this region. So #267 is a genuine **two-edit fix** (delete dead local + reword comment), not verify-and-confirm. Variant 3 of [[weak-test-assertion-passes-without-feature-being-exercised]].
...
| D2 | #267 | Comment + dead local | **APPLY both halves** (re-grounded: NOT pre-applied — both un-fixed at HEAD): reword the comment to "re-audit and approve+land; blocked:'X' is absent", and delete the unused `t1Log` local. Confirm RED-on-bind-deletion and re-run green. | Moving the load-bearing logic (the three token-`'X'` assertions are already correct — only the narration + dead local need fixing). |
...
| File | Change |
|------|--------|
| [`skills/war/assets/workflow-template.test.mjs`](../../skills/war/assets/workflow-template.test.mjs) | #266: one-token regex tighten (`null\)`→`:\s*null\)`) + optional anchor comment. #267: reword mispredicting comment + delete dead `t1Log` local (both un-fixed at HEAD). #250: five label sites renamed (1 two-line comment block + 4 titles; `grep -c` counts 6 lines — the plan's task uses the line-count). #221: one deliberate-choice comment at the auditor throw (or no-op if closed as superseded). |
| `.claude-plugin/plugin.json`, `.claude-plugin/marketplace.json` (×2), `README.md` (`## Status`) | Version serialization to v0.8.6 (four canonical slots, replace-in-place; master HEAD reads 0.8.0). |
...
5. **Whole-suite green + version bump.** `node --test "skills/**/*.test.mjs"` passes (6 suites), and all `*.test.sh` runners (self-discovered, not a hardcoded count — **13 at master HEAD** after the v0.8.0 submodule increment added `assert-no-submodule-mutation.test.sh`) pass. The four canonical version slots (`.claude-plugin/plugin.json`, `.claude-plugin/marketplace.json` ×2, `README.md ## Status`) all read **0.8.6** with no other slot left at a prior version (master HEAD reads 0.8.0).
...
- **#267 is a real two-edit fix (re-grounded).** NOT pre-applied — both halves are un-fixed at master HEAD `89e5968` (and were at the fork `88c64cc`; the earlier "uncommitted in the worktree" claim was never true). Apply both edits: reword the mispredicting comment + delete the dead `t1Log` local.
- **Version serialization.** Replaces the four canonical version slots in place (no badge), 0.8.0 → 0.8.6. Lands serially as landOrder 6 — last in the stack, behavioral-before-cosmetic.
