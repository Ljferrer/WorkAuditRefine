...
WAR advertises a "three-layer resumable source of truth" — GitHub issue labels, `ledger.json`, and the Workflow `resumeFromRunId` journal ([`design.md`](../../skills/war/references/design.md) § "6. State & resume" — the "One authority, two durable advisory records" block) — but the three are written by different actors at different moments, with **no stated precedence** and **no check** that a recorded `merge_sha` is reachable on the branch; the resume sentence ([`SKILL.md`](../../skills/war/SKILL.md) § "Resume (on restart after a crash or interruption)") says only "read it + open issues and continue" (agent-architecture audit finding L2, 2026-06-29). A future reader will ask why the `ledger.json` — listed as a "source of truth" — is treated as disposable, and why resume trusts git over the records the Lead actually wrote. This records it. Full mechanics: [the design spec](../specs/2026-06-29-resume-precedence-reconciliation-design.md).
...
1. **A stated precedence: git branch state > GitHub issue labels > `ledger.json`.** Git wins because the refiner's push-first CAS ([`design.md`](../../skills/war/references/design.md) § "6. State & resume") never `--force`es a shared branch ([`SKILL.md`](../../skills/war/SKILL.md) § "Invariants (never violate)", the never-`--force`/`reset --hard`-on-shared-branches clause), so the integration/working branches are monotonic — a recorded merge is real *iff* its SHA is reachable. Labels beat the ledger because they are remote-durable and human-visible (they survive a local wipe); the ledger is the richest record but the weakest authority — local, uncommitted, written by no code, a **lagging view**.
