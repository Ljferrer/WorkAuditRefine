...
| 11 | [2026-08-06-war-strategy-mirror-guards](../plans/2026-08-06-war-strategy-mirror-guards.md) | authoring-surface family — `skills/war-strategy/SKILL.md`, `skills/war-strategy/references/plan-interview.md`, `skills/war-strategy/war-strategy-structure.test.sh`, `skills/war-machine/SKILL.md`; 2026-08-15 amendment (#1431, latitude authoring + runtime mirror): + `skills/war-machine/war-pipeline-structure.test.sh` (one twin pin), `agents/war-worker.md`, `agents/war-auditor.md`, `skills/war/assets/workflow-template.js` + `.test.mjs` (intent-clause twins + registry anchors), `docs/adr/0013-…` (appended amendment); release slots | next free | 4 (order-only) |
...
family they land in: #1386 → plan 5; #1410, #1412 → plan 6; #1395, #1408, #1411, #1413, #1430 → plan 9; #1431 → plan 11;
...
| #1331, #1333, #1289, #1380, #1381, + #1395, #1408, #1411, #1413, #1430 (folded 2026-08-15; #1430 observed live in this campaign's plan-5 phase 1) | [handoff-schemas-contract](../specs/2026-08-06-handoff-schemas-contract-design.md) (#1380 and #1381 folded by operator direction, 2026-08-12) | plan 9 |
| #1373, #1286, #1334, #1375, #1332, #1252 | [structural-pin-extractors](../specs/2026-08-06-structural-pin-extractors-design.md) | plan 10 |
| #1307, #1308, #1309, + #1431 (folded 2026-08-15; operator-reported latitude-doctrine gap) | [war-strategy-mirror-guards](../specs/2026-08-06-war-strategy-mirror-guards-design.md) | plan 11 |
...
| `skills/war/assets/workflow-template.test.mjs` | 3, 6, 9, 10, 11, 13 | Hottest suite in the batch. Spine-serialized 3 → 6 → 9 → 10 → 11 → 13; constructs disjoint per plan 10's Note 1 construct census, plan 11's amendment (intent-threading registry anchors only), and plan 13's additive card-pointer guard rows; each downstream worker rebases onto the landed suite. |
...
| `skills/war/assets/workflow-template.js` | 3, 6, 9, 11 | Spine-serialized 3 → 6 → 9 → 11; each plan rewrites regions the next stacks on (`doneWhenFloorClause` → sweep routing → handoff/followUp dispatch → the intent-clause twins, plan 11's amendment — additive inside `workerIntentClause` + the auditor intent threading); measured-at-base + expected-post-predecessor tagging in plans 6 and 9. |
...
| `skills/war-machine/war-pipeline-structure.test.sh` | 4, 11 (amendment) | Plan 4 (landed) owns the helper refactor; plan 11's Task 2.1 adds one additive machine-SKILL latitude twin pin — no helper edit, no existing pin moved; serialized by the same 4 → 11 spine edge. |
| `agents/war-auditor.md` | 6, 11 (amendment), 13 | Spine-serialized 6 → 11 → 13: 6's Phase-2 verdict-boundary doctrine, 11's latitude-clause arm on the Latitude rule bullet (additive), 13's pointer re-anchors — construct-disjoint, each rebases onto the landed card. |
| `agents/war-worker.md` | 9, 11 (amendment), 13 | Spine-serialized 9 → 11 → 13: 9's ENV_OUTCOME/prompt widenings, 11's latitude-clause arm on the intent block (additive), 13's pointer re-anchors — construct-disjoint. |
...
| `skills/war/assets/skill-doc-contracts.test.mjs` | 5 (red-team pass), 10, 12 | Plan 12 Context 6: 10 owns the D31 block (file tail) + the Task 2.1 doc-cascade banner; 12 owns the D22 ordered-span block — construct-disjoint though adjacent at the banner boundary. Order-only edge 10 → 12 with the `D31_ARMS_COLLIDED` witness. **Plan 5 joined this row at its /red-team pass (2026-08-16, D18):** its Task 1.1 adds a **D30** pointer-pair row for the evicted `CONTEXT.md` glossary terms, modelled on D28. Plan 5 lands **first** on the spine, so 10 and 12 rebase onto D30; the suite's additive row-per-contract shape makes that mechanical, and D30 is construct-disjoint from both D31 and D22. |
