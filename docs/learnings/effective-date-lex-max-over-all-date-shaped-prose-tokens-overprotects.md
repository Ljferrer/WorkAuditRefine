...
description: effectiveDate takes the lex-max of every 20\d\d-\d\d-\d\d token in frontmatter+prose — a stray future/invalid date-shaped token permanently protects a lesson from tighten eviction
metadata: 
  node_type: memory
  type: project
