...
description: "A zero-byte first file in a two-file awk NR==FNR diff degenerates into 'every stdin record is new' — arg-parse validation must check -s, not just -e/-f/-r"
metadata: 
  node_type: memory
  type: project
