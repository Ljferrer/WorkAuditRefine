...
description: "Test a shared semaphore with a drain assertion after rejected dispatches plus an exact call-site census; a peak bound cannot see a leak."
metadata: 
  node_type: memory
  type: project
  keywords: 
...
the file routes through this one seam (PIN-4): worker, auditor, ace, fix worker, refiner,
...
`function makeSemaphore(n)` and `async function dispatch(prompt, opts)`; the `finally` release
sits in the `dispatch` body). The paired tests in `skills/war/assets/workflow-template.test.mjs`
(search `semaphore: at most N agent dispatches` and `leak guard`). The census test (search
`dispatch-seam census`).
