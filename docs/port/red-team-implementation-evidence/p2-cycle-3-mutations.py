...
cases=[
 ('worktree-directories','directoryIdentity(repository)','[]','directory identity detects worktree$'),
 ('git-directories','[current,...metadataIdentity(root,path)]','metadataIdentity(root,path)','directory identity detects git$'),
 ('git-root-mode',"[['',...fileIdentity(common)],...metadataIdentity(common)]",'metadataIdentity(common)','directory identity detects git-mode'),
 ('worktree-directory-mode','[[relative(root,directory),...fileIdentity(directory)],...readdirSync','[...readdirSync','directory identity detects worktree-mode'),
 ('no-symlink-traversal',"entry.isDirectory() && !(directory===root", "(entry.isDirectory() || entry.isSymbolicLink()) && !(directory===root",'directory identity never follows'),
]
...
Path('/private/tmp/red-team-p2c3-mutations.json').write_text(json.dumps(results,indent=2));assert all(r['assertionKilled'] for r in results)
