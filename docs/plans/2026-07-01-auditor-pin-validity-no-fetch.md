...
**Files:** [`agents/war-auditor.md`](../../agents/war-auditor.md) (pin-validity lens Step 3; anchor by the `If this is a gitlink-bump task` / `Verify the new SHA is reachable on the submodule remote` construct), [`hooks/validate-auditor-git.test.sh`](../../hooks/validate-auditor-git.test.sh) (deny-cases + a `-C cat-file` allow-case), [`docs/specs/2026-06-29-submodule-support-design.md`](../specs/2026-06-29-submodule-support-design.md) (§5.4 pin-validity lens line ~158 — it prescribes an auditor `git -C <submodule> fetch`, which this change removes; align it — Step 5b).
...
- [ ] **Step 2 — Add the deny-cases + a cat-file allow-case.** In `validate-auditor-git.test.sh`, add: (a) `git -C /abs/sub fetch` → assert exit 2 with a `WAR:` deny marker; (b) bare `git fetch` → exit 2 with `WAR:`. Confirm the existing read-verb allow-cases **`-C <path> rev-parse` (H1) and `-C <path> show` (H2)** still pass (NOTE: there is currently **no** `-C cat-file` allow-case — so **add** `git -C /abs/sub cat-file -e <oid>` → **allow**, since the reworded lens's optional confirmation relies on `cat-file` being permitted). Together these prove the resolution did **not** widen the allowlist beyond the already-allowed read verbs. Run → **GREEN**.
...
- [ ] **Step 5 — Grep-verify the prose.** Verify the fetch **command invocation** is gone — NOT the substring `fetch` (the reworded lens DELIBERATELY keeps a bare-`git fetch` rationale line — `` Do NOT `git fetch` here `` — + a "need not be fetched" citation; those must STAY, so any grep that matches a two-word `git fetch` will false-fail). Anchor to the **command form only** (it always carries `-C <path>`): assert `grep -nE 'git -C [^ ]+ fetch' agents/war-auditor.md` returns **nothing** — this matches `git -C <submodule> fetch` (the removed command) but NOT the bare `git fetch` rationale (no `-C`). AND the lens still names the dep-task landed-SHA ledger match as authoritative and cites the pre-flight reconciliation; AND the deliberate "do NOT `git fetch`" rationale prose is **present** (expected to stay — do not strip it).
- [ ] **Step 5b — Align the submodule-support spec (§5.4 pin-validity lens).** `docs/specs/2026-06-29-submodule-support-design.md` §5.4 (line ~158) prescribes the auditor validating "new SHA reachable on the submodule remote (`git -C <submodule> fetch && git cat-file -e <oid>`)" — a **claim that the auditor runs `git fetch`**, which this change removes (the guard denies it). REWORD that line to match the no-fetch lens: the auditor validates the new SHA **equals the dep's landed SHA** (ledger match — authoritative); remote-reachability is **established upstream** (dep-task land + Lead pre-flight reconciliation), not re-verified by an auditor fetch; a read-only `git -C <submodule> cat-file -e <oid>` is an **optional non-blocking** confirmation (its absence is not a finding). Post-condition: `grep -nE 'git -C <submodule> fetch' docs/specs/2026-06-29-submodule-support-design.md` returns nothing; the two files' pin-validity descriptions now agree.
