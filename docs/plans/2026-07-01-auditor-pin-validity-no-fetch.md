...
- [ ] **Step 5 — Grep-verify the prose.** Verify the fetch **command invocation** is gone — NOT the substring `fetch` (the reworded lens DELIBERATELY keeps a `do NOT `git fetch`` rationale line + a "need not be fetched" citation, so a bare `grep -n 'fetch'` will still match and MUST NOT be read as failure). Assert: `grep -nE 'git( -C [^ ]+)? fetch' agents/war-auditor.md` returns **nothing** (no `git fetch` / `git -C <sub> fetch` command remains in the pin-validity block); AND the lens still names the dep-task landed-SHA ledger match as authoritative and cites the pre-flight reconciliation; AND the deliberate "do NOT `git fetch`" rationale prose is **present** (expected to stay — do not strip it).
- [ ] **Step 5b — Verify-and-close the conditional cross-note.** Spec's Affected-Files table marks `docs/specs/2026-06-29-submodule-support-design.md` "verify-and-touch **only if** it claims the auditor verifies remote-reachability." Verified 2026-07-01: `grep -inE 'auditor.*reachab|reachab.*auditor|auditor.*fetch' docs/specs/2026-06-29-submodule-support-design.md` returns **nothing** — no such claim exists, so the conditional is vacuous → **no edit** (verify-and-close; [[verify-task-no-op-is-correct-when-already-covered]]). Re-grep at execution to confirm it still holds; edit only if a claim has since appeared.
