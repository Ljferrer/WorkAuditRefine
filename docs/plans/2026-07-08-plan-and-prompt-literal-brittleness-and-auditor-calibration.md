...
- **Matrix row-count completeness** (`drift-matrix-stated-vs-covered-cells`) · why deferred: mechanically enforcing rows × columns requires parsing prose matrix dimensions — spec §9 non-goal; the enumerate-all-cells meta-comment stays a convention · runner: the auditor's test-fidelity lens + the enumerate-all-cells convention in the war-strategy template.
