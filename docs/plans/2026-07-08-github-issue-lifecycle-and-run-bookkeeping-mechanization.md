...
8. No shipped script or committed file contains the literal personal account handle; `node skills/_shared/war-memory.mjs lint docs/learnings/` stays green. (spec criterion 12)
9. `skills/war/SKILL.md` names `gh-preflight.sh` before gh write batches (Decompose + Checkpoint), `assert-issues-filed.sh assert` as the Checkpoint gate before the DAG advances, and `--close-epic` as the landed-phase action; `skills/war-room/SKILL.md` surfaces the `overrides.ghUser` knob.
...
- **The floor cannot see a task never recorded in the ledger at all (spec §8 ponytail)** · why deferred: guarding recorded-but-unfiled and done-but-open covers the observed failures; a wholly absent bookkeeping intent surfaces at schema validation and human ledger reads · runner: doctrine note in ADR 0026; ledger schema validation.
