...
Critical path (plan-side, per End state; the chain orders, the file boundary carves): End states 1 and 7 ← T1.2 ← T1.1 (bottleneck: T1.1, because every engine fixture chains from the five files and their census rows). End states 2–6 ← T2.1 ← Phase 1 landed. End state 11 ← T2.2. End state 12 ← T2.3. End state 13 ← T3.1.
