...
End states 1 and 7 ← T1.2 ← T1.1 (bottleneck: T1.1, because every engine fixture chains from the five files and their census rows). End states 2–6 ← T2.1 ← Phase 1 landed. End state 11 ← T2.2. End state 12 ← T2.3. End state 13 ← T3.1.
...
| D15 | Tag drift guard | Four surfaces carry the relation-tag vocabulary: the audit rules block, the fixer file's Round 2 table, the examples file's H2 list, the engine regex. One guard asserts H2 set = tag set ∪ {convergence}; T1.2 binds the three file surfaces, T2.1 adds the engine regex | (verified: the `PLAN_DEFECT_SENTINEL` source-text extraction idiom in `workflow-template.test.mjs` at `668e4ff9`) — the fixture reads the three files and extracts the regex literal from the template source text | slice (T1.2, T2.1) |
...
| A4 | The history digest stays compact: titles and tags, plus full text for the survival-registry blockers only | operator ruling at the mid-budget checkpoint | the fixer misses a detail it cannot recover, because the audit log is in-memory until phase end | End state 6 (check) |
...
- **End state:**
  ...
  6. From corrective round 2 the fixer and audit prompts carry the history digest with the D11 field set, the relation-sequence line, the worker's `Critical path:` block, and full text for the survival-registry blockers only · check: `node --test skills/war/assets/workflow-template.test.mjs`
...
Phase 1 (five reference files + census rows, wave 1; skeleton guard, wave 2) → Phase 2 (engine + cards; plan-author pointers; ADR + glossary + schemas) → Phase 3 (release).
...
- Plan slice: author `backward-chain.test.mjs`: pin each file's exact H2 set and rule count, the examples index line, the entry-shape fields, the seed-entry slugs, and the three-file tag guard (audit rules block tag list = fixer Round 2 table rows = examples H2 set minus `convergence`); a positive control per assertion (mutate a copy in a temp dir, assert red). Zero-commit or count-only assertions are defects (delete-the-feature probe). Wave 2: the guard reads the five files, so it carries the `deps` edge (§3 rule 7).
...
- Done when: `grep -Fq 'corrective round' CONTEXT.md && grep -Fq 'relation:' skills/war/references/schemas.md && ls docs/adr/*backward-chain-doctrine*.md && echo DOCS_OK`
- requiresTest: false
- requiresPackaging: false
- deps: []
- target repo: superproject
