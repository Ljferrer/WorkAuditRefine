...
- **Collateral doc-rot from the message reword (red-team Minor, non-blocking):** `docs/learnings/lessons-learned-tooling-traps.md` currently asserts the war-memory refuse message "states it archives ALL candidates" — factually stale once Task 1.1 rewords that message to *list*. Out of Task 1.1's code+test file scope (it is a committed learning, not a call-site); the servitor / next `/lessons-learned` housekeeping pass refreshes that one sentence post-land. The Task 1.1 OLD-absent grep guards the message source itself; this note tracks the downstream learning-doc line so it is not silently left wrong.
