...
primitive the engine uses today; the sandbox also offers `pipeline()` (verified: `workflow-template.js`
header and Workflow tool contract at current tip). The live `parallel()` NULLs a rejected thunk
...
| D2 | Pin-transfer predicate | patch equality: conflict-free rebase AND the task's own diff (base→tip) byte-equal before vs after rebase; mismatch ⇒ in-lock full-panel re-audit for that task only | (user) | PIN-1 · guardrail ‡, PIN-7 · end-state |
...
| D5 | Doc sweep | OLD per-site wording absent from the four live surfaces; CHANGELOG untouched; per-site lesson gains a dated RESOLVED note | (user) | end-state |
...
|----|-----------|-------|----------------------|-------|
| A1 | `aceBisect` and `aceReentry` hoist wave-side with the ace (they close over merge-queue scope today) | [assumed: mechanical hoist — if wrong: bisection stays in-lock and the wall-clock win shrinks] | partial win only | End state 3's structure test |
...
| A3 | an ace result without `changed_files[]` routes to the full panel | [assumed: fail-safe default — if wrong: over-audit only, never under-audit] | wall-clock only | End state 4's test |
...
- **Method:** gate every agent dispatch through one global counting semaphore (a shared counter
  ...
  merge slot the refiner checks patch equality: the task's own diff, base to tip, must be
  byte-equal before and after the rebase, and the rebase must be conflict-free. On a match the
  audit pin transfers. On a mismatch that one task falls back to the in-lock full-panel
  re-audit, today's behaviour (PIN-1). The merge-floor retry loop stays in-lock and full-panel,
  unchanged. One new ADR records seat-approval transfer, rebase pin transfer, and the global
...
- **Mechanism latitude:** the semaphore's internal shape (counter plus waiter queue, whether
  `batched()` is retired or wrapped, and the census-test rewrite that follows), the
  `changed_files[]` field's exact name and position in the ace result schema, script versus
  inline git steps for the patch-equality check, the RESOLVED note's wording, and structure-test
  shapes. Substituting any of these mechanisms while the End states and binding guardrails hold
...
- **Binding guardrails:** PIN-1 to PIN-12 verbatim. Unanimity survives in transferred form:
  ...
  `run.maxParallel` shape in `war-config.mjs` is untouched. No check, gate, floor, or backstop
  is waived (ADR 0017).
- **End state:**
  1. With `run.maxParallel: N` set, at most N agent dispatches run at once across every site,
     wave and roster nesting included · check: `node --test skills/war/assets/workflow-template.test.mjs`
     (new concurrency-recording tests, printed pass count).
  ...
  3. The ace batch, re-audit, and bisection dispatch on the wave side, before the merge queue
     starts · check: `node --test skills/war/assets/workflow-template.test.mjs` (structure test
     on dispatch order).
  4. Re-audit scale follows the file-level subset rule, full panel on any file outside the
     findings' set, full panel when `changed_files[]` is absent · check:
     `node --test skills/war/assets/workflow-template.test.mjs`.
  5. The merge slot transfers the pin on patch equality and falls back to the in-lock
     full-panel re-audit on mismatch, with a positive test of the recorded `reauditedTip` and
     `rebasedTip` (PIN-7) · check: `node --test skills/war/assets/workflow-template.test.mjs`.
  ...
  9. The transfer rule appears in both prompt layers · check:
     ...
     (two separate checks, one decisive token each).
...
- Plan slice: add a counting semaphore (counter plus waiter queue, hand-rolled — the sandbox
  cannot import) acquired around each agent dispatch when `run.maxParallel` is set. Retire or
  subordinate the four `batched()` call sites under it (PIN-4). Preserve the #742 invariant: a
  rejected dispatch resolves to null, never rejects a group, results stay in input order.
  Absent knob ⇒ byte-identical dispatch path (PIN-3). Rewrite the census tests for the new seam
  (exact-substring census, absent-knob row, new concurrency-recording tests driving a fake
  `parallel`). Reword the in-file comment surfaces (args-contract header, the #1722 block, the
...
- Plan slice: move the ace batch dispatch, its re-audit, `aceBisect`, and `aceReentry` from the
  ...
  path (PIN-12). Extend the ace result schema with `changed_files[]`; absent ⇒ full panel (A3).
  Apply the file-level subset rule (D3) to choose originating-seat-only vs full-panel re-audit;
  ...
  dispatch labels (PIN-8). Land the transfer rule's wording in BOTH prompt layers here — the
