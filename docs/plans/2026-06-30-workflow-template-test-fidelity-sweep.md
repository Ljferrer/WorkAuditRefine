...
- **Target version:** **v0.8.6** (roadmap landOrder 6, severity LOW). Bumps `0.8.5 → 0.8.6`. (History: the six-spec stack was re-baselined off master v0.8.0 after the two submodule increments and lands v0.8.1…v0.8.6 in serial order; as of 2026-07-01 specs 1–5 have landed, so master is now **v0.8.5** and this slug is last at v0.8.6.)
...
- **Four-slot serial land (replace-in-place, no badge):** `.claude-plugin/plugin.json` `version`; `.claude-plugin/marketplace.json` `metadata.version` **and** `plugins[0].version`; `README.md` `## Status` (line ~236). All four read `0.8.6` after the release task; no other slot left at a prior version. (**Re-grounded 2026-07-01:** specs 1–5 have landed — the stacked base / master reads `0.8.5` in all four slots, so this task bumps `0.8.5 → 0.8.6`; the earlier "0.8.0" grounding prose was stale.) There is no cross-slot consistency test — verify all four by hand ([[version-slots-no-cross-slot-consistency-test]]).
- **Standalone fallback:** if this is run **off a tip other than the v0.8.5 stack base**, **re-baseline the release to the next free patch off the live tip** and **drop the prior-tip pin** — the version literal is NOT authoritative; the operator directive + the actual landed baseline is ([[stacked-per-branch-releases-make-main-lag-cumulative]]). (Both stack and current master are v0.8.5, so the primary `0.8.5 → 0.8.6` path and the fallback converge.) The six test-prose edits (#266/#267/#250/#221/#317/#326) are baseline-independent and unaffected by the fallback.
...
- [ ] **Step 3 — GREEN (L3 T1 is the load-bearing proof).** Run `node --test --test-name-pattern='L3 T1' skills/war/assets/workflow-template.test.mjs` → still **passes** (the colon-anchored regex captures the full arrow, all four totality assertions hold — a truncated/mis-captured regex fails this `new Function` eval loudly). The tightened `:\s*null\)` terminator is visible in the one-line diff (no fragile shell-escaped post-condition grep needed — the earlier `':\\s\\*null\\\\)'` form was over-escaped and matched nothing). Then run the **full** gate → green.
