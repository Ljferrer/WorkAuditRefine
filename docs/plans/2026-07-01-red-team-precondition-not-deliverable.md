...
- [ ] **Step 1 — RED: write the scaffold assertions first.** In `workflow-scaffold.test.mjs`, assert (all **automated**, so each fails if its target text is removed) that the composed prompt for **each analyzed probe** (`claims-vs-reality`, `consistency-placeholders`, `coverage-vs-source`, and a representative bespoke analyzed probe): **(1a)** contains a stable rule token (e.g. `PRECONDITION vs DELIVERABLE`); **(1b) contains a stable phrase from the retained-findings carve-out** (e.g. `false claim about EXISTING code` — this is the **load-bearing non-blunting assertion**: because it is an `assert.match` on that phrase, deleting or blunting the carve-out to a bare "ignore proposed changes" makes Step 1 itself go RED, satisfying spec Validation #2's "asserted present," NOT a downstream manual check); **(1c)** the `executed` spine probe (`executable-proof`) prompt does **not** gain the rule. Run → **RED** (tokens absent).
...
- [ ] **Step 4 — not-blunting is guaranteed by Step 1 (1b), not a manual check.** The load-bearing guarantee is the **automated** assertion `1b`: because the test `assert.match`es on the carve-out phrase, blunting the rule to "ignore proposed changes" makes Step 1 go RED. Document this here (WHY 1b is load-bearing: over-correcting the rule would strip the carve-out and fail the test) — this step adds no new manual gate; spec Validation #2 ("asserted present") is satisfied by 1b.
