...
- **Integration base:** **latest `origin/master`** (anchors grounded at `72d07c7` v0.8.11; red-team verified unchanged through `95b385f` — the PR #395 docs-only merge moved no plan-relevant file). Branch off the live tip, not the literal sha. This plan **lands FIRST** in the batch — it is the sole behavioral change; specs 13/14 (docs/test hygiene) stack on its landed tip.
...
- **Spec corrections caught in grilling (the spec's own text is wrong here — follow THIS):**
  ...
  - **(f) No guard change.** `hooks/validate-auditor-git.sh` allowlists `cat-file` and peels global `-C <path>` before re-validating the subcommand, so `git -C <refineryPath> cat-file -t <sha>` passes ([[guard-c-peel-resolves-pin-blocker-but-scoped-to-read-verbs-only]]). Corollary baked into the prompt wording: when the pin **is** the sentinel, the command carries **parens**, which are outside the guard's char-allowlist (**space is permitted** — `A-Za-z0-9 ./_=:,@^-`; red-team executed proof: deny message names `()` only) → denied → "command cannot run" → the existing cannot-confirm SOFT path, exactly as D2 intends. Do not drop the parens from the sentinel text — they are what makes the guard deny it. #393's stale "Related" sub-item (a plugin-root/worktree divergence) is not in scope.
