...
1. `skills/war/assets/version-slots.test.mjs` passes with all four slots aligned; manually bumping `plugin.json#version` alone makes it fail under `node --test 'skills/**/*.test.mjs'`; a `## Status` paragraph naming a different version also fails. Root resolved from `import.meta.url`, never `process.cwd()`. Reintroducing the undersell phrase `three version-of-truth files` into `README.md` also fails the test (spec criterion 1; **spec criterion 2 restored** — `README.md:316` reworded to name the four slots across three files, guarded here). (spec criterion 1)
2. Editing the inline `HARD_ESCALATION_REASONS` (the deepEqual row), emitting a `landDecision` string literal outside `KNOWN_LAND_DECISIONS` (the ⊆ subset row — the template emits a documented 6-of-7 subset, there is no inline `KNOWN_LAND_DECISIONS` array), or diverging any mirrored roster helper (`validateRoster`/`widenRoster`/`resolveWidenSource`, and the curried `spawn` vs canonical `spawnOpts`) in `workflow-template.js` from its canonical export makes the mirror-registry test fail; the registry lists at least those six rows. (spec criterion 3)
...
5. Adding a `PRESETS` entry or a new role without the enumerated matrix covering it makes the matrix-completeness test fail; the matrix covers every current preset (`thorough`, `economy`, `balanced`) × every role (`worker`/`auditor`/`refiner`/`servitor`); the cascading-impact lens carries the matrix-consumption duty (Task 1.5) — the matrix's landed reader (spec §4.5/§8). (spec criterion 7)
...
10. `skills/red-team/SKILL.md`'s spine-**doctrine** section (the Lead runs these probes each red-team, as with the backstop-legitimacy check — **not** the hardcoded `const SPINE` array in `workflow-scaffold.js`, which plan 8 owns) and `references/lenses.md` carry the two probe descriptions (unguarded new mirror; default-flip gate asserting only NEW-present); a grep asserts both present. (spec criterion 6, validation half — the declared Task 1.7 backstop)
...
- Files: `skills/war/assets/version-slots.test.mjs` (new), `README.md`
- Plan slice: Framework-free `node:test`. Resolve repo root from `import.meta.url` (subagent cwd is the main repo; cwd resets between bash calls — never `process.cwd()`). Read `.claude-plugin/plugin.json` → `version`; `.claude-plugin/marketplace.json` → `metadata.version` and `plugins[0].version`; `README.md` → the paragraph under `## Status` (regex-extract the leading version token by construct). Assert all three JSON strings `===` each other and the Status token equals them. **Also assert the slot-count doc contract:** `README.md`'s version-bump instruction must NOT undersell the four slots — the literal phrase `three version-of-truth files` must be **absent** (its presence is the recorded undersell rot; the corrected wording names the four slots across three files). Fail-closed. **Rescope note (operator-ratified, narrowed at red-team round 2):** decision #7's phantom-*badge* prose fix is dropped — no release-plan-template surface and no badge *instruction* exists under `skills/` (the lone `grep 'badge' skills/` hit is a `campaign-ledger.test.mjs` fixture asserting a badge's *absence*). BUT decision #7(b)'s **three-files/four-slots undersell** fix is NOT moot: `README.md:316` literally reads "ALL three version-of-truth files" (the learning `release-bump-slots-canonical-no-badge.md` names it as the grep-cue) — **this same Phase-1 task both rewords `README.md:316`** (→ name the four slots across three files; the two `marketplace.json` fields make four slots across three files) **and guards it** with the absence-assert above. Reword + guard in one task so the guard is green the moment it lands (a Phase-2 reword would leave the Phase-1 guard red). Task 2.1's later `## Status` bump is a separate README edit in Phase 2. (spec criterion 2, restored.) Do not invent a template to fix.
- requiresTest: true
...
- Plan slice: All mirror pairs, one task (standing-vs-dispatched coverage split). **(D2 — mirror registry)** In `workflow-template.test.mjs`, generalize the scattered membership checks into a registry array of `{ canonical, extractInline, mode }` rows, one assertion per row: `HARD_ESCALATION_REASONS` (**deepEqual**, regex-extracted inline array vs the `land-decision.mjs` export — a full inline mirror); the emitted `landDecision` literals vs `KNOWN_LAND_DECISIONS` (**subset ⊆, NOT deepEqual** — there is no inline `KNOWN_LAND_DECISIONS` array; the template computes `landDecision` from a ternary + scattered assignments and deliberately emits a documented proper subset, "6 emitted ⊆ the 7-member export" per the template comment, so the row extracts the emitted string literals and asserts each ∈ the export); and the four mirrored roster helpers — `validateRoster`/`widenRoster`/`resolveWidenSource` are genuine same-name inline mirrors, while the fourth canonical `spawnOpts(config, role)` is mirrored inline as the **curried `spawn = role => …`** (closes over the phase-local `agents`, defaults via a hardcoded `ROLE_MODEL` table), so its `extractInline` targets `spawn` and the behavioral-equivalence comparison must **adapt inputs across the arity difference** (`ROLE_MODEL` ↔ `DEFAULTS.agents` model values is itself a distinct latent mirror this row does not by itself guard) — the "Mirror of war-config.mjs …" comment block names all four. Const rows assert on parsed literals (deepEqual for a full mirror, ⊆ for the subset-emitter); helper rows assert **behavioral equivalence on an enumerated fixture-input set** (run canonical and inline on the adapted same inputs, deepEqual the outputs — function mirrors can't byte-compare safely). `// ponytail:` the registry is the deliberate ceiling; no AST scanner. Existing overlapping checks (e.g. "inline includes land_stale") are absorbed, not duplicated. Keep `land-decision.test.mjs`'s own guards untouched (different file, plan-1/-2 contention). **(D3 — both-surfaces registry)** Generalize the existing T1 both-surfaces pattern into a registry of correctness-critical directives, each row naming its standing surface and its dispatched surface(s) — token-anchored, case-tolerant (`-i`, mid-sentence anchors), never full-line bytes — **including rows asserted against the inline gate-audit seat prompt strings** (`execution-evidence`, end-state) extracted from template `src`, which sit outside `auditPrompt()`. Seed rows: the existing T1 servitor mutation-guard/recurrence-flow directives (migrated into registry form), the D8 obligation, the D9 directive, the D12 narrative duty (below). **(D8)** Add the ADR-policy-table obligation to the **cascading-impact** lens in `agents/war-auditor.md` AND its dispatched mirror in `auditPrompt()` (there is **no** `doc-honesty` lens — cascading-impact is the lens that follows a changed mechanism to its dependent docs — the same lens D9 (below) extends with the comment-lag standing duty, so the doc-drift duties (D8, D9, D12, and the D6 matrix-consumption duty below) land on cascading-impact in this task; `auditPrompt()` is the dispatched surface the both-surfaces row binds): when a task changes a mechanism attribution, check the ADR chosen-option/policy-table row for that mechanism (grep misses under-attribution — this is a lens duty, not a grep). **(D9 — prompt-directive only, operator-ratified; no advisory script)** Add to the dispatched worker prompt AND `agents/war-worker.md`: before commit, grep your touched files for the old behavior's concrete terms (retired values, old approach names, stale counts) and update lagging comments/JSDoc; ADD the comment-lag standing duty to the cascading-impact lens in `agents/war-auditor.md` (+ its `auditPrompt()` dispatched mirror) so the auditor enforces it standingly — it is NOT currently present on that lens. **(D12 auditor half)** Add the mechanism-style-narrative duty to the **cascading-impact** lens (both surfaces — `agents/war-auditor.md` + its `auditPrompt()` dispatched mirror; same lens as D8/D9, not a `doc-honesty` lens): narrative docs assert invariants and the guard that holds them, never snapshot counts. **(D6 matrix-consumption — the spec's designated matrix consumer, §4.5/§8)** Also add to the **cascading-impact** lens (both surfaces — `agents/war-auditor.md` + its `auditPrompt()` dispatched mirror): when a task changes a role/model/effort config claim in prose or docs, consult the exported `(preset, role, model, effort)` matrix from `war-config.mjs` (Task 1.2's export) as the source of truth, not just `DEFAULTS.agents`. This is the **landed reader** of the Task 1.2 matrix — the D6-residual backstop's named runner (without it the matrix has no consumer). Registry tests and their clause edits land together in this task — the registry is red without the clauses.
- requiresTest: true
- requiresPackaging: false
- deps: none
- target repo: superproject
...
- **Preset matrix ≠ prose correctness (D6 residual)** — the matrix guarantees every preset/role is *watched*, not that prose consulted it · why deferred: prose reading is a judgment act · runner: the **Task 1.5 cascading-impact-lens matrix-consumption duty** — it ships this campaign as the matrix's landed reader (there is no `doc-honesty` lens); what remains deferred is whether the auditor's judgment actually catches a given prose drift, a judgment act not mechanically verifiable.

## Notes / conscious deviations

- **Three operator-ratified rescopes against the spec's letter (2026-07-08 conversion volley):** (a) D7's phantom-*badge* prose fix **dropped** (no badge/template surface exists), but its sibling three-files/four-slots undersell fix (`README.md:316` "ALL three version-of-truth files") is **RETAINED** — Task 1.1 rewords + guards it in one Phase-1 task (red-team round-2 correction); (b) D10's example-list fix **already landed** — test-only; (c) D12 **retargeted** — the spec's quoted phrase is gone, the live rot is step 17's "8 reasons — the same 8" count and "≈841" line ref. All twelve spec *decisions* stand; only three tasks' factual premises were corrected (recorded pattern: plan-rescope-note-supersedes-stale-spec-surgery-list).
