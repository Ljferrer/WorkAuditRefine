...
- **End state:**
  ...
  2. Rendering the live corpus post-restructure yields ≤ 17,000 B (verdict `ok`) with zero archives.
  ...
  5. `CONTEXT.md` carries the three new terms and two sharpened entries; `CLAUDE.md`'s budget sentence
     points at tighten.
...
- Plan slice: In `buildProjection`: emit 2-column rows `| [[slug]] | <summary> |` where the summary cell
  ...
  `inboundCiters`; never lessons created/recurrence-stamped within 14 days), rank eligible rows by
  ascending hits with ties by tier rank then age, and emit a JSON plan — eligible list in eviction
  order with per-slug `{hits, tier, ageDays, inbound, bytesFreed}` plus the cumulative cut line to
  ...
  wrong order must fail); floors fixtures (one per floor); per-entry hit dedupe; log-absent fallback
  ordering equals tier+age; dupe both-or-nothing assertion; fixture-corpus bound asserted as
  ≤ 17,000 B (never a byte literal for the rendered size).
- requiresTest: true
...
- Plan slice: Add the `tighten` mode alongside `migrate`/`evict` in the argument dispatch, documenting
  the five-step run shape: **preflight** (read-only render-state check; ≤ target ⇒ "nothing to tighten",
  stop) → **plan** (consume the `tighten-plan` verb's JSON — the verb is the corpus authority, the skill
...
- Plan slice: In `CONTEXT.md`: add **Tighten pass**, **Cell budget**, **Usage-scored eviction** (spec
  §6 wording basis); sharpen **Index projection** (capped by selection **and per-cell budgets**, both
  view-mechanisms — knowledge stays in files) and the advisory-line language (trigger **and** exit
  target of tighten). Author the ADR "the index projection is a bounded two-column view" — cell budgets
...
- End state 2 on the **live** corpus (≤ 17,000 B, verdict `ok`, zero archives) · why deferred: depends
...
- The projection format change deliberately preserves the safe-swap extractors (first `[[slug]]` per
  ...
  an auditor finding it unmodified is confirming the design, not catching an omission.
