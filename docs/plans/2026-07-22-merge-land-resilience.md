...
- **End state:**
  ...
  8. `skill-doc-contracts.test.mjs`: a new row extracts SKILL.md's `gate_failed`-routing
     ...
     **and** the gate-time zero-retry claim absent within that construct, with a non-vacuous
     presence companion.
  9. Full-phrase greps over `skills/war/` + `agents/` return zero hits for "zero-round env-blocked
     escalation", "an environmental failure passes on retry", and "env-blocked doctrine applied at
     gate time"; the §4.5 same-scope hand-scan is performed and every same-meaning straggler is
     listed as a survey-derived correction (the schemas.md `gate_failure_class` environment-value
     sentence is one, already identified at plan time).
...
- Plan slice: **Merge site (spec §4.1)** — in the serial-merge routing under
  ...
  `escalated.push({ reason: 'env-blocked', detail: mr })` with a bounded recovery structurally
  ...
  **Anchor safety:** every landDecision literal assigned is already in the emitted set; keep all
  ...
  exact). Rewrite both gate-site environment-arm comments to the bounded-retry mechanics — the
  literal "an environmental failure passes on retry" must not survive in
  `workflow-template.js`. **Refiner card (spec §4.3, both-surfaces rule — same commit):** in
  ...
  recovered land; plus a new D3 both-surfaces registry row — surfaces `['war-refiner.md',
  ...
  fallback), token-anchored (`/environment-proceed/i`, `/fresh TMPDIR/i`, a bounded/one-re-run
  anchor, a green-required anchor) — and bump the `REGISTRY.length` floor and its enumerating
  message to the new exact row count (floor equals true count, no slack — #693).
- requiresTest: true
...
- Plan slice: **SKILL.md** — rewrite the `gate_failed`-routing **`environment`** arm (the bullet
  ...
  **Doc-contract row (spec decision 10; mapped test, End state 8):** a new row in
  ...
  in the provision `env-blocked` bullet), asserting the OLD claim absent within the construct (the
  "env-blocked doctrine applied at gate time" phrase / an unconditional "0 FIX rounds" gate-time
  pairing) and the new mechanics present (`/environment-proceed/i`, a green-required anchor,
  ...
  (spec §4.5; End state 9):** after rebasing onto the integrated tip carrying 1.1's rewrites, run
  the full-phrase greps over `skills/war/` and `agents/` for "zero-round env-blocked escalation",
  "an environmental failure passes on retry", and "env-blocked doctrine applied at gate time" —
  zero hits required (scope excludes `docs/` — specs, plans, and red-team reports quote the
  retired phrases with legitimate history). Grep is a floor: hand-scan SKILL.md's Checkpoint
  ...
  env-blocked"); list every straggler corrected as a survey-derived correction in the task report.
  The GateGuard "passes on retry" lines in `agents/war-worker.md` and
...
- **Cross-plan shared-file contention (spec §8):** `workflow-template.js`,
  `workflow-template.test.mjs`, and `agents/war-refiner.md` are touched by sibling plans in this
  survey (at minimum `2026-07-22-auditor-guard-ergonomics.md` Task 1.2 touches all three, and the
  audit-adjudication-threading / servitor-wrapup-landed-tip groups touch the same files), and the
