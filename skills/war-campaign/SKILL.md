...
- **Campaign ledger** (`.claude/campaigns/<id>/ledger.json`) — the plan queue + per-plan status/branch/PR/SHA/stop-point/backstops/red-team rounds. Single-writer (only the campaign Lead's `sweep`/`record` touch it); every write atomic. The entry shape's maintained home is the `campaign-ledger.mjs` helper header + its test ([ADR 0046](../../docs/adr/0046-specs-are-posterity-skills-cite-maintained-surfaces.md)).
