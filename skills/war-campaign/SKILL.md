...
1. **Materialize, then sweep + select.** Ordering is forced: `sweep` reads each drop's plan file (`extractFilesFromPlanFile`), so materialization runs **first** — the lifecycle within this step is `[Materialize → sweep() → next]`.
   ...
   - **Sweep + select.** `campaign-ledger.mjs sweep --campaign <dir>` — contention-checks every dropped `inbox/` plan against the existing queue and against each other, inserts in dependency-safe (deterministic) order, deletes the consumed inbox files. A drop whose resolved path already has a non-`landed` entry (in the ledger, or from an earlier drop in the same sweep) is **not** inserted again: `sweep` refreshes the existing entry's `files` and reports it under `skipped: [{ plan, reason: 'refreshed' }]` — only a `landed` entry lets a re-drop append a fresh queued entry. Then `campaign-ledger.mjs next --campaign <dir>` to pick the plan to run. The **fail-loud backstop** stays: a drop whose line-1 path is still missing at `sweep` time throws ENOENT (a skipped materialization can never pass silently) — the executable owner of End-state #3, without moving git I/O into the stdlib helper.
