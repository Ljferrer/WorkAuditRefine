...
## Lifecycle (per plan, in queue order)
...
1. **Materialize, then sweep + select.** Ordering is forced: `sweep` reads each drop's plan file (`extractFilesFromPlanFile`), so materialization runs **first** — the lifecycle within this step is `[Materialize → sweep() → next]`.
   ...
   - **Sweep + select.** `campaign-ledger.mjs sweep --campaign <dir>` — contention-checks every dropped `inbox/` plan against the existing queue and against each other, inserts in dependency-safe (deterministic) order, deletes the consumed inbox files. A drop whose resolved path already has an entry — in the ledger (`landed` or not), or from an earlier drop in the same sweep — is **not** inserted again: `sweep` refreshes the existing entry's `files` and reports it under `skipped: [{ plan, reason }]` (`'refreshed'` for a non-`landed` match, `'landed'` for a landed one, whose recorded fields are left byte-intact apart from `files` — so `record()`'s first-match can never overwrite a landed run and no permanently-`queued` sibling can keep `is_active` true); a deliberate re-run of a landed plan is an explicit operator act, never a sweep side effect. Then `campaign-ledger.mjs next --campaign <dir>` to pick the plan to run. The **fail-loud backstop** stays: a drop whose line-1 path is still missing at `sweep` time throws ENOENT (a skipped materialization can never pass silently) — the executable owner of End-state #3, without moving git I/O into the stdlib helper.
...
3. **Harden — the three-arm triage.** Run `/red-team <plan>`, self-adjudicating under AFK, then triage its verdict + gate output:
   - **(a) Proceed** — `CLEARED`, `CLEARED-WITH-NOTES`, or `ADJUDICATED` (`ADJUDICATED` proceeds unattended under `--afk`): `git add` + commit the patched plan **and** the red-team report onto `dev/<slug>` — the same load-bearing-handoff rationale as step 2: `/war`'s worktrees and any resumer only see what is committed on the branch, never the Lead checkout's uncommitted patches. Then `campaign-ledger.mjs record --campaign <dir> --plan <path> --redteamRounds <n>` (the report's `**Rounds:**` header) and proceed to step 4. A proceed verdict commits-and-continues regardless of any routing metadata — the gate's invariant makes `routeUpstream: true` imply `BLOCKED`, so this arm and arm (b) never collide.
   - **(b) Route upstream** — the gate emits `routeUpstream: true`, or `BLOCKED` with residual open questions: halt-and-hold (Failure section) with `stopPoint: redteam-route-upstream`, stamping `--redteamRounds <n>` (the report's `**Rounds:**` header) alongside `--stopPoint` in the same `campaign-ledger.mjs record` call — the wrap-up's hardening row sources rounds from the ledger, and the halted plan is exactly the one whose round count it exists to surface. CAMPAIGN-STATE.md carries the `/war-strategy` regrill command plus the report's `## Route upstream` agenda (the handoff block — [loop-budget doctrine](../red-team/references/loop-budget.md)); re-entry is `/war-campaign add <plan>` after the regrill (the entry is still queued; `sweep` refreshes it idempotently, never duplicating — a bare `/war-campaign` — its resume invocation — equally re-picks it) — never skip-and-continue: the stack lands plans in order ([ADR 0011](../../docs/adr/0011-campaign-stack-and-plow-branch-model.md)).
   - **(c) Persistent `INCOMPLETE`** — the same halt arm as (b): halt-and-hold, `stopPoint: redteam-route-upstream`, stamping `--redteamRounds <n>` in the same `record` call as `--stopPoint`, regrill command + agenda in CAMPAIGN-STATE.md, re-entry via `/war-campaign add` (the entry is still queued; `sweep` refreshes it idempotently and never duplicates — a bare `/war-campaign`, its resume invocation, equally re-picks it).
4. **Execute.** Run `/war <plan> --working dev/<slug-N> --landing dev/<slug-(N-1)> --campaign-ordinal <N> --afk --ace` (plan 1 lands to `master`). The ordinal is the plan's **1-based position in the ledger's `plans[]` queue at selection time** (the step-1 `next` pick — the same position the `dev/<slug-N>` stacking derives from); `/war` stamps it onto the `war-c<K>-…` staged phase-script basenames and never scans campaign ledgers to derive its ordinal; under a campaign it does read the ledger for the phase-close contention set ([sweep-exclusion.md](../war/references/sweep-exclusion.md)); a plain `/war` run omits the flag and the `c<K>` segment is simply absent.
5. **Record.** `campaign-ledger.mjs record --campaign <dir> --plan <path> --status landed --branch <b> --pr <n> --sha <sha> --stopPoint <token> --backstops <json>` — atomic temp+rename, so a laptop-close mid-write never corrupts the queue. Omitted flags leave the entry's existing values untouched (`--stopPoint` is set on halt-and-hold, not on a clean land; `--redteamRounds` was already stamped by step 3 — every arm stamps it — and survives here). `--backstops` carries that plan's `/war` handoff `backstops[]` (the validations its run deferred, schemas.md) as a JSON array string, stamped onto the plan's ledger entry for the wrap-up aggregate; omit it for a plan that deferred nothing.
...
- **Campaign ledger** (`.claude/campaigns/<id>/ledger.json`) — the plan queue; each entry's shape is `makePlanEntry` in [`assets/campaign-ledger.mjs`](assets/campaign-ledger.mjs), the shape's maintained home ([ADR 0046](../../docs/adr/0046-specs-are-posterity-skills-cite-maintained-surfaces.md)) — cited rather than re-enumerated here, so the field set cannot rot on this surface (ADR 0025). Single-writer (only the campaign Lead's `sweep`/`record` touch it); every write atomic.
...
**Halt-and-hold** when step 3's triage lands in arm (b) or (c) — `routeUpstream: true`, `BLOCKED` with residual open questions, or persistent `INCOMPLETE` under AFK, recorded as `stopPoint: redteam-route-upstream` — or `/war` hard-halts (`audit-blocked`, `conflict`, dead phase, `land_stale`, `held:submodule-pr`): checkpoint → record the stop point → `PushNotification` → stop. Nothing above the failed plan starts; everything below it has already landed as stacked PRs.
...
## Campaign wrap-up — Unexecuted backstops & per-plan hardening
...
The wrap-up also renders a **per-plan hardening row** — *rounds · blockers · adjudications · report path* — one row per plan: rounds from the ledger's `redteamRounds` (`n/a` when null or absent — a pre-hardening or in-flight entry), blockers and adjudications counted from that plan's red-team report, and the report path (`docs/red-team/YYYY-MM-DD-<plan-slug>.md`). Any value without a source renders `n/a`, never fabricated.
