...
3. **Harden — the three-arm triage.** Run `/red-team <plan>`, self-adjudicating under AFK, then triage its verdict + gate output:
   ...
   - **(b) Route upstream** — the gate emits `routeUpstream: true`, or `BLOCKED` with residual open questions: halt-and-hold (Failure section) with `stopPoint: redteam-route-upstream`. CAMPAIGN-STATE.md carries the `/war-strategy` regrill command plus the report's `## Route upstream` agenda (the handoff block — [loop-budget doctrine](../red-team/references/loop-budget.md)); re-entry is `/war-campaign add <plan>` after the regrill — never skip-and-continue: the stack lands plans in order ([ADR 0011](../../docs/adr/0011-campaign-stack-and-plow-branch-model.md)).
...
5. **Record.** `campaign-ledger.mjs record --campaign <dir> --plan <path> --status landed --branch <b> --pr <n> --sha <sha> --stopPoint <token> --backstops <json>` — atomic temp+rename, so a laptop-close mid-write never corrupts the queue. Omitted flags leave the entry's existing values untouched (`--stopPoint` is set on halt-and-hold, not on a clean land; `--redteamRounds` was already stamped by step 3's proceed arm and survives here). `--backstops` carries that plan's `/war` handoff `backstops[]` (the validations its run deferred, schemas.md) as a JSON array string, stamped onto the plan's ledger entry for the wrap-up aggregate; omit it for a plan that deferred nothing.
