...
2. **Offer overrides.** Ask "anything to change?" and grill **only** the dimensions the user names, one at a time. Allowed values (reject anything else — the validator will too):
   ...
   - `memory.commitLearnings` (bool) — ask this one explicitly, with the pitch: *"commit distilled, lint-scrubbed engineering lessons so they travel with the repo and compound across your team; existing `docs/learnings/` presence is a signal you already do."* **Default stays `false` when the user doesn't ask for it** — a lesson is never committed by default. When `true`, published lessons ride each phase PR under `docs/learnings/` and are human-reviewed like code.
