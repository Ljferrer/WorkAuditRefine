...
2. **Offer overrides.** Ask "anything to change?" and grill **only** the dimensions the user names, one at a time. Allowed values (reject anything else — the validator will too):
   ...
   - **Worker tiers** — refine which model runs which worker dispatch (same `model`/`effort` vocabulary; neither is a phase role):
     - `agents.worker.docs` (default `{ model: sonnet, effort: default }`, every preset inherits it) — the tier for **docs-only tasks**. The predicate is mechanical: a task is docs-tier **iff every `Files:` entry matches `*.md`**, evaluated against the **plan's file list, not the diff**. One non-`.md` file in the task → it falls back to the base worker tier.
     - `agents.worker.fix` (**absent by default → inherits the base worker**) — the tier for **fix rounds and `--ace`** dispatches. Set it to run follow-up work on a different model than the first pass, e.g. first-pass workers on `sonnet` escalating fix rounds to `opus`. Absent = fix-round and ace workers run on the base worker config.
   - `agents.redteam.model` / `agents.redteam.effort` (same value vocabulary) — an **optional** dimension read **fail-open by `/red-team`** to pick the model/effort for its probe and adversarial-confirm sub-agents. Not a phase role and no preset sets it; **absent → red-team inherits the session** (today's behavior). Only worth setting if you run `/red-team` against this repo.
   ...
   - `overrides.gate` / `workingBranch` / `landingBranch` / `learningsTarget` / `testPattern` — `null` lets `/war` auto-detect; a string pins it. `testPattern` is the **test-floor glob set** (space-separated single-star tokens, e.g. `'*.test.ts *.test.tsx'`): `null` = today's floor defaults (`/war` proposes one from the target's test conventions at Setup, pinned **together with the gate** — floor ⊆ gate); a string pins the target's glob set. Its **glob-safe charset is enforced by the validator** — quotes, `;`, backticks, `$`, and newlines are rejected (the value is embedded into an agent-executed shell command).
