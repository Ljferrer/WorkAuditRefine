...
2. **Offer overrides.** Ask "anything to change?" and grill **only** the dimensions the user names, one at a time. Allowed values (reject anything else — the validator will too):
   ...
   - `memory.topK` (integer ≥ 1, default `10`) — max prior-lesson blocks the Lead prefetches per seat. `memory.retrieval` (bool, default `true`) — set `false` to skip the prefetch entirely.
   - `memory.commitLearnings` (bool — default `true`; the economy preset pins `false`) — surface it explicitly so the operator knows lessons will publish: *"distilled, lint-scrubbed engineering lessons are committed under `docs/learnings/` so they travel with the repo and compound across your team — each rides its phase PR and is human-reviewed like code."* Set `false` for a repo whose lessons must stay local-only (the fail-closed redaction lint guards either way).
