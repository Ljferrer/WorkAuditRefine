...
- Close with the operator's options (absorb by hand — per `${CLAUDE_PLUGIN_ROOT}/skills/war/references/fix-round-doctrine.md`, the plugin-root form because the file lives under the `war` skill; file follow-ups on request; drop) — and take **no** action on them unprompted. Loop bound: after two consecutive all-approve rounds with only Nits, stop unless a new commit lands.
