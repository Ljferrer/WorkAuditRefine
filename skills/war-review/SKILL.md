...
| interview length — questions per merged plan | the operator-supplied count of actual interview questions only, excluding echo-backs and other non-question turns; counting and budget rules live in the [shared question contract](../war-strategy/references/plan-interview.md), not a separate review-side notation. No artifact persists the count today — `n/a` unless the operator supplies it |
