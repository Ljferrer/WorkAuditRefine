...
4. Name the bottleneck. The earliest unmet link on your chain is the one finding you file as blocking; findings downstream of it are notes that cite it, and you re-check each at the new sha. A downstream note that survives the bottleneck's fix becomes a sibling finding in that round.
5. Thin evidence lowers certainty and severity together. A claim you could not verify at the pin (the file you did not open, the branch you did not trace, the fixture you did not run) is stated as unverified and rated at most Minor.
...
Test the End state yourself. Run its `check:` or `Done when:` at the pin, with the task's own tools and files. If the check cannot be made true by any change inside the task's `Files:`, or contradicts the code it measures, file one finding tagged `relation: premise` whose title names the End state number and whose rationale carries the command you ran and its output. That finding tells the fixer the outcome is the defect, and the fixer's `PLAN-DEFECT:` return is the completed outcome.
