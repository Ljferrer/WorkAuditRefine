...
| 7 | Audit verdict | Severity-tagged; Critical/Major block, Minor/Nit route by auditor-owned **disposition** (absorb/follow-up/note/ask — ADR 0013, amended 2026-08-25), `escalate`→halt; **unanimous on one SHA** |
...
- **New role — `war-servitor`** (sonnet): after a phase *lands*, a single write-mode pass captures **durable learnings** into the **local memory root** (the absolute `memoryLocalRoot` threaded at Setup — `~/.claude/projects/<proj>/memory/`, one file per fact; `type: project` marks a file promotable, but *every* file is written locally). Repo-root publication is the Lead's Gate-2 promotion, never a servitor write. It is fed the phase's audit log + escalations. Auditors stay **read-only**; the servitor is the only reviewer-side writer, and its writes are confined to the local memory root by its capability allowlist (no Bash — only Read/Grep/Glob/Write/Edit), with the PreToolUse scope hook gating the residual Write/Edit paths by `agent_type` to the local memory path-pattern and denying mutation of any pre-existing untagged memory file ([ADR 0002](../../../docs/adr/0002-scope-by-agent-type.md), [ADR 0022](../../../docs/adr/0022-servitor-local-root-writes-gate-2-promotion.md)). Cadence: **once per phase** (not per task). Captures signal only — gotchas, plan↔code mismatches, deviations+why, patterns — never routine "implemented X" notes.
...
- **Outcome:** grills the user on every blocker, patches the plan **in place** until it reaches a gate-emitted proceed verdict — **CLEARED**, or **ADJUDICATED** when a patched blocker was not re-verified by a probe re-run ([ADR 0043](../../../docs/adr/0043-adjudicated-clear-distinct-terminal-verdict.md)) — and leaves a report under `docs/red-team/`.
- **Surface.** `skills/red-team/` — `assets/red-team-gate.mjs` (verdict + severity-classify logic, tested), `assets/workflow-scaffold.js` (copy-per-plan Workflow: spine + probes + adversarial-confirm), `references/lenses.md`, `SKILL.md` runbook — those maintained surfaces are the design's home (spec-posterity rule, ADR 0046). v0.4.0 is purely additive (new front-end skill + version bump).
...
- **Disposition routing (ADR 0013) — classify-at-collection.** Every Minor/Nit finding routes **once**, by auditor-owned `disposition`, orthogonal to severity: `absorb` (mechanical, intent-consistent — executed in-phase), `follow-up` (files a `war-followup` issue; the auditor must state why it is not absorbable), `note` (phase report + servitor feed, never an issue), `ask` (#1550, ADR 0013 amendment 2026-08-25 — a decision-shaped Minor/Nit only the operator can rule; carries the mandatory question+fork field, parks on the run's `asks[]`, and is ruled at the Checkpoint strike-list gate: **never filed unruled**, ruled asks filing Lead-side with filing parity). Omitted → Minor becomes follow-up, Nit becomes note; `absorb` and `ask` are never defaulted; legacy `autoFixable:true` reads as `absorb` for one release (deprecated). `minorsFiled` receives only `follow-up`; a `notes` array receives `note`; `asks` receives `ask` — riding the top-level return beside `minorsFiled` and the handoff's ninth (lossy) `asks` key, excluded from the follow-up consolidation and the `file-followups` dispatch. This **replaced** the old eager `minorsFiled` push + aced-splice mechanics. The **terminal-disposition demotion ladder** guarantees zero unrouted findings on every exit path — now **per subset**: demote one step toward durability, never drop silently, every demotion logged — failed absorb → follow-up; findings on a task that never reaches the approve branch → follow-up, filed with the escalation; a held phase's queue → drained to follow-up; a fileless absorb → its severity default. `demote()` **refuses an ask** unconditionally and loudly — log() + re-route onto `asks[]` (exactly-once by finding identity), never `minorsFiled`/`notes`, never a throw; the sole lawful ask→follow-up conversion is the Checkpoint `--afk` no-match arm, Lead-side, question preserved. An ace re-audit **regression** no longer demotes the whole batch: the bounded **ace bisection** ladder (canonical: `aceBisect` in `skills/war/assets/workflow-template.js` — culprit-first excision, serial subset re-application at the tip, depth ≤ 2, same-file findings never split, failed tips forward-reverted in-loop) salvages the good majority, and only finally-failing subsets (or a budget-exhausted remainder) demote. The ace's orchestrator backstop **narrowed** to the two pure version-slot JSONs (`plugin.json`/`marketplace.json`); README and other shared files route to the sweep instead of being refused.
- **Phase-close coherence sweep (ADR 0012) — fail-open, at the integrated tip.** Runs **after** the land decision is computed and **before** the land dispatch, iff `landDecision === 'landed'` and the `phaseCloseQueue` is non-empty. The refiner provisions a `p<N>-polish` worktree (branch `war/<slug>/p<N>-polish`) at the post-merge integrated tip via the existing `ensure-worktree`; **one** war-worker drains the queue verbatim (one commit, gate green, no version-slot literals, no ad-hoc seam hunting); the **full config-default roster** re-audits the polish SHA under the same unanimity rules (the Lead may not downgrade it). Re-approved → merged at the serial queue's tail, the land proceeds on the polished tip, the queue is recorded on `aced` at the polish SHA. Anything else → **discard**: branch + worktree left in place (reaping is a human act), queue demoted to follow-up, the pre-polish tip lands exactly as it would have — a discarded sweep recomputes nothing. Bookkeeping is a Lead-side task-grade ledger entry + the handoff block; **no owned-refs registration** (the spec's §4.5.5 described machinery that does not exist).
- **Intent threading + handoff (ADR 0013).** `args.intent` (the plan's `## Commander's Intent` or `## AI-Commander's Intent` — either heading, Lead-extracted verbatim, never Lead-invented except by `/war-machine --afk` (ADR 0014), string|null) threads into worker, auditor, ace/sweep, gate-audit, and servitor prompts — plan slice = floor, intent = ceiling; `null` ⇒ byte-identical intent-less prompts (literal behavior). The gate-audit pass additionally verifies the phase's claimed End-state conditions at the confirmed tip **artifact-first** (the 2026-08-05 precision-chain plan): a `check:`-tagged condition is executed once per phase by the refiner's **land-barrier endstate-check dispatch** — after the serial merge queue's last merge, before the gate-audit seats spawn — teeing one per-condition artifact (`_refinery/.war/endstate-<phaseId>-<n>.log`, stamped with the tip SHA it ran at); seats read the artifacts (a `gate:`-tagged condition attests from the captured gate logs) and return one `endStateAttestations` row per claimed condition (`status: met | unmet | unverified` plus cited evidence, never a bare verdict), while findings stay defect-only (a condition provably unmet at the confirmed tip is a HARD Critical/Major finding; a condition owned by a later phase — or by a deps-chained sibling task of the same phase not yet landed at the audit's scope — is out-of-scope, never a hold; when no task is gate-auditable but conditions are claimed, ONE End-state-only seat spawns and consumes the same artifacts). The handoff maps a condition **no seat attests** to `unverified`, never `met` (whole-pass absence stays all-`deferred`). The return gains `notes` and — on `landed`/`held:escalation` only — the machine-readable `handoff` block (see `schemas.md`).
