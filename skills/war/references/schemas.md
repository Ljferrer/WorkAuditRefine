...
- **`tasks` vs `land`** — `tasks` maps each task id to its **task** terminal status (the `ledger.json` task-status enum above: `merged`|`escalated`|`blocked`, or `env-blocked`/`env-died`); `land` is the phase `landDecision` (`landed`|`held:*`, the Workflow per-phase return enum below). Both are copied from the Workflow per-phase return / handoff, not re-derived.
