...
{ phase,                              // phase id
  ...
  minorsFiled: [ { task, ...finding } ],   // disposition:'follow-up' findings (incl. every logged demotion) filed as war-followup — the `file-followups` dispatch stamps each filed entry's `.issue`, which the handoff's followUps mapping reads — deterministically COLLAPSED in place pre-filing ONLY on a handoff-emitting land decision (`landed` / `held:escalation`) with rows present (#1597); on any other hold the rows stay raw. Collapsed rows may carry a `seats[]` corroboration key and can be fewer than the raw seat findings; mechanics canonical in the template's FOLLOW-UP CONSOLIDATION block (see § Filing result)
