...
{ task_id, branch, worktree, head_sha,
  ...
  acceptance_criteria_covered: ["<end-state id>"],   // claimed End-state ids (A1/D9, the 2026-08-05 precision-chain plan): the numbered `## Commander's Intent` End-state conditions this task claims to satisfy; [] when the task claims none; the gate-audit seat cross-checks the claims (cross-check lands with Task 3.2)
...
{ phase,                              // phase id
  ...
  handoff?: {                         // ADR 0013 — the machine-readable debt map the next phase's decompose reads
    ...
    notes: [ { task, title } ],
    endState: [ { condition, status: "met" | "unmet" | "unverified" | "deferred" | "out-of-scope" } ],   // keyed on the verbatim condition text (the endStateAttestations / gate-audit plan_ref key); a condition with NO attestation row from any seat ⇒ 'unverified', never 'met' (D8); no gate-audit ran at all ⇒ all 'deferred', never a silent 'met'
    intentPresent,
