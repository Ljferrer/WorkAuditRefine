...
- **`endStateAttestations` is the positive End-state channel** (D8, the 2026-08-05 precision-chain plan; gate-audit-family seats only — per-task (post-merge), integrated-tip, end-state-only; produced by its Task 3.2, landed 0.17.0). Each gate-audit-family seat returns **one row per claimed condition**: `condition` is the claimed condition text **verbatim** (the `plan_ref` key), `status` ∈ `met | unmet | unverified`, `evidence` cites what the seat actually read — the teed per-condition artifact for a `check:`-tagged condition, the named observable for a judged one — never a bare verdict. Ordinary audit-seat results are unchanged and never carry the field. Findings stay **defect-only** (findings carry defects; attestation rides `endStateAttestations` — two separate contracts): attestation rides this channel, never a finding. The handoff maps a condition with **no attestation row from any seat** to `unverified`, never `met`.
