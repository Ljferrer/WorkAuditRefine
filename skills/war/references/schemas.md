...
{ task_id, branch, worktree, head_sha,
  ...
  acceptance_criteria_covered: ["<end-state id>"],   // claimed End-state ids (A1/D9, the 2026-08-05 precision-chain plan): the numbered `## Commander's Intent` / `## AI-Commander's Intent` End-state conditions this task claims to satisfy; [] when the task claims none; the gate-audit seat cross-checks the claims (cross-check lands with Task 3.2)
...
- **`endStateAttestations` is the positive End-state channel** (D8, the 2026-08-05 precision-chain plan; gate-audit-family seats only — per-task, integrated-tip, end-state-only; defined-but-not-yet-emitted — produced by Task 3.2). Each gate-audit-family seat returns **one row per claimed condition**: `condition` is the claimed condition text **verbatim** (the `plan_ref` key), `status` ∈ `met | unmet | unverified`, `evidence` cites what the seat actually read — the teed per-condition artifact for a `check:`-tagged condition, the named observable for a judged one — never a bare verdict. Ordinary audit-seat results are unchanged and never carry the field. Findings stay **defect-only** (the two-contract rule intact): attestation rides this channel, never a finding. The handoff maps a condition with **no attestation row from any seat** to `unverified`, never `met`.
...
{ mode: "merge-task" | "land-phase",
  ...
  mappedTests?: ["path"],                            // (merge-task) ALL test paths assert-test-in-diff.sh matched in the task diff, captured from its exit-0 stdout (one per line — the accumulating scan); the gate-audit seat greps them against the captured gate log (D7 — "provably unrun mapped test" becomes mechanical). Defined-but-not-yet-emitted — produced by the precision-chain plan's Task 2.2/2.3
