...
- **`held:land-failed`** — the in-flow land step itself failed (non-stale failure; distinct from `land_stale`) **or the land dispatch died / returned an unrouted result** — a dispatch that died producing no `MergeResult` routes here with `reason:'error'`, `detail:null`; the Lead re-runs the land manually per SKILL.md §4.3's root-cause branches.
