...
// run /war-room to regenerate the config (D3: no shims, no accepted-but-ignored keys).
  run: { roundLimit, redteamRoundLimit, maxParallel, afk, ace, provision, provisionSource, provisionAuto },
                                             ...
                                             // maxParallel — per-run fan-out cap consumed by the engine's batching helper (and read fail-open by /red-team for its own fan-out); shape and default per war-config.mjs validate() — never restated here (absent = unthrottled fan-out, no preset sets it);
