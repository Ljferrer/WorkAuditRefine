...
3. Set **`requiresTest`** per task (default `true`). Set `false` only for tasks with no executable surface: pure docs/config edits, VERIFY no-ops, or prose-only contract tasks. When `requiresTest:false`, the refiner's test-floor check is skipped for that task.
4. Present the DAG to the user as an issues preview **and any per-phase Workflow patches** you intend to inject. **Wait for approval/edits.**
5. On approval, file **all phase epics up front** (labels `phase:N`, `status:todo`) so the full scope exists before any teammate launches. Break each phase into **task sub-issues just-in-time** at that phase's start (so later phases absorb learnings/drift). Record everything in the ledger.
...
- **Fail-closed phase classification (§4.2 — apply on every phase notification, before any land/hold/advance decision).** On each phase Workflow notification, derive `landDecision` as follows:
  ...
  2. Else if the result is missing, unparseable, or `landDecision` is not in the known set (`landed`, `held:escalation`, `held:nothing-merged`, `held:land-failed`, `held:phase-incomplete`, `held:workflow-error`) → classify as `held:workflow-error`.
