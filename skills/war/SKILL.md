...
- **Outcome handling (§4.3).**
  - **`held:workflow-error` — terminal.** HARD-halt **regardless of `--afk`**. Surface the `workflowError` to the user; do **not** auto-resume and do **not** advance to the next phase. A `held:workflow-error` classified at this layer (infra death / garbage return) similarly HARD-halts without retry.
