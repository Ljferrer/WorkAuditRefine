...
Run **one Workflow per phase** from [assets/workflow-template.js](assets/workflow-template.js), passing the approved phase DAG + config (incl. `learningsTarget`, and **`agents` / `audit` / `run`** from the resolved run config) as the Workflow `args`. **Do not free-author orchestration** — use the template; only inject stages the user approved at the gate. The Workflow:
