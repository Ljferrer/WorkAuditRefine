...
test('stale-looking-but-correct calibration (Task 1.4): the "only when the live artifact confirms" qualifier survives per rule on BOTH surfaces', async () => {
  ...
  // intro-line qualifier sits before anchor 1 on the standing card (the dispatched skeleton no longer
  // carries one — prompt-surface simplification), so where present it is outside every window and never
  // pads a rule.
...
test('stale-looking-but-correct calibration (Task 1.4): the per-rule qualifier lock discriminates each rule on BOTH surfaces (delete-the-feature)', async () => {
  ...
  // from trailing text, and that the intro-line qualifier (standing card only since the prompt-surface
  // simplification; pre-anchor-1, outside every window) can never rescue a rule. Initial-round capture only — a deliberate scope cut matching the sibling anchor test;
