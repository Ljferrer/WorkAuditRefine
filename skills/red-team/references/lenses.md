...
Two universal doctrine probes the Lead runs on every plan (vacuous when the plan has no matching feature), enforcing ADR 0025's drift-guard discipline. **Not** members of the `SPINE` array — Lead-run like the backstop-legitimacy check (see [backstop-legitimacy.md](backstop-legitimacy.md)):
