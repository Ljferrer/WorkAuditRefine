...
const scopeLock = (technique) => [
  ...
  technique === 'executed'
    ? [
        `To inspect or run anything, first copy the repo into a throwaway sandbox. Pick the form by what your probe actually DOES — the wrong one is not isolating:`,
        `  • Your probe runs \`git\` AT ALL -> \`git clone --no-hardlinks ${repo} <tmp>\`. This is the only form that stays isolated even when ${repo} is a linked worktree. Caveat: a clone carries COMMITTED STATE ONLY — uncommitted edits and untracked files do not come across — so a probe that needs the working tree as it stands must overlay it explicitly (copy the dirty paths in, or apply \`git -C ${repo} diff\`), never assume the clone reflects it.`,
        `  • Your probe ONLY READS AND EDITS FILES -> \`cp -R ${repo} <tmp>\` is fine. But when \`${repo}/.git\` is a FILE rather than a directory, ${repo} IS a linked worktree and this copy is NOT git-isolated: it duplicates a pointer to the real repo, so any \`git -C <tmp>\` would drive ${repo} itself. Never run \`git\` against a \`cp -R\` sandbox.`,
...
const SPINE = [
  ...
  { name: 'coverage-vs-source', kind: 'spine', technique: 'analyzed',
    prompt: `Read the source of truth (${sourceSpec}) and the plan ${planFile}. MERGED ARM — when the source IS the plan itself (a merged plan whose Part 1 carries the decision record; the plan is its own source of truth, so the Lead defaulted --spec to it): confirm every Part-1 requirement/decision maps to a Part-2 phase/task — Part-1→Part-2 coverage. PER-ISSUE EVIDENCE JOIN (merged arm only): if the plan carries an Evidence-consumed block, read each cited source issue's "## Evidence artifacts" section and confirm every artifact it lists appears in the block as read or unread-with-reason — a listed artifact the block omits is a Major gap. An issue whose "## Evidence artifacts" section is ABSENT makes the join vacuous for that issue (a pass, not a finding). An issue you cannot reach (gh/network/auth failure) is a named unverified note in your findings identifying that issue — never silent, never vacuous. Otherwise confirm every requirement/section in the source maps to a plan task or step. Report unmapped requirements as Major gaps. Read-only.` },
