...
const SPINE = [
  ...
  { name: 'intent-vs-plan', kind: 'spine', technique: 'analyzed',
    prompt: `Read the plan ${planFile}. If it has a "## Commander's Intent" section (Purpose / Method / End state), check: (1) each End-state condition is individually checkable — a concrete predicate you could verify true/false against ${repo} (a vague/unverifiable condition is a Major); (2) each End-state condition maps to at least one phase/task that claims to deliver it (an unclaimed condition is a Major); (3) the End-state conditions are collectively sufficient for the stated Purpose (a sufficiency gap is a Major with needsDecision:true — only the user can settle what the Purpose really requires). If the plan has NO "## Commander's Intent" section, that is a Minor, never a Major: return status:"pass" with a single Minor note recommending the intent interview. Read-only.` },
]
