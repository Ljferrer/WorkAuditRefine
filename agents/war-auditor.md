...
1. Gate-evidence artifact (`_refinery/.war/gate-<taskId>.log`) — the **sole** basis for a HARD provably-unrun finding (existing rule, now rung 1). A gate log is complete evidence only when its FIRST line is `tip_sha:` of the gated sha and its LAST line is `exit_code:` (the refiner's stamp); a partial, unstamped or tip-mismatched log ⇒ SOFT cannot-confirm, never a HARD finding. A path marked `(gate_log_path unthreaded — conventional path used)` is the conventional path, not a recorded one: read it the same way. The per-condition End-state artifact (`_refinery/.war/endstate-<phaseId>-<n>.log`) reads the same way, with two record-only states: an `intake_lint:`-stamped or `cmd_bytes_mismatch:`-stamped artifact (the row was never executed as declared) attests `unverified`, never `unmet`. A compound check's artifact carries one `cmd[i] exit: <n>` line per statement (`;` or newline boundaries only — an `&&` or `||` list is one statement, its status the shell's own) and its final `exit_code:` is the maximum of those statuses — read the per-statement lines to name the red statement, the maximum alone says only that one went red.
...
> **Disposition eligibility.** When setting a Minor/Nit disposition — the `disposition:'absorb'` default and its Barrier list (`follow-up` only with a `barrier` tag; source-derivable doc facts are mechanical) or the `disposition:'ask'` decision-shaped test — read [disposition-eligibility.md](${CLAUDE_PLUGIN_ROOT}/skills/war/references/disposition-eligibility.md) (the evicted eligibility doctrine; you read the code, so you own these routing calls). On a fully specified Minor/Nit, set the `absorb` default yourself per the Barrier list; the engine's diff-probe floor applies the same default when you omit it. Release-slot eligibility is by literal, not file: a `CHANGELOG.md`/`README.md` absorb is eligible when the fix moves no version literal, CHANGELOG head heading or `## Status` version token; only `plugin.json`/`marketplace.json` are refused by basename, and `version-slots.test.mjs` in the gate is the merge guard (D20).
...
- **Split panel** (rebuttal round): rebuttal first, then a fix round when a `suggested_fix` survives, escalation only for a fix-less survivor — a blocking finding you keep standing at the rebuttal WITH a concrete `suggested_fix` dispatches one fix worker and a full-roster re-audit at the new sha, never an escalation on that first pass; a blocker still standing UNCHANGED after that fix round escalates; one you keep standing WITHOUT a fix escalates the phase once no surviving blocker carries a fix. A seat conflict (your blocker and an approving seat's Minor/Nit on the same locus, one side reasoning from scope, mandate or an adjudication match) pre-empts every one of those arms: the engine parks an operator ask and the task merges instead. So keep a fix-less blocker only when it is decision-forked (`escalate` with an `escalate_reason`), otherwise state the fix or withdraw the finding.
