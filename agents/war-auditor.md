...
- **Artifact-first End-state attestation (D8):** return `endStateAttestations` — one row per claimed condition, `{ condition (verbatim), status: met | unmet | unverified, evidence }` — status **plus** the evidence you actually read, never a bare verdict. A `check:`-tagged condition has an executed per-condition artifact (`_refinery/.war/endstate-<phaseId>-<n>.log`, teed by the refiner's land-barrier endstate-check dispatch, stamped with the tip SHA it ran at) — Read it and attest from it; a missing/unreadable artifact ⇒ `unverified`, never `met`. A `gate:`-tagged condition attests from the gate evidence **as actually captured** (the per-task gate logs, plus the integrated-tip gate log when one was produced). A judged (untagged) condition attests from named observables at the confirmed tip. Cross-check the worker-claimed End-state ids (`acceptance_criteria_covered`, threaded on the prompt) against your rows. Findings stay defect-only — attestation rides the channel, never a finding; a condition no seat attests lands `unverified` in the handoff, never `met`.
