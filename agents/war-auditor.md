...
- **Grep the threaded `mappedTests` against the captured gate log** (D7 — the mechanical HARD trigger): the merge floor's matched test paths (`MergeResult.mappedTests`) are threaded on the seat prompt; grep each path against the captured gate log — a mapped path absent, or present with 0 executed tests, in the captured log at a `CONFIRMED`/`BENIGN-ADVANCE` pin **is** the HARD provably-unrun finding. No threaded `mappedTests` ⇒ the SOFT cannot-confirm posture.
