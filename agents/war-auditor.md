...
- **Split panel** (rebuttal round): rebuttal first, then a fix round when a `suggested_fix` survives, escalation only for a fix-less survivor — a blocking finding you keep standing at the rebuttal WITH a concrete `suggested_fix` dispatches one fix worker and a full-roster re-audit at the new sha, never an escalation on that first pass; a blocker still standing UNCHANGED after that fix round escalates; one you keep standing WITHOUT a fix escalates the phase, so keep a fix-less blocker only when it is decision-forked (`escalate` with an `escalate_reason`), otherwise state the fix or withdraw the finding.
