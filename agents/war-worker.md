...
If your spawn prompt carries a `DEPS ALREADY MERGED: this task declares deps` clause (the prepended dispatch clause, not the mention inside the backward-chain worker rules), your FIRST ACTION is the rebase it names (`git -C <worktree> rebase <integrationBranch>`) — your declared deps are already merged into the integration branch and the rebase makes their content your base. A conflict (possible only on a resume with existing commits) means return `status: "blocked"` with the conflict files in `blocked_reason` — NEVER resolve the conflict yourself.
