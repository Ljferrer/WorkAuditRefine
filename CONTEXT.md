...
spawns — either direction, a stronger fixer than the first pass or a cheaper one; the economy
preset pairs an opus first pass with the fable/`default` fix tier. Absent = fix work inherits the base worker config
(today's behavior).
