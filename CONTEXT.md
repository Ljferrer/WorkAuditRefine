...
spawns — either direction: a stronger fixer than the first pass, or a faster one (the economy
preset: opus first pass, fable/`default` fixer). Absent = fix work inherits the base worker config
(today's behavior).
