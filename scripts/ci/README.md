...
There is no selector that can silently omit a discovered suite. The programmatic
`inventory` argument is an exact expected census, not a filter.
...
failure. Node TAP summary counts and skip lines are recorded; legacy shell suites
retain their native case counts in raw logs (`counts: null`, not an invented zero).
...
The report records source SHA and worktree status so dirty runs cannot be mistaken
for exact-release certification. T4–T8 own workflow integration, production-adapter
binding, full parity and release enforcement.
