...
- **D22 ordered key repaired and extended (#1287, #1288)** — `D22_ORDERED_SPAN` stays one ordered, markup-tolerant regex and gains a fetch arm (the `git fetch origin` adjacent form, placed between the docs-commit arm and the range-probe arm — a bare `fetch` key would be greened by the freshness sentence's own explanatory prose) and an exemption arm (the mid-sentence `This reverts commit` adjacent form), and its terminal arm is re-anchored on the push invocation shape — `provision-worktrees.sh` adjacent to `ensure-origin` — never the bare token, whose ordered match had ended inside an in-region prose mention with the actual push invocation outside the match. Five new negative references — (d) command form, (e) revert routing, (f) push invocation, (g) fetch, (h) exemption — join the three pre-existing ones, each updated to carry the fetch step and the exemption sentence and re-proven red at its designated arm; the block comment's RESIDUAL paragraph is retired — every load-bearing fragment of the key now has a both-ways proof, one live key shared by all eight references. Home: [`skill-doc-contracts.test.mjs`](skills/war/assets/skill-doc-contracts.test.mjs).
