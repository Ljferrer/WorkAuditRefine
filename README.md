...
Release scope: the window from the 0.21.10 head (`517bdad`) to this release, bucketed whole. Config authority: `war-config.mjs`. Engine: `workflow-template.js` (two sandbox literals, no routing change). Agent cards: the two frontmatter `model:` lines. Docs: `README.md`, `CHANGELOG.md`, `CONTEXT.md`, `skills/war/SKILL.md`, `skills/war-room/SKILL.md`, `schemas.md`, `design.md`, `gastown-design-params.md`. Tests: `war-config.test.mjs` and `workflow-template.test.mjs`. No guard, hook, floor script, shell suite, or `docs/learnings/` file changed. The version slots and this blurb's CHANGELOG twin are part of the same window.
