...
**0.14.41** — Lessons-learned repo-projection integrity (#891): a `/lessons-learned` housekeeping pass can no longer swap a `MEMORY.md` projection that has silently dropped every `[repo]` lesson row on a repo-adopted store. Two coupled defects allowed it — the Phase 5 archive step invoked `war-memory archive` without `--repo`, so `cmdArchive`'s trailing re-render rebuilt the staged projection from the local root alone (observed 2026-07-13: ~24 KB / 144 lines collapsing to ~4 KB / 30 lines), and the shared `do_verify` gate in `skills/lessons-learned/assets/safe-swap.sh` could not catch the loss because its row→file hard fail deliberately excludes `[repo]` rows (the load-bearing Rule 2) and was never told about a repo root. Both are closed: the Phase 5 archive command now threads `--repo "$REPO_ROOT"`, and `do_verify` gains a repo-completeness hard fail driven by the `CLAUDE_MEMORY_REPO` env convention `resolveRoots` already honors — it fires only when the env names a directory holding ≥ 1 top-level hot lesson (`MEMORY.md` excluded, non-recursive) AND the staged `MEMORY.md` carries zero row-scoped `[repo]` rows, so a local-only store, an unset env, and the post-evict emptied root all skip (a non-directory path is a `WARN`, never a hard fail). `verify` and `commit` both route through the single `do_verify`, so one landing site gates both; Rule 2's `grep -v '[repo]'` exclusion survives byte-intact as a separate, additive predicate over the same marker. `skills/lessons-learned/SKILL.md` threads the Phase 6/7 `CLAUDE_MEMORY_REPO="$REPO_ROOT"` prefixes so the check fires in a real pass, with new `lessons-learned-doc-contract.test.mjs` locks pinning the presence.
