...
- `vale-md.py` (`PostToolUse` on `Edit`/`Write`, harness-filtered to `.md` paths by per-handler `if` rules so non-Markdown edits never spawn a process) runs [Vale](https://vale.sh) over the edited file when its path ends in `.md`, using a self-contained profile beside the script — `.vale-google.ini` by default, `.vale.ini` when `hooks.valeGoogleFork` is `false`; no synced packages, no network either way. When Vale reports findings, the hook returns one `additionalContext` line naming the count and the top three rules. It never blocks, and the line itself cautions the agent never to reword ratified, byte-pinned, or drift-guarded literals to satisfy a style rule.
...
**Google profile (default):** the default `.vale-google.ini` layers the Google developer-documentation style on the house rules, vendored under `styles/Google/` ([errata-ai/Google](https://github.com/errata-ai/Google) v0.7.1, MIT) and tuned: sixteen Google rules are disabled — thirteen that judged entrenched repo voice in the prototype sweep (parens, em dash, semicolons, acronyms among them), plus `Google.Spelling` (the card mandates British), `Google.Latin` (yields to the house `LatinAbbrev` twin), and `Google.WordList` (a replace-action substitution list that fires on repo-ratified vocabulary like "CLI"). Vendoring rather than `vale sync` keeps lint time offline and makes rule updates deliberate commits. `{"hooks": {"valeGoogleFork": false}}` swaps back to the house-only `.vale.ini`; subordinate to `valeMarkdown` — when that is `false`, nothing runs.
...
**0.21.5** — The memory archive mover now fails closed on an occupied `archive/` destination (#1924). `fs.renameSync` replaces its destination, and the repo arm's `git mv` (no `-f`) refuses and falls through to that same rename, so archiving a slug already present in `archive/` deleted the archived copy while still printing the ordinary `archived <slug> → <dst>` success line. That turned the pipeline's stated safe operation ("knowledge is archived, never deleted") into a silent delete. The reachable shape is **same-root**: a slug re-captured hot in the root it was archived from, then archived again. A cross-root hot/cold split never collides, because the destination is scoped to the record's own root. Both archiving verbs, `archive` and `migrate --apply`, now route through one mover (`archiveRecord`) that tests the destination before appending the archive note, so a refused record keeps its hot file byte-intact: the batch skips that slug, names both paths on stderr, archives the rest, writes a count, re-renders, then exits 1. The count is written **before** the re-render, which takes its own exit above a hard budget axis. `migrate --apply` gains the same refusal and the archive note its sibling always wrote. ADR 0028's rejected-option rationale is amended, and the two documented callers (`/lessons-learned` Phase 5 and `tighten` Execute step 4) carry the new route. Nine tests cover both verbs, the repo `git mv` arm, the budget-refuse interaction, the `--candidates --apply` arm, the untouched-hot-file guarantee, the cross-root non-collision, and a repeated slug in one batch. No other engine, guard, or agent surface changed in this window.
