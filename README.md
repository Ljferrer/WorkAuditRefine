...
- **ADR truth notes (#1363, #1305, #1266, #1398, #1290)** — append-only dated notes and in-place pointer repairs, every pre-existing ratified ADR byte unchanged: ADR 0030's amendment restates the war-strategy §2 convention-block member count as owned by the live block and names its newest member, **Dated snapshots (D12 staleness rule)**; ADR 0033's correction records the `--baseline` ref-diff as built and the porcelain half exact only for tracked and untracked-but-not-ignored paths, widens the exit-2 enumeration, and adds check (c); ADR 0025's correction re-measures the `CONTEXT.md` drift-guard glossary at seven bold terms, owned by the live subsection; ADR 0008's line-anchored citation and mis-named construct pointer are re-anchored by construct.
- **Comment, tour, and memory truth (#1291, #1292, #1253, #1399)** — `stage-workflow.mjs`'s `ARGS_FALLBACK_ANCHOR` comment now states the true direction per ADR 0037's correction (`workflow-template.js`'s args-fallback tail is canonical; the constant is the hand-maintained mirror site); the architecture tour's rotted advisory anchors are re-derived against the live files; the auditor-guard suite's inherited VACUOUS claim is replaced by the fail-loud trace at C11, H5, and the #1412 L-block header; the archived printf-escaping memory's vacuity claim is bounded to deny-asserting assertions; the awk-degeneracy lesson carries its dated mechanism correction; and #1363's mined lesson opens with its RESOLVED stamp.
