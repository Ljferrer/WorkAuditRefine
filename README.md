...
**0.14.22** — `/red-team` now proves a plan *can be applied*, not that it already was: a Lead pre-flight classifies each artifact into an artifact kind (implementation plan, TDD plan, design doc, or PRD, defaulting to the suppression-safe implementation plan) and threads it into every probe, so a shipped test running red before its implementation grades as a pass rather than a defect while a false claim about existing code still blocks. A typed deliverable-absence finding flag lets the gate drop a wave of not-yet-built-symbol findings without any prose parsing, killing the false-BLOCKED misfire at the gate layer. A post-run repo-escape guard runs between the verification Workflow and the gate, routing any stray working-tree file or junk sandbox ref through the self-confound gate so the verdict can never reach CLEARED on a contaminated repo, backstopping a hardened `git -C <sandbox>` scope-lock on every executed probe. A drift-guard pins the pass-only demotion set and the two-contract sentence on both surfaces, and the red-team report gains a machine-readable Adjudications block the WAR Lead threads into the auditor prompt so version precedence runs task instruction first, then red-team adjudication, then the plan body literal. ADRs 0032 and 0033 ratify.
