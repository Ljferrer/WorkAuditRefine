...
**0.14.44** — Structural-test integrity: narrow the corrupt block-comment strips, bound them by census, and widen the `pt`-tag floor to a default-deny literal census (structural-test-integrity). Two text-preparation idioms in `skills/war/assets/workflow-template.test.mjs` were blind to string literals that merely *look* like syntax. **#929:** the two-step comment strip mis-read the `/* … */` sequences inside `resolveGate`'s glob literals as comment delimiters and deleted three fake spans — the largest swallowing the refine-loop/gate-audit core — so three structural tests passed only by token position, and a new `held:*` literal assigned inside the deleted span would have bypassed the hand-mirrored `KNOWN_LAND_DECISIONS` drift guard with no red test. The three sites now strip line comments only, made durable by a `blockCommentSpans` block-comment census (the ordered exact `{head, tail}` span list — red on any added, removed, or merged span) carried in both `workflow-template.test.mjs` and `skills/red-team/assets/workflow-scaffold.test.mjs`. **#931:** the `pt` undefined-interpolation guard rejected only a bare untagged backtick immediately after `agent(` — untagged literals behind variables, in helper bodies, or nested inside `pt` interpolations slipped through. A string-aware `scanTemplateLiterals` scanner now backs a default-deny template-literal census: the multiset of untagged literal heads must equal an enumerated class-grouped registry exactly (a new untagged prompt-feeding literal is a red test) and every true block comment must be backtick-free. Completing the discipline, every prompt-feeding literal in `workflow-template.js` is now `pt`-tagged — a **runtime change**: an undefined interpolation now throws at prompt-build (per-task thunk catch → bounded `verdict:'escalate'`, or the top-level catch → `held:workflow-error`) instead of silently rendering literal `undefined`, with a per-interpolation required-vs-optional audit keeping the fail-open polish stretch absence-tolerant.
