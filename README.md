...
**0.14.66** — Four changes to the auditor's read-only git guard (`hooks/validate-auditor-git.sh`) and the two prompt surfaces that teach it. Two measured runs — #1025 (244 denials across 14 seats) and #1138 (52 denials across 29 of 31 seats, *after* the contract had already reached both prompt surfaces) — showed that front-loading the contract into the prompts was necessary but not sufficient: the residue was a small set of denial families the guard refused for reasons its message never explained. **The fail-closed posture does not move.** Exit codes, the `WAR:` stderr marker, the bash-3.2 `LC_ALL=C tr -d` residue method, the set of deny sites and every pre-existing shell-test case arm are all unchanged; no verb joins the read allowlist; exactly one deny message gained a suffix. **(1) The character allowlist admits one more character, `+`.** The residue set is now `A-Za-z0-9 ./_=:,@^~%+-` — the `+` inserted before the final `-`, which stays last so it never forms a range — so the range-length line forms stop tripping the forbidden-character deny *before* their already-allowlisted verb is ever consulted: `git blame -L 10,+5 <file>`, `git blame -L /re/,+10 <file>` and `git log -L 5,+3:<file>` now pass. `+` carries no shell-metacharacter semantics in the composition space that remains — quotes, globs, braces, `$`, parens, pipes and redirects still deny — and a new `expect_deny` case pins exactly that: `git blame -L 1,+5; rm -rf .` still dies on the semicolon, before the allowlisted `blame` verb is reached. All three same-file comment homes for the char set moved in the same edit. **(2) The forbidden-character deny now names the compliant form.** Its byte-preserved prefix — `command contains forbidden character(s)` plus the `head -c 20` residue echo — is followed by a teach suffix: one bare git command per Bash call, split `&&` / `;` chains and continuations into separate calls, filter and search output with the Read/Grep/Glob tools. That suffix is the only message text that changed. Four `expect_deny_teach` cases on a single `git diff HEAD && git log` payload pin the byte-preserved prefix tail together with its residue echo, and each of the three teach elements separately, so no element can be trimmed away while the others stay green — and the assertions are site-pinned by routing, since the character check is the first check after the empty-command guard. **(3) Both auditor prompt mirrors now characterize `branch` the way the hook's own deny string does.** The standing card (`agents/war-auditor.md`) and the dispatched `auditPrompt()` clause in `skills/war/assets/workflow-template.js` carried a blanket single-arm claim that its own parenthetical contradicted with five bare flags, and omitted **seven** flags the hook admits. Both now carry the hook's two arms — value-carrying flags `=`-attached (`--contains=`, `--no-contains=`, `--merged=`, `--no-merged=`, `--points-at=`, `--sort=`) and bare read flags (`--list`, `--all`, `-a`, `--remotes`, `-r`, `--show-current`, `--verbose`, `-v`, `-vv`) — with a bare name, a space-form value (`--contains <rev>`) or any write flag denying. Enforcement is untouched: the hook shed the blanket wording on 2026-07-24 and the two mirrors had straggled behind it since. **(4) A new extraction-equality test binds the mirror flag lists to the hook.** It reads `hooks/validate-auditor-git.sh`, slices the canonical `branch` deny string, extracts the tokens from its two parenthesized arms, and asserts every one of them onto both mirror surfaces with **token-boundary** matching — a `(?<![\w-])<flag>(?![\w-])` per-token regex, never a naive substring, because `-a`, `-r` and `-v` are substrings of `--all` / `--points-at=` / `--remotes` / `--verbose` / `-vv` and a dropped short flag would otherwise pass vacuously. It fails loudly rather than vacuously in three directions: an unlocatable deny string, an arm that parses to zero tokens, and a *third* labeled arm appended to the string — that last one caught by counting the deny statement's own open parens, since the two-arm regex would happily match past it. So the next hook-side flag change reds the mirrors instead of stranding them, which is the defect class #1124 records. Alongside it, the existing both-surfaces registry row gains `value-carrying` / `bare read flags` anchors on both surfaces (a re-anchor — the row-count floor is unmoved), and the retired blanket claim is asserted absent from both **non-vacuously**: each surface's pre-change sentence is kept as an unwired negative-reference fixture in its own real bytes and asserted to match the absence regex, so the absence check cannot pass through a regex that quietly stopped firing. **`git grep` stays denied — this release ratifies that a third time, it does not revisit it.** ADR 0029 gains a dated, append-only amendment recording the re-ratification while its Decision, its Considered options and every other line of its pre-existing body stand byte-unchanged (only the `**Status:**` header line gained the new amendment date), and the `expect_deny` case keeps its byte-identical payload with its comment now citing all three ratifications. Beyond the sufficiency argument the ADR already recorded (`git show <sha>:<path>`, `git log -S`/`-G`, and the auditor's unrestricted Grep tool cover the verify-and-close need), `git grep -O<cmd>` / `--open-files-in-pager` executes an arbitrary pager command that a fail-closed character allowlist cannot distinguish from a benign search — admitting the verb would require per-flag policing of its own, on the one verb whose whole appeal was that it needed none.
