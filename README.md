...
**0.21.14** — Workflow compatibility fixes: pin-transfer dispatches use a flat result schema accepted by the API, while the engine retains its independent Git evidence checks and status-specific refusal/re-audit paths. Transfer receipts retain the independently measured patch IDs, including a meaningful empty post-rebase value (#2299). Gate capture prefixes derive deterministically from the run ID, phase and dispatch counter; Workflow entry no longer reads the forbidden clock or random globals (#2300). Every launch, including explicit-path and empty phases, must carry a valid run and phase identity. The Lead mints fresh run IDs with `new-run-id.mjs`; phase IDs follow provisioning’s numeric grammar. Each capture still allocates its directory with `mktemp`.

Release scope: `workflow-template.js`, its behavioral tests, the run-ID helper, launch/recovery/staging/manifest guidance and its ADR amendment, schema reference and preflight commentary, the four version slots, this release note, and audit evidence under `docs/port/snipe/`. Regression coverage exercises API schema compatibility across the harness's dispatches and a phase through land with clock/random globals denied, including replay and fresh-run/phase capture separation.
