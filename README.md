...
The command set, in the order you'll run it: **`/war-help`** orients you → **`/war-room`** configures a run → **`/war-strategy`** structures the spec and the plan (and converts spec → plan) → **`/red-team`** hardens the plan → **`/war`** executes it. Scaling up: **`/survey-corps`** turns open issues into specs → **`/war-machine`** turns specs into plans + a roadmap → **`/war-campaign`** runs the plans back-to-back unattended → **`/aftermath`** cleans up the debris → **`/lessons-learned`** keeps the accumulated memory honest.
...
**What "today's defaults" actually are.** With no config file WAR runs the built-in `DEFAULTS`: opus workers on `max` effort, opus auditors on `xhigh`, `rosterPolicy: auto` (the Lead composes each task's roster), the pre-merge ace-fix on, and a 3-round fix budget. For **memory** the defaults are `retrieval: true` with `topK: 10` (prefetch the ten most relevant lessons into each seat's prompt) and **`commitLearnings: true`** (publish `project`-typed lessons to `docs/learnings/` — see [Tidy the memory](#tidy-the-memory-lessons-learned) for why that default leans toward sharing). The three presets move the whole profile at once: **`balanced`** *is* the defaults, **`thorough`** widens rosters and deepens effort (and pumps tokens), and **`economy`** pins the cheaper knobs it always had — sonnet across every role, a solo roster, a 2-round budget, ace off, and `commitLearnings: false` so lessons stay on your machine. `/war-room` only ever asks about the overrides you want *on top of* the chosen preset.
...
Your backlog is raw material. `/survey-corps` sweeps the repo's open GitHub issues (run-bookkeeping labels dropped, `war-followup` debt first-class), fans out a reader agent per issue, clusters the summaries into coherent groups, and synthesizes **one war-shaped design spec per group** into `docs/specs/` — then verifies every swept issue is claimed or explicitly deferred. It commits nothing; it hands off to `/war-machine` via an uncommitted survey manifest.
...
**Two roots, and why lessons commit by default.** Every lesson routes by its `metadata.type`. `user` / `feedback` / untyped lessons are *yours* — they stay in the **local root** (`~/.claude/projects/<project>/memory/`, untracked, never leaves your machine). A `project`-typed lesson — a durable fact about *this codebase* — is committed to the **repo root** (`docs/learnings/`) so it travels with the repo and compounds across your whole team, not just your laptop. That publication is `memory.commitLearnings`, and it defaults to **`true`** (only the `economy` preset flips it off) because a lesson about the code is worth more shared than siloed — the fiftieth run *anyone* does should know what the fifth one learned. Commit is guarded, not blind: a **fail-closed redaction lint** scrubs every candidate for home paths, emails, handles, and credential shapes, and anything it trips is **demoted to the local root and reported — never dropped, never silently published**. So the effective default is *"share what's about the code, keep what's about you,"* erring toward local whenever it's unsure.
...
- **`/lessons-learned evict [slug…]`** — the **undo**. It returns repo-root lessons to the local root (temperature preserved — repo `archive/` lands in local `archive/`), re-renders the projection local-only, and opens a **reviewed deletion PR**. Because `commitLearnings` defaults to `true`, it also **asks whether to set `commitLearnings: false`** — skip that flip and the next landed phase simply republishes `docs/learnings/`, making the eviction temporary.
...
**0.14.15** — Land-path integrity and status-enum discipline: a WAR phase now lands only when git proves the working ref advanced. Every land routes through the one `land-advance` primitive, which captures the pre-push origin tip (`git ls-remote origin refs/heads/<working>`, with its exit code checked) at the last moment before the push and **refuses a phantom land with exit 3** when that tip already equals the new SHA and the local follower sits at it; it reconciles-and-exits-0 when origin is already at the new SHA but the follower lags (an interrupted prior land, repaired toward git per ADR 0008), skips the guard only on an empty origin readback (a true first land), and treats a failed readback as an error rather than the first-land carve-out. The reland loop distinguishes a contender-less transient from a real divergence with `git rev-list --left-right --count <merge-sha>...origin/<working>` after a fresh fetch — a right count of 0 buys exactly one extra push-first attempt beyond the round limit before `land_stale`, stated byte-parallel in `agents/war-refiner.md` and the `workflow-template.js` land prompt. The `held:land-failed` auto-recover branches on root cause (checkout collision vs. absent-origin baseline, the latter gated on a `merge-base --is-ancestor` trust check), and escalation-completion reuses the same detach-`_refinery` + `merge --no-ff` + `land-advance` topology every land uses — no raw force-with-lease dance. Two previously prose-only invariants — the land-phase-reachable subset of `HARD_ESCALATION_REASONS`, and the phase-vs-task status-enum separation sourced fresh from `schemas.md` — are now drift-guarded tests, with no new status, `HARD_ESCALATION_REASONS`, or `KNOWN_LAND_DECISIONS` member added. New ADR 0023 ratifies that the land asserts git ground truth and a `landed` result is never self-reported.
