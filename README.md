...
**0.14.18** — GitHub surface becomes verified, not trusted. A new `gh-preflight.sh` asserts the active `gh` account equals `overrides.ghUser` before every gh write batch — switching on drift, re-verifying via `gh api user`, failing loud on an unrecoverable mismatch; the knob ships `null`, so no personal handle enters any committed file. A Lead-invoked floor `assert-issues-filed.sh` (exit contract 0/1/2, `2` never collapsing into `1`) reconciles the run ledger against live `gh` at the Checkpoint — every `epic_issue` and `tasks[].issue` must exist — and on a landed phase asserts the epic is `CLOSED` + `status:done`; its `--close-epic` mode makes label-and-close one atomic action and the done-but-open route makes the coupling non-bypassable. A committed `docs/aftermath/known-stranded.tsv` (ref + landing-PR evidence per row) routes permanently-stranded remotes to a new acknowledged-stranded report bucket — suppressed from needs-human, never auto-deleted, the tip-reachable + PR-merged deletion bar untouched. `snap-shared-docs.sh` packages the proven merge-master/`--theirs`/fast-forward recipe for churny shared docs, guarding byte-identity outside the pathspec and never force-pushing. Everything is Lead-side — zero confined-agent capability widening. New ADRs 0026 (GitHub side-effects mechanically gated) and 0027 (known-stranded allowlist, deletion bar unchanged) ratify the discipline.
