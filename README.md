...
**0.14.27** — Lessons publication is now a conscious opt-in, and model spend is controllable where it's actually differentiated. `memory.commitLearnings` defaults to `false` across DEFAULTS and all three presets — the repo-root `docs/learnings/` publication turns on only when you accept the standing `/war-room` opt-in ask (or `/lessons-learned migrate`, which now asks before writing and aborts cleanly on decline), so nothing reaches the shared store without a deliberate yes. Two new model tiers ride the same config: `agents.redteam { model, effort }`, read fail-open by `/red-team` and threaded into every probe and adversarial-confirm spawn (absent → inherit session); and worker sub-tiers `agents.worker.docs` (sonnet by default, dispatched for a task whose every `Files:` entry is `*.md`) and `agents.worker.fix` (absent → inherit worker; drives both fix-round and `--ace` spawns). Absent keys reproduce today's behavior byte-for-byte, and every new hand-mirrored tier default ships its `workflow-template.test.mjs` registry row. A permanent doc-claim guard binds the documented default to `DEFAULTS.memory.commitLearnings` by extraction-equality and asserts the retired `true`-default claim is absent across every doc surface, so the flip can't silently rot back.
