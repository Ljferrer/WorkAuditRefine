...
**0.14.17** — Drift-guard discipline: every fact WAR duplicates across surfaces, or asserts in prose about a canonical code construct, now carries a mechanical guard that binds it to its source by extraction and equality, so surface drift turns the gate red instead of waiting for a manual doc sweep. A fail-closed lock-step test locks the four release version slots together, resolving its root from the test file's own location so it holds regardless of the working directory (this very bump is its first live exercise). Two explicit registries generalize the previously fragmented guards in the workflow-template test: a mirror registry pins each canonical export to its inline sandbox mirror with a per-construct verification mode (deep-equal for constant sets, behavioral equivalence for helper mirrors), and a both-surfaces directive registry pins each standing agent directive to its dispatched prompt, including the inline gate-audit seats built outside `auditPrompt()`. A byte-identity check locks the memory pointer line across its surfaces, an exported preset/role/model/effort matrix gains a completeness test that fails on any uncovered preset or role, and the `held:*` classifier example lists bind to their routing predicates. The registries are the deliberate ceiling — grown by row through a plan-authoring rule and a red-team probe, never by an AST scanner. New ADR 0025 ratifies the discipline.
