...
**0.21.14** — Workflow compatibility fixes: pin-transfer dispatches use a flat result schema accepted by the API, while the engine retains its independent Git evidence checks and status-specific refusal/re-audit paths (#2299). Gate capture prefixes derive deterministically from the run ID, phase and dispatch counter; Workflow entry no longer reads the forbidden clock or random globals (#2300). Fresh Recovery launches use a new run ID, and each capture still allocates its directory with `mktemp`.

Release scope: `workflow-template.js`, its behavioral tests, the pin-transfer schema reference, the four version slots, and this release note. Regression coverage exercises API schema compatibility across the harness's dispatches and a phase through land with clock/random globals denied, including replay and fresh-run/phase capture separation.
