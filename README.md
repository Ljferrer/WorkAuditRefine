...
**0.14.38** — Three prose surfaces corrected to match landed behavior; a doc-only release, zero behavior change. In `skills/war/SKILL.md` the land-recipe snippets now invoke the `provision-worktrees.sh` helper under `bash` rather than `node` — the script is `#!/usr/bin/env bash`, so the stale `node` prefix SyntaxErrored on every Setup, Gate-2, and manual-land recipe a Lead copied verbatim (#741). The Setup "Daemon reachable" docker bullet no longer misattributes the platform-signature list (`EBADPLATFORM`, `no matching manifest for <platform>`, `exec format error`) to the gate-time failure classifier: that list scopes only to Setup-time per-image probe-build deferral, while the gate-time `classOf` classifier keys on re-running the failing gate at the classification base and comparing failing identifiers — the two share only the `'introduced'` fallthrough (#799). Both SKILL.md corrections are now locked against re-drift by two assert-OLD-absent doc-contract guards (D13/D14) in `skill-doc-contracts.test.mjs`. And §5.3 of the concurrent-run land-isolation design spec now shows the landed compare-and-swap contract — `git push origin HEAD:refs/heads/<working>`, classification solely on the `[rejected]` token (never the unreliable `non-fast-forward` literal), the 0/2/3 exit contract, and a local follower `update-ref` only after push success plus an `ls-remote` readback — with a supersession pointer to the `cmd_land_advance` subcommand of `provision-worktrees.sh` as the contract of record (#804).
