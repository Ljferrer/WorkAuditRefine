...
**0.14.40** — Red-team analyzed-agent fallback made sticky per run, plus two anchor-comment corrections. The #727 fallback that recovers audit coverage when a plan's preferred analyzed-agent type (`Explore`) is absent from the harness registry was per-dispatch, invisible, and pre-emptable — every analyzed probe and confirm first died on the dead preferred type before recovering (15 zero-token `Error` rows in the recorded 0.14.38 run), and the engagement survived only in the ephemeral `log()` stream. The fallback is now **sticky per run**: the first dead preferred dispatch sets `analyzedFallbackPinned` in `workflow-scaffold.js`, so every later analyzed dispatch routes straight to the substitute — one failed dispatch, not one per slot — and each recovered result carries a durable `stampResult` trace on the returned object instead of vanishing into the log. `skills/red-team/SKILL.md` Step 3 adds the **Lead pre-flight** (#890) that pre-empts the whole dance by passing `args.analyzedAgentType` when the Lead can already see `Explore` is missing from its harness agent registry — the prompt-enforced primary, with the scaffold pin as the in-run backstop. Riding along: two decoy anchor comments in `red-team-gate.test.mjs` are corrected (recurrence 2 of the decoy-comment lesson, #895/#910) — 1c now names *deleting* the envGap demotion branch (not moving it) as the edit that reds its case, because a severity-less finding falls through the `KNOWN_SEVERITIES` membership check; and 1e records that its both-flags case is outcome-unobservable by branch order, since both branches return the identical Minor demotion.
