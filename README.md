...
**0.21.3** — `/snipe` (#1920): a new operator-triggered skill that convenes 1–5 read-only WAR auditor seats against the current changes and reports their verdicts in chat — no plan, no worktrees, no refinery, nothing filed or fixed without the operator. Trailing-integer seat count (1–5, default 1) and a trailing comma-lens list with `auto` seats (default auto) parse through the owned `skills/snipe/assets/snipe-args.mjs` CLI — order-tolerant, target text preserved, reserved lenses and duplicate lenses refused, effective seats = min(5, max(integer, lens tokens, 1)). Each seat spawns as a `war-auditor` subagent under the existing agent_type guard confinement, at the config tier ladder `agents.snipe` → `agents.auditor` → defaults: `agents.snipe` is newly defaulted opus/`high` in `war-config.mjs` and validated like `agents.redteam` (explicit `null` = unset; never a phase role — `agentMatrix` stays four roles). Default target is the current branch against its merge-base with the default branch, a dirty tree downgrades the whole report to advisory, every seat reviews at `deep`, and Critical/Major findings are labeled "would block in a phase" while gating nothing. Docs: a new README TL;DR led by `/snipe`, the CONTEXT.md **Snipe** term, the schemas.md agents row, and the `/war-room` override row. Release scope: 0.21.3 lands on 0.21.2 (the Vale Markdown lint release, PR #1918, carried by this branch); beyond the snipe skill, asset, and tests, the `agents.snipe` config key with its schema tests, and the named doc surfaces, no engine, guard, hook, or agent surface changed in this window.
