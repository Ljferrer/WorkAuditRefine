...
**0.14.56** — CLI main-guard normalization. The three run-as-CLI guards that still compared `fileURLToPath(import.meta.url)` against a raw `process.argv[1]` — in `skills/war/assets/stage-workflow.mjs`, `skills/war/assets/war-config.mjs`, and `skills/war-campaign/assets/campaign-ledger.mjs` — now use the canonical `process.argv[1] && fileURLToPath(import.meta.url) === realpathSync(process.argv[1])` idiom already proven at three sibling sites. Under a symlinked invocation (and, for the ledger's `file://`-string form, a percent-encodable path) the raw comparison came out unequal, so the CLI silently exited 0 without ever running `main()`; resolving `process.argv[1]` through `realpathSync` restores the fail-loud contract at every site. Each fix is locked by a symlink-invocation regression test that goes red against the pre-change guard. The origin lesson is amended RESOLVED with the corrected mechanism: the Node ≥ 24 baseline pre-resolves `process.argv[1]`, so the originally-reported relative-invocation trigger never fired — symlink invocation (plus percent-encodable paths on the ledger) is the real one.
