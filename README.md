...
- **Metacharacter-naming denial diagnostics (#1412)** — when the auditor git guard's forbidden-character check fires, the deny message now names the rule that fired (glob/alternation/expansion metacharacters are refused outright) and the remedy (search with the Grep tool, not shell `grep`/`git grep`); the guard's allowlist and deny decisions are byte-unchanged — only the diagnostic text grew. The search-tooling sentence rides both auditor prose surfaces. Home: [`validate-auditor-git.sh`](hooks/validate-auditor-git.sh).
