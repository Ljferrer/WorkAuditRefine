...
**0.14.8** — Working-branch checkout guard: `/war` now lands every phase autonomously from *any* launch context, including a worktree that already has the run's own working branch checked out — closing the `held:land-failed` footgun that forced manual per-phase lands. The collision is prevented at Setup: `provision-worktrees.sh resolve-working-branch` auto-creates a dedicated `dev/<date>-<slug>` working branch (at the desired tip, checked out nowhere) whenever the desired branch is already checked out in some worktree, otherwise returns the desired branch unchanged; Setup then bootstraps that branch on origin before Phase 1 and records the resolved branch in the ledger. A strictly-scoped `--afk` auto-recover completes the `--no-ff` land (gate-green, then push) for a checkout-collision `held:land-failed` on a clean fast-forward superset — every other `held:land-failed` cause stays a hold, and the push-first-CAS / never-force invariants are untouched. Git-topology logic stays confined to the tested `provision-worktrees.sh`; a new ADR records the dedicated-working-branch + origin-bootstrap decision, and `CONTEXT.md` defines *launch-worktree collision* and *dedicated working branch*.
