...
**0.14.31** — Seven verified fidelity gaps in the `/war` engine's audit/gate/dispatch spine are closed — each an engine-side record, routing set, or prompt clause that diverged from what the dispatched agent was told or reported back. `recordBaselineDebt` now dedups by **subset-containment** (with an empty-set carve-out that keeps the `(see gate_output)` entry minting), so a subset baseline report at the same `gate_base_sha` no longer mints a duplicate `auto` backstop entry (#798). A pin-mismatch **demotion** now strips the finding's `disposition` and legacy `autoFixable`, so a demoted `Minor` falls to the Nit default and can never ride `--ace` (#805). Each gate-audit evidence dispatch now stamps its **true immediate-predecessor tip** via a module-scope `lastLandedTip` updated on every real-sha land — a sentinel `integration_sha` leaves it untouched — fixing both the `requiresTest:false` over-count and the sentinel-poisons-successor defect the old `[i-1].gateHeadSha` chain carried (#806). The land-authoritative **integrated-tip gate-audit** seat, previously locked out of its own HARD path, now receives the teed `gate_log_path` through `EVIDENCE_RESULT.integratedTipGate` and a GATE LOG ARTIFACT clause (captured file authoritative for a provably-unrun finding; missing path → SOFT), mirrored into `war-refiner.md` in the same commit (#818). The floor-retry `add-test:`/`package-it:` dispatch is now tier-aware (`spawnWorker('fix')`), honoring an operator's `agents.worker.fix` override; the phase-close `polish:` dispatch stays non-tiered by design (#817). The land-step gate prompt names its true cwd — `the _refinery land worktree` — locked by a new source-count test so the three near-identical clauses can never silently re-converge (#815); and the `COST_CLAIM_SHARED` byte anchor carries adjacent comments naming its three coupled surfaces so any quote-style lint touches all three in one commit (#811). No task/phase status enum, `KNOWN_LAND_DECISIONS`, or `HARD_ESCALATION_REASONS` surface moved (ADR 0005); every fail-open boundary stays fail-open.
