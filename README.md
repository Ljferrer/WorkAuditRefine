...
**0.14.21** — Confinement guards, merge floors, and guard tests now cover their full equivalence class instead of a single hand-fixed instance, and standing meta-guards keep them that way. The worktree-scope hook's `..`-traversal reject widens to the full four-shape class — bare, leading, embedded, and trailing traversal — still evaluated before the agent-type switch so it binds every agent. A new hooks meta-guard enforces two authoring conventions with synthetic red fixtures: a frontmatter negation check must extract the whole fenced block rather than a bare header line, and a guard-test search root must anchor to the narrowest subtree or exclude the run's worktree directory. The rename absence guard now ignores the README Status and changelog prose regions, so a release note describing a renamed-away construct can no longer re-trip it while a real structural reintroduction still fails. `war-config.mjs` rejects glob and substring-prone test-pattern tokens at config time. A parity test makes the merge floor's discovery set a proven subset of the gate's output rather than one trusted by eye, and the packaging floor's added/renamed/copied-only scope is ratified in its header and an ADR 0017 addendum. A stderr precondition-marker reader contract lands on both refiner surfaces so an environment-caused gate failure is never misclassified as introduced, and the workflow template injects and verifies the absolute worktree root in every reported path. ADR 0031 records the equivalence-class and meta-guard conventions.
