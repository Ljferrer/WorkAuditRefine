...
**0.14.9** — Auditor calibration & lesson graduation: every built audit prompt and its standing `agents/war-auditor.md` mirror now carry two calibration sentences — one guarding against softening a finding under rebuttal or re-audit pressure, one disciplining cost/throughput claims — byte-locked across both surfaces by drift-guard tests. The `/lessons-learned` housekeeping pass now records recurrence trails and flags graduation candidates (frequently-recurring lessons worth promoting) as a fail-open advisory that never stalls a run or auto-files. All three adoptions ride existing always-present surfaces; no wholesale rules import anywhere.
