...
**0.21.4** — Vale Google profile, on by default: the Google developer-documentation style ([errata-ai/Google](https://github.com/errata-ai/Google) v0.7.1, MIT, provenance pinned in the profile header) is vendored under `hooks/vale-md/styles/Google/` — committed rather than `vale sync`-fetched, so lint time needs no network and rule updates are deliberate commits — and the hook's default profile is now `.vale-google.ini`: the house ReplyStandard rules plus a tuned Google cut with sixteen rules disabled (thirteen that judged entrenched repo voice in the 0.21.2 prototype sweep, plus `Google.Spelling` — the Reply Standard card mandates British — `Google.Latin` as a house-twin duplicate, and `Google.WordList`, an audit-flagged substitution list that fires on repo-ratified vocabulary). Selection is a new validated `hooks.valeGoogleFork` key (`war-config.mjs` DEFAULTS + validation, default `true`, `null` = unset, subordinate to `hooks.valeMarkdown`): only an explicit `false` swaps vale-md.py back to the house-only `.vale.ini`. The hook's advisory-only contract is unchanged — it still exits 0 on every path and blocks nothing. Also in this window: the committed repo run config `.claude/war/config.json`, deleted by a stray smoke-test cleanup in an earlier commit on this branch and caught by a two-seat audit, is restored with the three `hooks.*` keys added explicitly. Beyond the vendored style directory, the two profiles, the vale-md.py profile selector with its reworked test cases, the config key with its schema tests, the restored run config, the README, `/war-room`, and schemas doc lines, and the version slots, no engine, guard, or agent surface changed in this window.
