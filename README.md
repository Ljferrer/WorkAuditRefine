...
**0.21.15** — backward-chain doctrine: every role chains backward from the cited End state and fixes the class at its earliest unmet link, and the doctrine discloses by corrective round ([ADR 0052](docs/adr/0052-backward-chain-doctrine-and-corrective-round-disclosure.md)). Five reference files carry it, one per role plus an examples bank: `backward-chain-worker.md`, `backward-chain-audit.md` and `backward-chain-fix.md` under `skills/war/references/`, `backward-chain-plan.md` under `skills/war-strategy/references/`, and the examples bank `backward-chain-examples.md` under `skills/war/references/`, whose relation-tag H2s hold seed entries that cite a WAR issue and its comment or section, every external private entry abstracted with no repo name or number (PIN-9). The worker, audit and fixer files each carry a `## The rules` block that the engine mirrors byte-equal into its dispatched prompts: the worker block on the WORK build only, and the fixer block on all seven fix-applying builds (FIX_NEEDED, the ace subset, the ace re-entry, the ace advisory polish, the floor family, the phase-close sweep and the terminal pass). A corrective round is a fix round or an ace charge, counted 1-based per site: FIX_NEEDED and the roster audit read `round + 1 + r.task.absorbRounds`, the three ace sites read `r.task.fixRounds + r.task.absorbRounds` (never `r.round`), and the sweep, the terminal pass, the floor family and the pin-content re-audit are round 1 by definition. The round-1 audit prompt is byte-identical to the pre-doctrine build; from corrective round 2 the roster seat carries the audit rules, closes every blocking finding's `rationale` with an `upstream link:` line and a `relation: <tag>` last line, and the fixer and audit prompts carry an engine-built history digest (per prior round: each blocker's title, file, severity, relation tag and upstream link, the fixer's `Fix:` and `Ignore for now:` lines, the relation-tag sequence, the worker's `Critical path:` block, and full text for the survival-registry blockers only). The relation-tag vocabulary is equal across four surfaces (the audit rules block, the fixer Round 2 table, the examples H2 set less `## convergence` and the engine regex), bound by `backward-chain.test.mjs` and a `workflow-template.test.mjs` fixture. The round-5 disclosure: the `## Round 5 and later` tier, where a fixer may return `PLAN-DEFECT:` against an End state as a completed outcome, is named by the prompt only when the corrective-round helper returns 5 or more; the slice-level `PLAN-DEFECT:` route stays at every round, and `run.roundLimit` and `run.absorbRounds` are untouched. The chain rides existing fields, no schema change: `Critical path:` and `Ignore for now:` in `WorkerResult.notes`, the fix commit body block echoed into `notes`, and the two `rationale` last lines; `schemas.md` documents each by pointer. The plan-author file is read by pointer from `skills/war-machine/SKILL.md` §2 step 1 and `plan-interview.md` stage 1b, whose stage-1 falsifier list gains the reachability-probe bullet. `CONTEXT.md` defines the four terms: corrective round, relation tag, critical path block, history digest. The #2302 study records marker adherence, the sibling re-block rate and corrective rounds per task over the next five runs; it holds the schema-field trigger, the examples-bank size watch and the deferred classifier subagent. The template's prompt-literal share was re-baselined by the operator's approved pass for this run (`+34,816` B, `Budget-Raise` trailer citing ADR 0042, ADR 0048); no task edited a budget constant. Also in this window, unbumped at its own landing: the PIN TRANSFER GIT VERIFICATION prompt names `refs/heads/<integration branch>` as the `remote_sha` source, never the task branch's pushed tip, so a first merge with no origin integration ref no longer throws a false hard halt.

Release scope: the window from the 0.21.14 `master` head (`4cf4533a`, PR #2301 merged) to this release, bucketed whole. Engine: `workflow-template.js`. Agent cards: `war-worker.md`, `war-auditor.md` (two `when <trigger>, read` pointers on the worker card, one on the auditor card). Skills: `skills/war-machine/SKILL.md`, `skills/war-strategy/references/plan-interview.md`. References: the five new backward-chain files named above, `schemas.md`. Docs: `README.md`, `CHANGELOG.md`, `CONTEXT.md`, the new ADR 0052, the plan and its red-team report. Tests: the new `backward-chain.test.mjs`, `workflow-template.test.mjs` (seven `backward-chain:` fixtures and the pin-confirm ref fixture), `prompt-surface-budgets.test.mjs` (the operator's re-baseline only), `doc-cli-consistency.test.mjs` (five census rows). Lessons: `docs/learnings/` gains seven new files. Slots: the four version slots — `plugin.json`, `marketplace.json` (two) and the `README.md` `## Status` line. No file under `hooks/` changed, no floor script changed, no shell suite changed, and `fix-round-doctrine.md` is untouched.
