...
**0.14.60** — Submodule scoping now rides *every* merge-task dispatch, not just the first. The per-task `SUBMODULE TASK:` note — the paragraph that roots a submodule task's rebase and gate in its own checkout via `targetRepo`/`targetBase` — is appended as the final prompt segment of all three recovery re-merges in `skills/war/assets/workflow-template.js` (`merge:<taskId>:floor-retry:r<round>`, `merge:<taskId>:environment-proceed`, `merge:<taskId>:baseline-proceed`), where it previously rode only the initial merge-task prompt: a `taskType:'submodule'` task whose first merge tripped a recoverable route was re-dispatched **unscoped**, pointing the retried rebase and gate at the superproject. The note is the empty string off the submodule path, so every non-submodule prompt stays byte-identical. Three new sibling dispatch-capture tests in `skills/war/assets/workflow-template.test.mjs` drive a `taskType:'submodule'` fixture through each recovery route and assert the prompt located **by dispatch label** carries both the `SUBMODULE TASK:` marker and the fixture's `targetRepo` — dispatched-prompt content, never a source-occurrence count. The `gateCaptureClause` (D5) header comment drops the consumer-site enumeration that had rotted to 2 of its 3 sites and states the invariant count-free as an **evidence-contract** property — the clause rides the dispatched merge-task prompts whose evidence contract requires the captured fully-green gate for the post-merge gate-audit (ADR 0024) — naming the `captureUses` drift guard in `workflow-template.test.mjs` as the arbiter of the site list; deliberately *not* universal over merge-task prompts, since the baseline-proceed re-merge (proceed-over evidence contract) and the class-exempt phase-close polish merge legitimately carry no clause, and `captureUses` still counts exactly 3. The decision record is repaired in both directions without retro-editing: ADR 0019 gains a dated `## Amendment (2026-07-24)` recording that its `environment` class-route — the class-routes-doctrine row and the `**Scope.**` paragraph's `env-blocked` restatement, both under the single `## Decision` heading — was superseded by ADR 0040, restating the routing ADR 0040 already shipped (one bounded `environment-proceed` re-run per gate site whose re-run gate must come back fully green, never chained with another recovery dispatch; exhaustion HARD-escalates via `'escalate'` at the merge site and falls back to `held:land-failed` with reason `env-blocked` at the land site) and stating that the original Decision prose is byte-intact and that the `introduced` and `baseline` routes and the target-derived execution values stand unchanged; ADR 0040's `## Relationship to prior ADRs` gains one **Supersedes one row of ADR 0019** bullet, so a reader landing on either record reaches the live routing in one hop. **No behavior change:** `skills/war/assets/land-decision.mjs`, every `MergeResult` status, `HARD_ESCALATION_REASONS` / `KNOWN_LAND_DECISIONS`, every dispatch label, and every merge-path floor are untouched — this release ships prompt-prose appends, one comment rewrite, three new dispatch-capture tests, and two ADR record repairs.
