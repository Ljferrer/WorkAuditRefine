...
**0.14.29** — `/survey-corps` now turns lesson-recorded debt into issues before it sweeps. A new **Step 0 (Mine)** enumerates the hot lessons in both memory roots, verifies each against the live tree, and — for open, actionable defects only — drafts a `memory-mined` issue with a greppable `Lesson: <slug>` citation. Every drafted body is run through the fail-closed redaction lint **via a temp file** (the lint reads files, not stdin), so a lesson carrying a home path, email, or credential shape is **withheld** — never scrubbed, never filed; drafts are slug-deduped against open **and** closed issues so nothing re-files, and a closed-as-fixed issue whose lesson still verifies is reported as a stale-lesson signal rather than re-opened. Bare `/survey-corps` mines and files autonomously (cron-able); `--erwin` adds a pre-filing pause on top of the existing post-clustering gate. The closing coverage report gains a Mining section accounting for every mined lesson (`filed #N` / `withheld: redaction` / `previously adjudicated (#N)` / `skipped`), and the README now describes the survey as open issues **and memories** → specs.
