...
- **Zero-byte baseline check** — one `-s` test in the guard's `--baseline`-only arg-parse validation block: a zero-byte `--baseline` file now exits 2, the stderr die naming it "zero bytes (a truncated or failed snapshot write — infra error, never a pass)". Previously the awk `NR==FNR` two-file idiom degenerated on an empty first operand — every live ref reported as "removed", exit 1, a truncated-write infra fault laundered into an escape. The check sits in the arg-parse validation block, ahead of every check, so within baseline validation an infra fault is never preempted by an escape conclusion — suite case 28's fixture carries an escape-worthy `rogue` branch and asserts the exit is explicitly not 1. Invocations without `--baseline` are byte-untouched. Home: [`assert-no-repo-escape.sh`](skills/red-team/assets/assert-no-repo-escape.sh).
