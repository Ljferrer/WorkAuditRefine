...
**0.21.9** — doc truth and drift-guard debt (plan `2026-08-25-doc-truth-and-drift-guard-debt`): the named doc surfaces are re-truthed against HEAD, and each corrected fact a machine can derive gains a pin that reds when the fact rots. Three mechanisms land.

- **Prose truth across the named surfaces (#1979, #1980, #1981, #1982, #1983, #1984, #1985, #1986, #1987, #1988)** — the release-prose sweep re-truths the `CHANGELOG.md` relocated deictics and the `README.md` default-config and ace narration, which now describes the culprit-first bisection ladder and the 6-round budget. The tour drops all twelve raw step-level `"line"` anchors for pattern-only ones (CodeTour resolves `line` before `pattern`, so the keys were never supplements), gains the step-10 baseline and environment-proceed carve-outs, anchors the `..`-traversal guard by its `case` construct, and de-counts the roster narration. The D3-fallback framing is requalified as plugin-repo-only on `worker-servitor-edges.md`, its 2026-08-06 plan mirror, and ADR 0047. ADR 0018 gains a dated amendment, ADR 0026 a file-followups addendum, and ADR 0013 a dated correction re-pointing the absorb-eligibility-block attribution to `references/disposition-eligibility.md` — each an appended row, never a rewrite of ratified text. `CONTEXT.md` gets the _Avoid_ don't-confuse marker plus an ADR-0042 byte-identical eviction into `references/glossary-cold.md`, measured back under its advisory line. Two archived lessons state the sound recipe with provenance and slugs unchanged. `schemas.md` gains the `doneWhen` value-versus-key row and retires two absolutes the landed behavior falsified. `skills/war/SKILL.md` routes the ruled-ask filing as its own preflighted `gh`-write site after the Checkpoint ruling, with its pin keys moved lock-step. The budget-floor surfaces name `assert-budget-raise-cited.sh` in `CLAUDE.md`'s floor list, cite ADR 0048 by number in `budget-rebaseline.md`, and drop the refiner card's "verbatim" claim about its land prompt. `provision-worktrees.sh`'s Behaviors header and its suite banner both name the full publication/refinery divergence set instead of one behavior. Three `workflow-template.js` comment blocks are corrected in the same pass: the `const recovery` block names three dormant recovery arms with the holder auto-free's two refusal arms, the entry-validation header counts four problem classes, not three, and the endState header's whole-pass-absence absolute carries the vacuous-phase qualification (#1769, the comment twin of ADR 0013's #1772 note). Those two blocks and the `provision-worktrees.sh` header are comment text only — no engine or script behavior changed with them.
- **The corrected facts get pins (#1991, #1992, #1993, #1995)** — in `reference-link-integrity.test.mjs`, `README.md` joins the relative-link sweep, the five-card D3 sentence gets a named pin against the merged wording, `QUALIFIED_HEADERS` widens from nine live eviction destinations to eleven (`setup.md` and `docker-gate.md`) with a coverage comment that matches its scan, the Arm-1 positive control asserts literal bytes, and the no-path-form retirement scan reads whole text. In `skill-doc-contracts.test.mjs`, D36 re-binds to the normative Evidence-artifacts homes through construct extraction, D37's canonical-side keys scope to the ask-ruling-gate bullet and D40's never-filed-unruled key to the GitHub-conventions bullet, the `(?!--afk)` and both `D22_ORDERED_SPAN` fragments are both-ways proven, the D36/D35/D33/D31/D19a comments state what they really scan, and the `CONTEXT.md` ↔ ADR 0013 Intent-ceiling latitude mirror gains a drift-guard row. `workflow-template.test.mjs`'s D6 census checks membership per occurrence, so a keep-green occurrence that relocates outside a fixture-reachable dispatch site reds where the old count-only floor stayed green, and the refiner-card file-followups pointer and its destination body are pinned to the dispatched-prompt literals. `war-config.test.mjs`'s `roundLimit` watched-surface array gains a `README.md` row beside the tour's, and `war-pipeline-structure.test.sh` gains the war-review OLD-absent and presence pair.
- **`specCitations()` stops swallowing link-shaped citations (#1994)** — its path capture is a greedy `\S*` run, so a markdown link collapsed into one match. When the link's text half carved out as mechanics — a bare `docs/specs/` mention, a glob, a placeholder — the swallowed target half went unscanned, and a real spec citation escaped the posterity rule. The carve-out arms now keep the truncated-away tail and re-scan it, so a citation hidden in a link target is flagged. A flagged head already reported the whole run, so existing counts are unchanged. Four fixtures cover three carved-head shapes with a real spec target and a both-halves-mechanics link that keeps its carve-out.

Release scope: this window carries only the plan's own work. Beyond the doc surfaces named above, the four JS suites their facts are pinned in (`reference-link-integrity`, `skill-doc-contracts`, `workflow-template`, `war-config`) plus `war-pipeline-structure.test.sh`, the `specCitations()` fix and its fixtures in `doc-cli-consistency.test.mjs`, this plan with its red-team report, the `docs(learnings)` commits, and the version slots, no executable engine path, hook, `war-config.mjs` default, or agent-card duty changed — `agents/war-refiner.md`'s only edit is the two prose corrections named above.
