...
**0.14.51** — Servitor wrap-up grounds on a threaded landed-tip anchor: phase-close learning capture no longer trusts the servitor's own working directory to stand for the just-landed phase. The engine now threads a landed-tip anchor — a `Landed tip:` line hoisted from the handoff block's existing `tipSha` computation (one source of truth, no new tip semantics) — into the servitor Wrap-up prompt, and both wrap-up prompt surfaces (the standing `agents/war-servitor.md` card and the dispatched prompt string-built in `skills/war/assets/workflow-template.js`) now carry a shared four-step stale-cwd grounding ladder: cwd preflight → `gitdir`-matched worktree lookup → ref-check dead-end → gate-audit `auditSha` fallback. The servitor no longer assumes its cwd is the landed tip — the old cwd-is-the-landed-phase premise is inverted at both surfaces — so phase-close capture stops minting confidently-wrong `code-verified` lessons from a stale checkout and stops spending rounds hunting worktrees Refine already reaped. A null tip pre-resolves to a named placeholder before any interpolation, so the dispatch never throws and never renders the string `undefined` (ADR 0034). Locked by a new **D3 both-surfaces registry row** at a no-slack `REGISTRY.length` floor plus whole-string premise-absence guards and threaded-value tests; `docs/adr/0029` gains a dated append-only amendment, the origin lesson a dated "Mechanized (#990)" mitigation note, and `CONTEXT.md`'s Finding-match check entry an `_Avoid_` clause. Servitor confinement is unchanged — Read/Grep/Glob only, no Bash (ADR 0002).
