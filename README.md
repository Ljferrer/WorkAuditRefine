...
**0.14.13** — Servitor learnings write path: a landed phase's lessons now actually ride the phase PR, and the operator's personal memory store is immutable to agents. The servitor's write surface collapses to **one root** — a threaded absolute local memory root, its only writable path; every lesson is written there, and `type: project` marks a lesson *promotable*. Repo-root publication becomes a **Lead Gate-2 promotion**: the Lead reconciles the servitor's `files_written` (absolute-prefix check, stray paths fail loud), provisions a transient phase-scoped `p<N>-publication` worktree on the working branch, copies + redaction-lints the promotable lessons there (fail-closed — a flagged file stays local, never dropped), commits `docs(learnings): phase N` plus the CLAUDE.md pointer duty, pushes via the existing push-first CAS (fetch → replay → retry-once → escalate, never force), then stamps each promoted local original with a nested `metadata.promoted:` marker — a copy-with-marker, never a move, so the lesson stays recall-visible in-run. A leftover publication worktree from a crash is healed at Setup and Gate-2 entry. Two hooks tighten, shape/content-based: the scope hook subtracts the servitor's repo-root allowance (#58 resolved), and a new **existing-target mutation guard** denies any servitor Write/Edit/NotebookEdit over a pre-existing memory file that lacks a nested `metadata.provenance` — an untagged file is user-authored, top-of-ladder, agent-immutable. Every dispatched-prompt change lands with its `agents/war-servitor.md` mirror; ADR 0022 and CONTEXT.md ratify the write/publish split, the promotion marker lifecycle, and the WAR-editable-memory-file discriminator.
