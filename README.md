...
**0.14.11** — Diagnosis pre-flight, the self-confound gate: WAR already adversarially verifies what agents *build*; this turns the same refute discipline **inward**, on what they *conclude about failures*. A standing-instruction doctrine — prose, not a hook, floor, or code path — now reaches every diagnosing role across six surfaces: the red-team Lead (its `skills/red-team/SKILL.md` canonical home plus the `references/lenses.md` Safety reference), the runtime adversarial-confirm stage (one sentence in the `confirmStage` prompt string), the WAR Lead (`skills/war/SKILL.md` Invariants), workers and fix-workers (`agents/war-worker.md`), and the servitor write-side (`agents/war-servitor.md` D3). Before an observed failure may be blamed on a systemic bug, the agent must enumerate and rule out its own and any concurrent actor's recent mutating actions (action-provenance first), keep validation single-path over shared mutable state (never manual + automated bring-up back-to-back), gate hypothesis promotion — a memory lesson, a `war-followup` issue, a fix plan or spec, a sub-agent fan-out (the closed four-channel list) — on primary evidence plus an inward refute pass, and state the falsifier. The gate blocks **promotion, never diagnosis or escalation**: when primary evidence is unobtainable, the diagnosis is recorded as a labeled hypothesis note and/or escalated, then work proceeds. Clause presence — per surface and per canonical part — is presence-locked by a new structure test, `skills/red-team/diagnosis-preflight.test.sh`; ADR 0020 and CONTEXT.md ratify the terms.
