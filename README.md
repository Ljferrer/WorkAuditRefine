...
- **Watchdog group kill** — `assert-done-when.sh` launches the acceptance command under `set -m`, so the command tree gets its own process group, and the timeout watchdog delivers TERM (and the KILL insurance 2s later) group-first — `kill -TERM -"$cmd_pid"` — with the single-PID form as fallback: a backgrounded grandchild that stayed in the command's group no longer holds the inherited stdout fd past the kill (suite-pinned: a `sleep 60 & wait` command under `--timeout 1` exits 1 with the timed-out diagnostic and the stdout capture returns inside the suite's 15s wall bound). The header states the bounded residuals: the group kill does not reach a grandchild that escaped into a NEW session/process group (bash 3.2 has no `setsid`), and the KILL insurance fires only while the watchdog is still alive, so a TERM-ignoring descendant that never left the group can outlive the teardown — neither survivor can change the exit code. Stdin is redirected from `/dev/null` explicitly: under `set -m` bash no longer supplies it to a background command, and without the redirect a stdin-reading acceptance command would block to the full budget and read as a false `done-unmet` (case 22 pins it). Home: [`assert-done-when.sh`](skills/war/assets/assert-done-when.sh).
