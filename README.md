...
**Why it's required.** A run is only as good as the spec behind it, and authoring that spec is an interview: `/grill-with-docs` walks you down every branch of the design tree, resolving one decision at a time, until the result is unambiguous and cleanly phase-decomposable — the shape `/war` needs to fan out workers and auditors. The rest of the pipeline assumes that work is already done: `/war-strategy` converts a spec into a plan but **never authors one from scratch**, `/red-team` proves the plan's claims, `/war` executes it. `/war-strategy`'s dependency check looks for `grill-with-docs` or `domain-modeling` and stops to point you here when neither is installed.

**Why WAR doesn't ship its own.** Deliberately: a bundled interview loop would be a second home for spec-authoring doctrine — drifting from the original, inheriting none of its upstream improvements, and doubling the surface you have to trust. WAR routes to the installed skill instead, so there is one canonical home, maintained by the people who wrote it.

Nothing else is required. Other plugins run happily alongside WAR — no part of the pipeline depends on them.
...
**0.15.1** — Four ratified-but-unenforced `/red-team` doctrines become mechanical and greppable. **`ADJUDICATED` is now a distinct, gate-emitted terminal verdict** ([ADR 0043](docs/adr/0043-adjudicated-clear-distinct-terminal-verdict.md)): `verdict()` in `skills/red-team/assets/red-team-gate.mjs` emits the precedence `INCOMPLETE` > `BLOCKED` > `ADJUDICATED` > `CLEARED-WITH-NOTES` > `CLEARED`, driven by a typed per-finding `adjudicated` flag read on strict `=== true` — the same typed-flag pattern as `deliverableAbsence`/`envGap`, with no NLP over finding text. What it encodes is **stamp-never-remove**: a blocker the Lead re-verified by re-running the probe that measured it is still *removed* (probe-proven, so `CLEARED` stays reachable), while a blocker that was patched but **not** re-verified is *stamped* in the Lead's working copy of the gate input and re-piped — never deleted — so the report keeps listing it and the run terminates `ADJUDICATED` instead of reaching a fake `CLEARED` by deleting findings. The flag is read inside `verdict()` alone: `classify()` bucketing is untouched, adjudicated blockers still appear in `blockers`, and the scan covers the blockers + `needsDecision` union only, so an unstamped Minor note never holds the verdict and a run with zero blockers and zero `needsDecision` can never be `ADJUDICATED`. A finding set carrying no flag anywhere lands exactly where it landed before. **The sandbox-escape guard's ref half graduates from a name heuristic to an exact ref-diff.** `skills/red-team/assets/assert-no-repo-escape.sh` gains `--snapshot <abs-file>`, run immediately before the verification Workflow launches, and `--baseline <abs-file>`, run after it returns: given a baseline, the guard flags any added, removed, or moved local ref against the pre-run snapshot — fully name-agnostic, no allowlist and no pattern — within `refs/heads/` and `refs/tags/`, the two namespaces it scopes the diff to because `for-each-ref` also covers `refs/remotes/*`, which a background `git fetch` moves without any probe writing into the repo. Origin-side detection is **not** widened: a probe pushing an invented, non-pattern ref name to `origin` still slips the heuristic floor, and that ceiling is named in the guard's rewritten ponytail beside two others — refs that predate the first baselined run, and gitignored leak paths, which `git status --porcelain` does not report. Snapshot mode refuses to baseline pre-existing residue or dirt: a junk-pattern local ref or an unclean working tree *before* launch is exit 1 and no snapshot file is written. A missing or unreadable baseline file is exit 2 — never 1, never a silent pass. Called **without** `--baseline`, check mode keeps its exit codes, stdout, and checks exactly as they were, with a single stderr advisory as the sole delta. The sanctioned sandbox idiom moved with the guard, on both `workflow-scaffold.js`'s SCOPE-LOCK preamble and `SKILL.md` Step 3: `git clone --no-hardlinks` whenever a probe runs `git` at all (the only form that stays isolated when the target is itself a linked worktree, and one that carries committed state only), `worktree add --detach` for read-only inspection of committed state, and `cp -R` only where nothing but file reads and edits will happen — because the plain, non-detached worktree form the old text sanctioned writes a `refs/heads/<tmp-basename>` branch into the target that outlives `rm -rf` of the sandbox directory, so the sanctioned idiom would otherwise have tripped the new ref-diff on `/red-team`'s own prescribed workflow. **The guard-split deps-edge rule gains a forcing function on every plan-authoring path**: a task that authors a drift guard over a fact a *different* task in the same phase authors must carry a `deps` edge onto that task — same wave is insufficient, because every task worktree is cut from one frozen phase base, so an unedged guard is red at its own base through no fault of its own diff. Three surfaces now force it — `/red-team`'s new `guard-split-deps-edge` spine probe (an unedged split is a plan defect, `needsDecision`; never skipped under `--fast`), `/war-machine`'s **Guard-split deps-edge directive** for the drafter, and `/war-strategy` §3's authoring rule 7 for the non-machine conversion path — recorded canonically in a dated amendment to [ADR 0025](docs/adr/0025-drift-guard-discipline.md). **And adjudication G's carve-out granularity is recorded once, canonically**, in ADR 0043's own subsection: the extraction's read *is* the carve-out — ordered/positional-key regions atomic, unread residue shrink-eligible, and the rationale bounded to runtime prompt strings, never a standing-doc card span. Every doctrine ships beside its own guard, in the same task or across the `deps` edge the rule itself requires: nine new gate tests pin the verdict arms (two of them Minor-note fixtures written so the pin cannot pass against a wrong flag scope), seventeen new guard-suite cases cover both ref namespaces, both pre-run refusal arms, the argument-containment layer, and the recorded #1244 miss this release closes, and `workflow-scaffold.test.mjs` adds a presence-pair guard over both prose surfaces, a sandbox-idiom guard asserting the ref-writing form is on neither, and a default-deny verdict-enumeration guard that extracts `verdict()`'s literal set and asserts token-set equality against each documented verdict line — so the next verdict widening reds a guard instead of drifting. The enumeration cascade lands with them: `references/lenses.md`'s severity/gate list and report template (which also gains the long-absent `INCOMPLETE`), `skills/war/references/design.md`, `skills/war-campaign/SKILL.md`'s halt-and-hold clause, this README's `/red-team` overview, and five new `CONTEXT.md` domain terms. **No `/war` engine file, hook, or merge-path floor changed:** outside `skills/red-team/`, the release diff is documentation — doctrine prose, two ADRs, the plan and its red-team report, and the phase's recorded lessons — plus the `war-pipeline-structure.test.sh` pins guarding the new authoring rule on both surfaces (case-insensitive, on mid-sentence anchors, with the retired rule-count word asserted absent — the default-flip half a presence pin cannot cover) and the `war-strategy-structure.test.sh` comment that points at them.
