...
**0.17.0** — the precision chain and the loop breaker: what the authoring contract writes, the run now proves — a task merges only when its own acceptance command passes, a claimed End-state condition lands `met` only on evidence a seat can cite, and a plan that cannot stop churning in red-team is routed back to the interview instead of ground forever ([ADR 0045](docs/adr/0045-red-team-loop-budget-and-route-upstream.md)). **The `done-unmet` floor:** the Decompose step parses each task's `Done when:` bullet in full (soft-wrap join) and treats a `requiresTest: true` task without one as an intake defect — Decompose halts at the plan-approval gate for the operator to supply the command or re-rule `requiresTest`, an intake rule pinned by `skill-doc-contracts.test.mjs`'s D31 row. The parsed command rides the worker prompt and all five fix-family dispatches as a `Done when:` line beside the gate (a task without one renders the clause `''`, so a legacy plan's prompts stay byte-identical to a done-when-less run — test-pinned by set-minus), and at merge the refiner executes it in the task worktree via the new `assert-done-when.sh` floor: file-threaded through `--cmd-file` (never interpolated into another script), a watchdog timeout (default 600s; a hung command fails that dispatch only — exit 1, never the merge queue), and the floor family's exit contract — exit 1 when the acceptance command exits non-zero or times out, routed `done-unmet`, a status new in both slots (the `HARD_ESCALATION_REASONS` canonical export plus its hand-mirrored workflow copy, and the `MergeResult` status enum plus `FLOOR_STATUSES`), with bounded fix rounds sharing `run.roundLimit` behind the new `MAKE_DONE_PASS` dispatch; exit 2 on a git/env error (bad worktree, missing/unreadable command file), kept `error`, never `done-unmet`. The floor runs the command unvalidated by design — no charset gate (F6, operator-ratified): a `Done when:` is red-teamed, operator-ratified plan content in the same trust class as `plan.gate`, which the floor family has always executed unvalidated. **Artifact-first gate-audit:** the Lead threads each phase's claimed End-state conditions as `{ condition, tag, check }` rows (a bare legacy `endState` string normalizes to the untagged judgment path); after the phase's last merge and before gate-audit spawns, the land-barrier endstate-check dispatch executes every claimed `check:`-tagged condition once at the integrated tip, teeing one artifact per condition to `.war/endstate-<phaseId>-<n>.log` stamped with the tip SHA it ran at; `assert-test-in-diff.sh` now prints every matched test path on exit 0, one per line, instead of stopping at the first hit, and the refiner returns those paths as `MergeResult.mappedTests`; the gate-audit seats then verify from artifacts — a `check:`-tagged condition from its teed file (stamped tip compared against the confirmed tip), a `gate:`-tagged one from the captured gate logs the `mappedTests` paths are grepped against, a judged one from named observables — and return per-condition `endStateAttestations` (`met` | `unmet` | `unverified`), where a condition no seat attests lands `unverified` in the handoff, never `met`: a missing, unreadable, or stale-tip artifact all map there, never to a block. Auditors stay read-only (ADR 0002) — the refiner and the endstate-check dispatch execute everything the seats attest from. **The red-team loop budget:** `red-team-gate.mjs` accepts `--rounds=<n>` / `--round-limit=<n>` (`=`-attached flags or top-level input keys; a flag overrides the key), echoes each supplied input in its output, and computes `routeUpstream` by pure arithmetic over the unadjudicated subset — the limit reached with an unadjudicated root blocker still open, or an unadjudicated `needsDecision` persisting at rounds ≥ 2 — with the routeUpstream ⇒ BLOCKED invariant pinned and a rounds-less invocation's stdout byte-identical to before; `run.redteamRoundLimit` validates in war-config (default 3, economy preset 2); `/red-team` counts rounds cumulatively (a round is one full grill sweep) and carries `--rounds`/`--round-limit` on every gate computation of a run, and a route-upstream terminal emits the `## Route upstream` block ([loop-budget doctrine](skills/red-team/references/loop-budget.md)) as the `/war-strategy` regrill agenda. Downstream, `/war-campaign` step 3 is the three-arm triage — a proceed verdict commits the patched plan and report onto `dev/<slug>`; `routeUpstream: true`, `BLOCKED` with residual open questions, and persistent `INCOMPLETE` halt-and-hold with `stopPoint: redteam-route-upstream` and the regrill agenda in CAMPAIGN-STATE.md — and every arm stamps the ledger's per-plan `redteamRounds`; `/war-review` renders three plan-scoped telemetry rows (this plan's rounds, the per-plan trend across campaigns, interview length), each cell `n/a` when its source is absent, never fabricated. And **spec posterity** ([ADR 0046](docs/adr/0046-spec-posterity-skills-cite-maintained-surfaces.md)): `docs/specs/` files are write-once history — no skill doctrine surface, and not `README.md`, cites one by concrete path; the enforcing `doc-cli-consistency.test.mjs` rule strips fenced blocks, carves out input-shape mechanics (placeholder and glob path-shapes, and the bare directory mention this very sentence uses), and pins its own corpus membership, so dropping a scanned surface reds the guard instead of narrowing it.
