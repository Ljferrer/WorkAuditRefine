...
Both events fire in the main conversation only: WAR's phase subagents (workers, auditors, refiners, servitors) see neither hook — `UserPromptSubmit` does not fire for a subagent's spawn prompt, and a subagent's turn-end is `SubagentStop`, not `Stop`.
