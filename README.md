...
**0.14.30** — Two live `/war` engine defects that surfaced errors as the wrong outcome deep in a run now get a named, boundary-time route. A launch missing a phase's `title`, `workingBranch`, or `integrationBranch` used to die opaquely inside pt-tagged prompt construction; the **Entry validation (H)** block now runs an unconditional phase-field check that fails at entry with a message naming the absent field(s) — zero agents spawned, git untouched — and appends the `(or supply explicit branch/worktree per task)` hint only when a derivation-class problem fired. And an engine-side throw inside a task's work+audit thunk used to be nulled by the live `parallel`, so the result was dropped, `nextWave()` silently re-dispatched the completed task every wave iteration (~660k tokens/round), and the task ended mislabeled `unrunnable-deps`; the whole per-task thunk body is now wrapped to return `verdict: 'escalate'`, so a single dispatch yields exactly one collected result and the task lands `held:escalation` with its real cause. The per-task provision evidence-gate and inside-thunk prompt-interpolation throws move from harness-only `held:workflow-error` to `held:escalation` in step. `assertReportedPathsInWorktree` is reshaped into `normalizeReportedPaths` (relative pass / this-worktree pass / sibling-worktree throw / `mainCheckout`-rooted rewrite+warn / other-absolute throw), and the worktree-relative `files_changed` contract is stated once as a canonical `FILES_CHANGED_RULE` byte-mirrored across both dispatched prompts and `schemas.md`.
