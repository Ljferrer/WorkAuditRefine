...
**0.21.7** — engine concurrency and pin transfer (plan `2026-08-30-engine-concurrency-and-pin-transfer`): `run.maxParallel` becomes one global agent ceiling, and a WAR phase stops paying for re-audits inside the integration lock. Three mechanisms land.

- **Global dispatch semaphore (#1929, #1930)** — the per-site `batched(thunks, n)` throttle is replaced by one counting semaphore (`makeSemaphore`) held at a single leaf agent-dispatch seam (`dispatch()`), so a nested wave × roster fan-out no longer composes to about N² agents in flight. The permit releases in a `finally`, so a thrown or rejected dispatch never leaks one; no enclosing slot holds a permit while it awaits a nested dispatch, so a wave wider than N still completes; and with the knob absent `dispatch()` returns `agent(prompt, opts)` straight through on a byte-identical path. The four live doc surfaces restate the knob as a global ceiling — `CONTEXT.md` (the **Batching helper** glossary entry becomes **Dispatch semaphore**), `/war-room`, `references/schemas.md`, and `references/design.md` — with `/red-team`'s own probe-group slicing named as a ratified non-goal. The new `doc-semantics.test.mjs` is the standing both-ways guard over those four paths, and the retired per-site lesson carries a dated RESOLVED stamp.
- **Wave-side ace path (#1931)** — the ace batch, its re-audit, `aceBisect`, and `aceReentry` move out of the serial merge queue into the wave thunk. They run per task at that task's panel-approved tip, concurrent across tasks, and finish before the merge queue opens. The task gate runs at every ace-family tip before its re-audit: a red gate forward-reverts the ace commit, the approved pre-ace tip merges, and the findings demote per their dispositions — never a fix loop, never a hold. Each re-audit scales by the git-derived ace file set carried on the new `ace_diff_files` result property: a set inside the findings' own file set re-runs only the originating seats and transfers every other seat's approval, per seat, onto the new `pinTransfers` ledger; an absent or empty set, a `files_changed` mismatch, a file outside that footprint, or a seat that detects one re-runs the full panel. A wave-side ace round still charges the shared `fixRounds` budget, and the audit-verdict entry records the pre-ace round count.
- **Rebase pin transfer (#1931)** — at the merge slot a refiner probe requires a conflict-free rebase, then compares `git patch-id --stable` of the task's own diff before and after. Equality transfers the panel pin. A mismatch degrades that one task to the in-lock full-panel re-audit, whose own fresh Minor and Nit findings now route by disposition instead of being dropped. One arm precedes the equality test: an empty post-rebase diff whose pre-rebase commits all cherry-match upstream records `already_upstream` with the matched task commits named, while an empty diff with zero task commits, or an empty pre-rebase patch-id, escalates instead. The probe carries its own `PIN_TRANSFER` schema, so no `MergeResult` status, `HARD_ESCALATION_REASONS` member, or `KNOWN_LAND_DECISIONS` member changes.

ADR 0049 records the three transfer forms under one principle — verification cost proportional to risk class — and `CONTEXT.md` gains the **Pin transfer** glossary entry. The transfer rule lands on both prompt layers in one commit: the standing auditor card and the dispatched prompts, with the refiner card and `schemas.md` carrying the probe's steps and return shape, and the probe's four arms evicted to `references/refiner-recovery.md` behind a trigger pointer. In-plan, two prompt-surface ceilings rose by the plan's pre-ratified PIN-17 row (+2,048 B each, after in-task compression); post-plan, a separate ADR 0048 §2 operator-gated re-baseline (#1952) replaced the `workflow-template.js` prompt-literal metric — `MIN_BLOCK_BYTES` 200 → 0, hard 137,216 B — because the floor hid prompt growth built from small fragments, so `agents/war-refiner.md`'s +2,048 B is the only PIN-17 raise still standing verbatim. A post-land audit family rides the same window: the ace-family `recordAced` split with its footprint-intersection gate (#1954), the ace gate-check's required placed `head_sha` (schema arm-split plus engine fail-closed, #1951), the merge-task rebase skip (card-authoritative, five prompt sites ruled by census), a doc-truth cascade scoping the floor-retry reserve to its three wave-side sources across five homes plus a dated ADR 0013 amendment, and a `CONTEXT.md` eviction of two cold glossary bodies to the new `references/glossary-cold.md` (headroom 1 B → 2,064 B, ceiling unchanged). Release scope: beyond the engine and its tests, `doc-semantics.test.mjs`, the two ADRs, the named doc, prompt, budget-suite, contract-suite, and glossary surfaces (`skills/war/SKILL.md` and `references/design.md` included), this plan with its red-team report, the three `docs(learnings)` commits, and the version slots, no guard, hook, or `war-config.mjs` surface changed in this window (`/war-room`'s `run.maxParallel` row is among the reworded doc surfaces above).
