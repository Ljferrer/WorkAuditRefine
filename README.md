...
**0.14.16** — Audit/gate verdict fidelity: the deterministic parts of the audit and gate anti-cheat spine move out of skippable prose into Node checks, refiner-run shell floors, and captured artifacts, so no seat hand-reconstructs a git proof and no finding-less `escalate` silently lands. A pure-Node pin-equality gate demotes any audit seat whose returned `audit_sha` does not match its dispatched pin to a soft `pin-mismatch` that contributes no hard hold. A new refiner-side floor (`gate-pin-status.sh`) computes the confirmed / benign-advance / stale-mismatch proof with the standard 0/1/2 exit discipline, treating a descendant integration tip that touched none of the task's own mapped files as a benign advance and a task-own file change or non-ancestor tip as a stale mismatch. A gate-audit `escalate` verdict now holds the land as a second hard trigger even with zero Critical/Major findings, reusing the existing `gate-evidence` reason with no new escalation member. The refiner tees full gate stdout and stderr to a per-task `.war/gate-<taskId>.log` artifact and returns its absolute path, and on a phase with intra-phase dependencies re-runs the gate once at the final integration tip as the land-authoritative execution evidence. A second floor (`assert-guard-specificity-in-diff.sh`) flags a new `die`/stderr guard whose unique message no same-diff test asserts. All evidence plumbing fails open — a missing token or artifact is a soft cannot-confirm, never a hold. New ADR 0024 ratifies integrated-tip authority, the pin-equality gate, the benign-advance floor, and verdict-as-second-hard-trigger.
