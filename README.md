...
**0.14.63** — Four code-verified defects in the memory tooling are closed — three that silently substituted a result the operator never asked for, one that leaked state — each bound at its own boundary, never a shared one. **(1) An invalid `--target` now refuses (#1059).** `war-memory.mjs tighten-plan --target` previously exited 0 and printed a plausible plan for two distinct bad inputs: `parseArgv` maps a **bare** flag to boolean `true`, so `Number(true) === 1` bound the pass at a one-byte goal and pre-selected **every** eligible lesson onto the `/lessons-learned tighten` strike list, while a non-numeric value became `NaN` and echoed `target: null` with a null cut goal and an **empty** strike list — each matching the default run only in its verdict, never in its cut plan. A supplied-but-invalid `--target` (bare, non-numeric, zero, or negative) now exits **1** with empty stdout and a stderr diagnostic naming the received token, mirroring the existing `requireLocal` refusal idiom. **This is a behavior change at the CLI edge:** anything scripting `tighten-plan --target <garbage>` got advisory-bound output before and gets exit 1 now — the only in-repo caller is the `/lessons-learned tighten` step-1 fence this release rewrites. The guard lives at `cmdTightenPlan`'s argv boundary **only**: `parseArgv` is shared by every verb and its bare-flag→`true` mapping stays load-bearing elsewhere (`archive --candidates`), and `tightenPlan()`'s exported signature, its `target = WARN_BYTES` default, and the #992 stricter-of verdict are byte-untouched — an **absent** flag still takes the 17,000 B advisory default, byte-identically, and every pre-existing `tighten-plan` test stays green unmodified. **(2) The step-1 tighten fence is dialect-safe (#1088).** `skills/lessons-learned/SKILL.md` threaded the operator's byte figure to the flag through an alternate-value parameter expansion on a shell variable — a construct zsh does not word-split, so the flag and its value **fused into a single argv word**, `--target` never bound, and the pass silently reverted to the 17,000 B advisory default. The fence now carries both invocations — flagged, with the operator's literal byte figure substituted for a placeholder, and bare — as two lines in **one** ```bash fence, with prose directing the agent to run exactly one of them; the set-then-thread variable is retired from the doc entirely. Two doc-contract locks hold it: the existing lock proving the fence still threads an operator-supplied `--target`, and a new absence lock pinning the banned expansion **shape** (not the narrower flag-attached literal, so a respelling that moves the flag inside the same expansion is equally red) plus a whole-file token check, so a prose-only reintroduction is red too. **(3) The seed-mode local render keeps its `[repo]` rows (#1086).** `skills/lessons-learned/references/seeding.md`'s `## Seed` **Local destination:** bullet mandates `--repo docs/learnings/` **iff** `$REPO_ROOT` is non-empty, while the code block right beneath it showed only the bare render — copied literally it rendered local-only and silently dropped every `[repo]` row, the exact recorded known trap. That block now shows both branches explicitly, one commented line each, with the same run-exactly-one direction; a new doc-contract lock **slice-scoped to that bullet** proves the `--repo docs/learnings/` variant is present — seeding.md carries two `render-index --local "$MEM"` fences and the **first** is the repo-destination one, so a whole-file check would pass vacuously against the wrong fence and guard nothing. The `evict` re-render stays local-only with its lock untouched, as do migration.md's two deliberately local-only renders. Neither doc fix introduces a new alternate-value expansion site — that construct is being retired, not relocated. **(4) seed-pack's error exits unwind their scratch cleanup (#1079).** `die()` wrote to stderr and called `process.exit` from **inside** `cmdPack`'s, `verifyTier`'s, and `cmdEvict`'s `try` regions; `process.exit` bypasses stack unwinding, so those `finally` blocks never ran and their `mkdtemp` scratch dirs leaked under the OS temp root on every error path. `die()` now throws an `Error` tagged with its exit code, and `main()` is the **one** place that inspects the tag, writes the message, and exits — after every `finally` has unwound. An untagged throw is rethrown unchanged so an engine bug never masquerades as a contract exit, and the unknown-verb refusal deliberately stays inside `main()`'s try (moved above it, the throw would escape uncaught and Node would print a stack trace instead of the clean single-line refusal). The observable subprocess contract is unchanged — every exit status and every stderr byte identical, the ratified exit-code map untouched, `rankEvictions` and the realpath-normalized CLI-entry guard byte-untouched, and the **entire** pre-existing `seed-pack.test.mjs` suite passes unmodified. New coverage proves the closure rather than asserting it: a `TMPDIR`-scoped test packs a valid seed dir, corrupts one manifest `sha256`, runs `verify` as a subprocess, and asserts the sha-mismatch exit **and** zero `seed-pack-*` entries left behind (red against the old `die()`); `skills/lessons-learned/seed-set.test.mjs` — which runs `seed-pack verify` against the committed `docs/seed/` and is picked up by the gate's own `*.test.mjs` discovery — stays byte-unmodified and green, so the reworked verifier is exercised against the real shipped corpus **pre-merge**, not deferred to a manual step. The three origin lessons under `docs/learnings/` carry `RESOLVED (…)` description prefixes naming this plan and the owning task slice.
