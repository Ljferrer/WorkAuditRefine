...
**0.14.68** — Audit evidence precedence (ADR 0041): the run's evidence-handling roles now judge claims against **four closed claim-shape ladders** instead of scattered prose. A claim under audit is classified by shape — `content-at-pin` ("is X present / worded-thus at the judged state"), `execution` ("did it run / did it pass"), `history` ("when did this change / was it ever removed"), `authority` ("what was decided / what version / what is in scope") — and judged at the highest rung of that shape's ladder that holds. Rank inverts by claim (for content the pinned blob outranks the gate log; for execution the gate log outranks the blob), which is why the ADR rejects any one total evidence order. The shape set is closed with a default arm: an unmatched claim is judged under `content-at-pin` (strictest), and **when** rung 1 is genuinely unworkable the licensed outcome is a SOFT `cannot-confirm`, never a minted fifth shape. Four floor rules hold across all shapes for the bound roles: the working tree and the worker done-report are never any ladder's top rung; prefetched lessons are priors, never evidence — a lesson-derived claim is re-grounded at the pin before it may appear in a finding; on a cross-rung conflict the higher rung rules the verdict and the contradiction is mandatorily recorded as a `disposition: note` finding naming both rungs (benign forward-advance — the steady-state pin/HEAD mismatch — is not a contradiction); and the default arm above. Bound is exactly the spec's D5 set: the auditor seats **including the reserved passes** (the `execution-evidence` gate-audit, the `pin-validity` pre-flight), the Lead's phase-close and Gate-2 evidence duties, and the servitor — workers and refiners are deliberately not bound. Pre-existing role disciplines are cited as instantiations, never restated: the `agents/war-servitor.md` diff is one added cross-reference line naming its ADR 0007 verify-on-write discipline as this doctrine's instantiation, and the Lead's remote-truth binding cites ADR 0008 rather than restating it. **Surfaces are tiered — full text in one agent-surface home, token skeletons in the hot path.** The full ladders with their rung bodies and all four floor rules land on the auditor standing card, a new `## Evidence precedence` section of `agents/war-auditor.md` — the only *agent or prompt* surface carrying them (ADR 0041 and the spec carry them as doctrine record). Four dispatched-prompt surfaces in `workflow-template.js` — `auditPrompt()` plus the three gate-audit-family seats (post-merge `execution-evidence`, the AUTHORITATIVE integrated-tip seat, and end-state-only), which sit outside `auditPrompt()` and inherit nothing from it — each carry an identical two-sentence skeleton: the four shape names, the two floor tokens, and a pointer to the card; the four discriminating §4.1 rung-body tokens measure 0 on those dispatched surfaces. `skills/war/SKILL.md` gains one `**Lead evidence bindings (phase-close + Gate 2; ADR 0041).**` paragraph — the same skeleton plus three Lead bindings: (1) close-out and publication evidence is gathered at the plan's branch tip in a dedicated worktree, never the main checkout; (2) remote truth comes from `ls-remote` (ADR 0008, cited); (3) a claim the Lead threads into a dispatch prompt becomes rung 1 of `authority` for the receiving agent, so the Lead grounds it at `content-at-pin`/`execution` first or marks it unverified. `CONTEXT.md`'s `### Audit` glossary gains three terms — **claim shape**, **evidence rung**, **rule + record** — each with an `_Avoid_` line. **Three new guard rows ship in the same commits as the mirrors they pin** (ADR 0025; counted at this release's tip, not from the plan literal): **(1)** the D26 row in `skill-doc-contracts.test.mjs` mirrors the three glossary terms against ADR 0041 — per-term extraction by construct (bolded term to the next bolded term or heading), token-anchored `\s+`-wrapped keys rather than sentence bytes, so a glossary reword away from a doctrine clause reds while sanctioned rewording latitude does not; the by-construct extraction is the `CONTEXT.md` side only — the ADR side is matched file-wide, so a key with a second home in ADR 0041 still greens when only one home is removed. **(2)** the five-surface evidence-precedence row in `workflow-template.test.mjs`'s both-surfaces registry anchors the shared skeleton tokens on the standing card plus all four dispatched surfaces, so a diff dropping the skeleton from any single one of the five copies reds (token-anchored, so a reword preserving the anchors deliberately does not). Its anchor choice is measured, not assumed: `content-at-pin`, `never the top rung`, and `never evidence` each counted 0 occurrences on both files at the pre-change base and are the discriminating anchors; a bare `execution` or `history` key is never used alone (both pre-exist on the template — `execution` 21 times at the base, 8 of them the reserved lens token), only inside an ordered four-shape chain that pairs each ambiguous name with a base-absent one. The same test's supplementary loop pins the four rung-body tokens on the card alone, where by design they exclusively live among the guarded surfaces. **(3)** the D27 row in `skill-doc-contracts.test.mjs` extracts the SKILL.md bindings paragraph by construct and pins the ADR pointer, the six skeleton tokens, **and one distinctive anchor pair per binding** — the pairing is the point: all six skeleton tokens survive the deletion of any single binding, so a skeleton-only row would stay green on exactly the loss that matters. The new ADR 0041 records the D1–D5 decision tree, the two rejected alternatives (one total evidence order — wrong somewhere by rank inversion; a runtime per-finding schema field — waits on a consumer that does not exist), and the precedent-lesson mapping: each of the three motivating lessons maps to exactly one claim shape — all three to `content-at-pin` — two of them to the surface that shape's ladder forbids as a verdict basis, the third (the Bash-guard grep lesson) to misuse of the D2 default arm, which the ADR's own Consequences records as the least clean fit. **No engine control flow, exit code, status enum, floor script, or schema moved — a label covering those surfaces and nothing wider:** `workflow-template.js` *did* change, in its string-built dispatched-prompt text and adjacent comments (the skeleton appended at the four seats above); the AuditVerdict schema is byte-unchanged (the ladder is judgment guidance, deliberately schema-free); and beyond the two suites gaining the rows above, `war-config.test.mjs` changed only a region-extraction comment (the retired-token-sweep row's region now deliberately terminates at the *named* `**Post-servitor publication` lead-in so it absorbs the new intervening paragraph — zero assertion lines moved there).
