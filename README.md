...
**0.21.2** — Advisory Vale Markdown lint (`hooks/vale-md/`): a new `PostToolUse` hook on `Edit`/`Write` runs [Vale](https://vale.sh) over the edited file when its path ends in `.md`, using a self-contained profile beside the script — four `ReplyStandard` rules recast from the card: sentence length over 25 words, slop words, Latin abbreviations, and a dash rule shipped but disabled in the profile. When Vale reports findings the hook prints one `additionalContext` line naming the count and the top three rules, with a standing caution against rewording ratified, byte-pinned, or drift-guarded literals to satisfy a style rule. Advisory only: the script exits 0 on every path, and a machine without the `vale` binary or without `python3` silently no-ops (the `gate.sh` precedent). Reach: unlike the Reply Standard pair, `PostToolUse` fires inside subagents, so WAR workers editing Markdown see the advisory line too; the hook adds context only — it changes no gate, floor, or verdict. Toggle: a validated `hooks.valeMarkdown` key (`war-config.mjs` DEFAULTS + validation, default `true`, `null` = unset, documented on the `/war-room` override list and the schemas.md run-config reference) read fail-open by the hook script itself — only an explicit `false` disables. Beyond the hook registration, the `hooks/vale-md/` files (shim, script, profile, styles, test), the config key with its schema tests, the README, `/war-room`, and schemas doc lines, and the version slots, no engine, guard, or agent surface changed in this window.
