...
**0.14.32** — Four recorded correctness ceilings in the refiner-side merge floors are closed, each inside its existing 0/1/2 exit contract and discovery set. The **packaging floor** (`assert-packaging-in-diff.sh`) now pre-joins backslash continuations into logical lines inside `parse_dockerfile`, so a `COPY`/`ADD` whose sources wrap across a trailing `\` is no longer silently dropped — the exact Dockerfiles the floor polices no longer fail open; and its main scan loop is inverted to Dockerfile-outer/added-file-inner so each Dockerfile parses exactly once (was re-parsed once per added file), the single call site locked by a structural count assertion and the flagged-pair set proven unchanged via sorted comparison (#800). The **submodule floor**'s `--declared` mode (`assert-no-submodule-mutation.sh`) reads the branch ref's `.gitmodules` via a `cat-file -e` existence probe plus `git config --blob "$branch:.gitmodules"` instead of the working tree, so declared-path judgment can no longer diverge from the diff endpoints (absent blob → clean skip; read failure after a successful probe → exit 2) (#802). The **guard-specificity floor**'s `extract_msg()` is scoped to the emit segment between the `echo`/`printf` keyword and `>&2` — recording the true quoted message on stderr-emit guards — and `record_guard` now skips bare-`$var` messages and truncates `printf` format strings to their pre-`%` literal prefix, ending the perpetual mis-flag of bare-variable `die` messages that polluted the `guard_specificity` gate-audit evidence; the floor stays advisory-evidence-only (#803). An opt-in **`--advise-vacuous`** flag prints one stderr advisory line per ADR-0017-ratified vacuous packaging-floor shape (no Dockerfile; a non-empty diff with zero A/R/C paths), threaded into all three dispatched refiner packaging-floor invocations for a task that explicitly declares `requiresPackaging: true` — advisory surfacing only, the ratified no-op scope stands, and stdout plus every exit code is byte-identical without the flag (#819). ADR 0017 carries a dated addendum recording the advisory surface; no floor discovery set moved.
