...
**0.14.42** — Campaign state anchors at the main checkout (#918): the compaction-survival hook, the campaign-ledger CLI, and the war-campaign placement prose stop trusting the Lead's cwd, so ADR 0016's guarantee holds for a campaign Lead running from a session git worktree. `hooks/inject-campaign-state.sh` now resolves its scan root through the ratified `git rev-parse --path-format=absolute --git-common-dir` idiom (fail-open to today's cwd behavior on any git-probe failure), so a Lead in a session worktree re-injects the main checkout's active `CAMPAIGN-STATE.md`; a single stranded-state helper wired into all three silent no-inject exits emits a bounded warning when an *active* ledger sits under a worktree that `/aftermath` will reap, while all-landed stranded ledgers stay silent. `skills/war-campaign/assets/campaign-ledger.mjs` resolves a relative `--campaign` against the same main-checkout anchor (absolute paths pass through, a non-git cwd keeps today's behavior), and `init()` now throws an error naming the campaign dir on a dangling `campaigns` symlink instead of a bare ENOENT stack. `skills/war-campaign/SKILL.md` states the anchor rule (locked by `war-pipeline-structure.test.sh`), and `docs/adr/0016` carries a dated amendment superseding the 2026-07-03 "campaigns live under the session's project dir" assumption — session worktrees are disposable, campaign state must outlive any one session.
