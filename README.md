...
**0.14.23** — WAR's engine seams stop turning imperfect input or half-run state into raw `TypeError`s, silent no-ops, or dangerous manual recovery. `war-config.mjs` `validate()` gains a non-object guard for `overrides` mirroring the sibling `memory` block, and both sandbox args-parse sites reject a non-object payload — the template throws a named error routing to the existing `held:workflow-error` while the scaffold falls back to an empty object so its title refusal fires cleanly. A `pt` tagged prompt template checks each interpolated value for identity `=== undefined` at build time, throwing before spawn and naming the adjacent literal fragment, with zero false positives by construction since prose is never scanned. On the provisioning side, `inject-campaign-state.sh` reads campaign directories with an indexed array instead of space-splitting, so a path containing a space still selects the newest active campaign; `provision-worktrees.sh` gains a catalogued exit-code set, an explicit target-repo argument on `ensure-exclude`, and an opt-in `--reclaim-empty-orphan` two-proof self-heal that reclaims a provably empty, origin-absent orphan branch and otherwise keeps the unchanged fail-loud default. Stable `dispatchKind` discriminators replace label-prefix parsing across the provisioning dispatches, and a marker names a recognized auditor dead-code exemption. ADR 0034 and an ADR 0013 addendum ratify.
