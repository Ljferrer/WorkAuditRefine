...
**0.16.0** — WAR owns its front door: **one interview, one merged artifact, uniform on every authoring path — interactive and AFK** ([ADR 0044](docs/adr/0044-authoring-contract-and-merged-artifact.md)). The pipeline's execution artifact is now the **merged plan** — Part 1 the ratified decision record (evidence-tagged `## Context`, `## Pivotal constraints`, `## Resolved design tree`, one required `## Assumptions ledger`), Part 2 the decomposed phases — produced by **one interview run to completion**, so the interactive path has no separate spec interview and no spec→plan conversion seam: the seam whose defect classes dominate the findings a 57-finding classification of the ten newest red-team reports measured (~40% false repo facts, ~35% under-specification). The interview doctrine lives in-repo at [`skills/war-strategy/references/plan-interview.md`](skills/war-strategy/references/plan-interview.md) — silent recon and a rehearsal-plus-pre-mortem (carrying the delete-the-feature probe) through a private full-template draft, the question-contract interview, a coverage sweep with two echo-back gates, and the provenance + executor gates, with the decisive-slots table as the completion bar and the 1:N → roadmap rule for oversized work — and **bare `/war-strategy` runs it itself**, terminating only on a merged plan in `docs/plans/`, with no auxiliary skill installed. The Grill Me family is demoted from required to a **recommended front door** (the README section now reads Recommended Auxiliary Plugin): when the family is installed, `/war-strategy` offers that route and binds it to the same merged deliverable via its HANDOFF DIRECTIVE; when it isn't, the interview runs all the same — reversing the prior README stance, whose out-of-repo placement pointed at doctrine the installed stub demonstrably did not carry. The **authoring output contract is template law for new artifacts**: every claim of fact in Part 1 carries an evidence tag — `(user)` · `(verified: <source> at <base>)` · `[assumed: <default> — if wrong: <consequence>]`, issue-derived claims in the `(verified: issue #N (<date>))` source form, memory or training recall never a `(verified:)` source — mapped onto ADR 0007's provenance ladder in one CONTEXT.md glossary entry that deliberately reuses none of the ladder's tier names; `Done when: <command>` is required iff `requiresTest: true` and permitted (not required) elsewhere; spec-§10-style criteria and plan End states unify into one numbered End-state list, each carrying exactly one tag from the closed set `check:` | `gate:` | `HARD at audit_sha` | `backstop:` row; and one `## Assumptions ledger`, whose rows conversion and AFK synthesis carry forward or retire each with a stated reason. Enforcement stays **advisory lint plus red-team grading — nothing fails closed at authoring, and legacy artifacts are grandfathered**: `plan-literal-lint.mjs` gains the five merged-template shape rules (untagged End state, `requiresTest: true` without `Done when:`, missing ledger, untagged factual-claim shape in `## Context`, vague-trigger vocabulary in End states), each printing beside its hit the merged-template slot it checks, and still exits 0 without `--strict`. **Uniform reach:** `/war-machine` conversion emits the merged shape too — Part 1 a decision digest citing the source spec — and its grill charter retires the training-memory question-tree wording for `plan-interview.md`'s falsifier probes + provenance scan, with the charter's sandbox-execution duty (a behavioral claim about the repo is proven by executing it in a throwaway sandbox, never by analysis alone); `--afk` rows carry per-row `AI-declared` markers. `/red-team` Step 1 gains the **merged arm**: a plan whose Part 1 carries the decision record is **its own source of truth** — `--spec` defaults to the plan itself — and the coverage-vs-source lens reads Part-1→Part-2 coverage in both of its homes, the `references/lenses.md` spine entry and the scaffold's `SPINE` prompt; `references/backstop-legitimacy.md` gains the `judge:`-tag grading rule — for each `judge:`/`HARD at audit_sha` End state or backstop entry, grade "could this have been a `check:` command?", with commandable-but-judged routed `needsDecision` — the box-ticking mitigation. `/survey-corps` synthesis now tags every claim it makes (issue-derived facts in the D11 source form, inventions `[assumed:]`), so AFK invention is visible and vetoable at the spec intermediate that path keeps. The **extraction bound held — zero `/war` engine change**: no file under `skills/war/`, `hooks/`, or `agents/` changed; Part 1/Part 2 is prose framing, never heading nesting; Part 2 keeps today's exact H2 headings — both ADR 0014 intent headings as either/or alternatives, the two-H2 tail, per-task fields as separate `- ` bullets, an extraction requirement the red-team run proved by executing `extractFiles` against the compact alternatives — and the merged template ships two example documents (operator-form and AFK-form) exercising both arms; engine consumption of the new slots stays deferred to the precision-chain spec, and the sole `skills/` touch outside the skill families named above is a `campaign-ledger.test.mjs` test-title clarification naming the ratified bullet `Files:` shape — that test's assertions are unchanged. The **gospel is swept, both directions, with committed pins**: README, CLAUDE.md, war-help, CONTEXT.md (Plan and Spec redefined; Evidence tag, Assumptions ledger, Done-when, Decisive slot, and Executor gate (authoring) added), and war-strategy's own surface each land the new wording under case-insensitive structure-test enforcement — `war-pipeline-structure.test.sh` gains a `lacks_i()` case-insensitive absence helper (mirroring `lacks()` with `-i` and inheriting `strip_prose`'s `## Status`/`## Changelog` drop, which is why this very paragraph can describe the retired framing without re-tripping the guard) and pins every retired anchor old-absent on all five surfaces plus a new-present twin on each surface that suite owns, while `war-strategy-structure.test.sh` pins the ratified template internals present and the retired two-template/required-Grill-Me wording absent; pin scope stays exactly the five doc surfaces — never the suites' own source, `docs/plans/`, `docs/red-team/`, or `docs/adr/`, which legitimately quote the anchors. The first field trial is the implementing plan itself, authored in the merged shape one phase before the template ratified it; its red-team run terminated [`ADJUDICATED`](docs/red-team/2026-08-05-interview-and-authoring-contract.md) per ADR 0043 and fed two shape rulings back into the template as ratified law.
