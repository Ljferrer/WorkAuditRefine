...
**0.14.61** — Two drift guards now discriminate the defects they police, and two prose surfaces state what is actually enforced. The D18 doc-contract guard in `skills/war/assets/skill-doc-contracts.test.mjs` pairs each gate site with its **own** exhaustion route: for `merge site` → `held:escalation` and `land site` → `held:land-failed`, the FIRST `held:*` token following the site token in the extracted environment bullet must equal that site's route, so a bullet with the two exhaustion routes **swapped** now reds — the retired four-entry companion loop asserted each of the four tokens present *anywhere* in the bullet and greened the swap (#1040); site presence is subsumed, since a missing site token fails the paired match loudly. The #1046 floor-site discovery guard in `skills/war/assets/workflow-template.test.mjs` re-anchors `FLOOR_SITE_RE`'s capture terminator on the enclosing `pt` template literal's **own closing backtick** — a negated-backtick class that cannot cross into a neighbouring template — replacing a terminator coupled to the `requiresTest:false` skip arm's line shape, which swallowed a 5th site written inline into its neighbour (#1050); a new cross-check then asserts the discovered site count `===` the raw invocation-head count, **both sides derived from the same template source**, so a genuinely new dispatched site grows them together and no literal exists to rot. That is deliberately *not* an exact-count hardening: the ratified `>= 3` non-vacuity floor and every per-site token arm are retained byte-unchanged, and the rewritten header comment records the reconciliation beside all three residuals — the fail-closed inline-backtick truncation, the accepted head-shape escape, and the mid-file unenclosed head (spuriously discovered with a junk capture, so the red arrives from the per-site arms rather than the equality). The test-floor near-miss advisory in `skills/war/assets/assert-test-in-diff.sh` now names **both** possible causes — the pattern (`--pattern` / `overrides.testPattern`) **or** an excluded location, enumerating `node_modules/`, `.git/` and `.claude/` as the mirror of the gate's own discovery exclusions — where it previously asserted one specific wrong cause; for a diff whose only test-shaped file sits under an excluded prefix no `--pattern` value could ever satisfy the floor, and because that stderr line is carried verbatim into `MERGE_RESULT.floor_diagnostic` and on into the add-test fix-worker prompt — the verbatim-capture contract established by row **D6** of the *test-floor-target-repo* design and enforced by the #1046 drift guard above, itself unchanged — the misattribution reached a worker (#1049). A new case in `skills/war/assets/assert-test-in-diff.test.sh` drives an excluded-location fixture and pins all three prefixes **row-scoped to the extracted advisory row**, never whole-stderr — the near-miss listing prints each path verbatim, which makes a whole-stderr prefix grep vacuous — so deleting any one prefix from the advisory reds the case, and the exit-1 stdout contract stays byte-empty. `agents/war-auditor.md`'s `## Read-only git guard contract` intro now states that the contract is carried on both surfaces and edited together in one commit, and names the both-surfaces registry row in `skills/war/assets/workflow-template.test.mjs` — which anchors four shared tokens per surface — as the drift arbiter, instead of claiming a verbatim mirror that the two deliberately different formats never made true (#1080); a committed absence assertion in that registry test keeps the retired overclaim from silently returning. **No behavior change:** `skills/war/assets/workflow-template.js` is untouched, and no floor exit code, matcher, `near_miss()` arm, routing enum, dispatched prompt byte, or guard verb allowlist moved — this release ships two hardened test guards, one stderr advisory line, one standing-card sentence, one new test case, and one committed absence lock.
