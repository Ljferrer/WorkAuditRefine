...
Two one-time modes manage the two-root split itself rather than tidying within it:
...
**0.14.46** — Class-1 gate discipline in `/aftermath`: per-ref SHA probes, `git cherry` row evidence, and the stranded-upstream `-d` recovery (aftermath-class1-gate-evidence). **#926** and **#932** are memory-mined gaps in the Class-1 (stray WAR branches) procedure of `skills/aftermath/SKILL.md` — and in this plugin skill prose *is* the executable behavior (the Lead runs Class-1 by reading it), so a missing rule is a wrong procedure. Both share one root mechanism: the /war refiner rebases task branches locally in the serial merge queue and **never force-pushes**, so a task branch's remote ref keeps the worker's pre-rebase SHA forever while the local ref advances to the rebased tip that landed — **"stranded" is a property of the specific SHA probed, not of the branch** (the same branch reads STRANDED via its remote SHA and REACHABLE via its local SHA). Three prose encodings, none of them a new deletion path: ADR 0027's C3 paragraph stays **byte-unchanged** and the bar it states (tip-reachable + PR-merged) is never lowered — the cherry probe is evidence for an *acknowledgement*, and the unset-upstream recovery is *confounder-removal* that restores git's own merged-into-HEAD check. (1) The Class-1 evidence-gate cell names the per-ref rule **and the verb per action**: the gate is derived against the *exact ref being removed* — a remote delete (`git push origin --delete`) gates on the remote (`ls-remote`) SHA, a local delete (`git branch -d`) on the local SHA, never mixed. (2) The acknowledged-stranded bucket gains the *why* (the rebase + never-force-push stranding mechanism), the **population split** that resolves its two remote-delete sentences into two routes rather than a contradiction (gate-**passing** refs are deleted in-run under the default evidence-gated scope; gate-**failing** acknowledged refs are cleared only by the deliberate C3 manual push-delete outside the gates), the comparator rule (the freshly-fetched `origin/<working|landing>` ref is always the `merge-base --is-ancestor` comparator; the candidate side is the exact ref being removed), and the **`git cherry` probe** the tip-reachability gate cannot produce — argument order stated (landing ref first, candidate second), healthy shape on a gate-failing ref (non-empty, every line `-`-prefixed; zero `+` among ≥1 `-` lines ⇒ every patch already in the landing branch by patch-id), **empty output a suspect result, never a PASS**, and the negative arm as **NOT PROVEN — never proven-unmerged** (squashes and conflict-resolved rebases legitimately change patch-ids) ⇒ candidate stays needs-human, no row. A zero-`+` result is **evidence for a `known-stranded.tsv` row, never a deletion license**, and **adding the row is a reviewed operator commit, never an in-run Lead write** (tsv header and schema untouched). Probe hygiene is objects-only: fetch the landing ref first so the comparison matches `ls-remote` truth, and reach absent candidate objects with **`git fetch --refmap= origin <ref>`** — the empty `--refmap=` is load-bearing, not decoration (red-team-proven on git 2.50.1): a bare `git fetch origin <ref>` in a normal clone matches the command-line refspec against `remote.origin.fetch`, whose `+refs/heads/*:refs/remotes/origin/*` wildcard matches every branch, so it **does** create `refs/remotes/origin/<ref>`; only `--refmap=` leaves `FETCH_HEAD` alone with **zero refs created**. (3) A new **Class-1 local branches — the stranded-upstream `-d` refusal** subsection: with an upstream set `git branch -d` checks merged-into-**upstream**, and every WAR task branch tracks its own stranded pre-rebase remote tip, so the safe verb refuses on branches provably in master (reproduced on all 6 affected branches, 2026-07-15) while the SKILL named only the `--scorched-earth` `-D` escalation — no correct route existed in prose. The recovery, in order: the evidence gate passes **on the local SHA** (a task branch's PR-merged half is evidenced by the plan's landing PR — campaign ledger PR#, or the Class-4 `gh pr list --search` fallback; tip-reachability on the exact ref stays the load-bearing half) → `git branch --unset-upstream` → `git branch -d` from a checkout whose HEAD carries the landing content (stale HEAD ⇒ refusal noise, fail-closed, never a wrong delete) — `-d` post-unset independently re-verifies merged-into-HEAD, **git's own second opinion, kept precisely because `-D` would discard it**. Refusal taxonomy keyed on the strings git actually prints: `is not fully merged` ⇒ the genuine un-merged signal ⇒ needs-human; `used by worktree at` ⇒ a **worktree-ordering** signal (Teardown normally reaps worktrees by path before branch deletes, so a survivor *is* the gap) ⇒ branch and worktree as one needs-human row — **never `-D` in default mode** for either, and tracking is **restored** (`git branch -u origin/<ref>`) on every needs-human route after an unset, so a sweep leaves no mutated config on rows it reports and never touches. The one-sweep asymmetry is stated once, there: the local branch deletes via the safe `-d` while the remote ref it tracked stays acknowledged-stranded on the remote side. `CONTEXT.md` gains **patch-equivalence probe** and **stranded upstream**; `skills/war-machine/war-pipeline-structure.test.sh` gains one **named** criterion (no bare number — the original pipeline spec owns the numbering) whose four checks pin *tokens and the gate-cell row, not semantics*: `git cherry` and `--unset-upstream` presence, plus two **row-scoped** pipes anchored on the `| 1. Stray WAR branches |` row literal — never on `merge-base --is-ancestor`, which this file's unwrapped one-line-paragraph style also puts on the bucket's comparator line, where it would make the co-location check pass against a byte-unamended cell and leave the keep-green pin unfalsifiable — one keep-green pin that the pre-existing deletion bar still lives *in the cell*, one co-location check that the per-ref rule does too. `git branch -d` is a whole-file review floor only (the verb is named in both the cell and the subsection, so it discriminates nothing); `--unset-upstream` is the sole mechanical pin for the subsection. Prose truth beyond those four checks stays the grep-guard family's known ceiling; Class-1 has no test asset family (the ADR 0027 named residual), so first live execution is operator-verified.
