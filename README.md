...
**0.14.43** — Land-failure recovery and per-phase dispatch identity (land-failure-recovery): a phase whose `land:phase-<N>` dispatch dies on a transient API error no longer finishes `status: completed` with the DAG silently advancing past content that never landed. `skills/war/assets/workflow-template.js` gains a terminal `else` on the primary land routing chain that routes both a null land result and any unrouted status to the existing `held:land-failed` enum member — a reused member, no new enum anywhere: `land-decision.mjs` stays untouched, the emitted-superset stays at 6 values, and `held:workflow-error` still owns only the *thrown* case via the top-level catch (the new arm and the catch partition returned-but-unrouted vs thrown). The Lead runbook (`skills/war/SKILL.md`) and `skills/war/references/schemas.md` document root cause **(c) dead land agent** — with a step-0 already-landed probe so a dispatch that died *after* its CAS push is recorded, never re-merged — plus an anti-`resumeFromRunId` warning steering recovery away from a live journal replay that would re-run merges and the push-first CAS during the same overload window. New `skills/war/assets/stage-workflow.mjs` stamps per-phase dispatch identity: a replace-exactly-once, write-if-absent stager writes `war-[c<K>-]<planSlug>-p<N>.js` under `$MAIN/.claude/war/runs/<runId>/` with `meta.name`/`meta.description` substituted as pure literals, so each phase renders in the /workflows UI with an operator-meaningful title instead of the generic `workflow-template`; an existing staged copy is reused byte-untouched (approved injections and journal-replay script stability survive restaging), and `--force` restages deliberately. A new ADR records the run-scoped staged-phase-scripts contract and `CONTEXT.md` gains the **Staged phase script** and **Dead-agent land failure** glossary terms.
