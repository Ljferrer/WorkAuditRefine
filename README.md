...
**0.14.35** — /red-team probe-integrity hardening across three surfaces. The env-gap→warn rule (#807) is now enforced gate-side: a typed `envGap: true` finding flag is demoted to Minor in `classify()` **before** the severity branch, so a failed provision step can no longer be force-promoted to a blocking `needsDecision` and mis-scored as a broken plan; `dedupe()`'s malformed-finding fallback key gains a `probe` component so two probes' identical env-gap notes stay two findings, never silently collapsing to one. The `workflow-scaffold.js` CONTRACTS header now states a pass-probe Critical/Major is filtered from `blockers` yet stays in `allFindings()`, not "discarded as a non-defect" (#808). And `assert-no-repo-escape.sh`'s `die()` default exit is the conservative infra code 2, not the escape code 1, with a standing call-site lock requiring every `die` to pass an explicit exit code (#812).
