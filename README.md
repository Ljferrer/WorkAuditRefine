...
The command set, in the order you'll run it: **`/war-help`** orients you → **`/war-room`** configures a run → **`/war-strategy`** structures the spec and the plan (and converts spec → plan) → **`/red-team`** hardens the plan → **`/war`** executes it → **`/war-review`** tallies what the run cost and flags any friction. Scaling up: **`/survey-corps`** turns open issues into specs → **`/war-machine`** turns specs into plans + a roadmap → **`/war-campaign`** runs the plans back-to-back unattended → **`/aftermath`** cleans up the debris → **`/lessons-learned`** keeps the accumulated memory honest.
...
**0.14.28** — A `/war` run now leaves an inspectable cost-and-friction record. `/war` accumulates a fail-open run manifest at `.claude/war/runs/<runId>.json` — per-phase timestamps, workflow run id + transcript dir, dispatch counts by role, task terminal statuses, land decisions, lessons, and issues — written best-effort at phase boundaries (bookkeeping never blocks a run and is never resume input; the git > issues > ledger correctness order is untouched). The new **`/war-review`** skill reads the newest (or `--run`-pinned) manifest, mines the referenced workflow transcripts for tokens and tool calls, and renders the full telemetry set — workflows, sub-agents by role, tokens, wall-clock per phase, audit rounds, findings by severity, tasks by status — plus a friction section enumerating the signal classes (`held:*`, `env-blocked`, reland loops, roundLimit exhaustion, dropped agents, guard denials) with a **clean** / **friction found (N)** verdict; anything unsourceable renders `n/a`, never a fabricated number. On friction it drafts one issue, targets it via the installed plugin's own metadata, and files it only on explicit confirmation; `--scavenge` reconstructs pre-manifest runs best-effort. The engine stays closed — the manifest is Lead-side bookkeeping, and its runtime fields (run id, transcript dir) come from the Workflow launch envelope, not the return object.
