...
**0.14.55** — Adds post-batch residual verification for Class-1 remote deletes to the `/aftermath` sweep — Class-1-remote-batch-scoped doctrine the Lead follows by reading the skill, not a mechanized floor, gate, or committed test. After the batched `git push origin --delete`, the sweep re-lists the remote once (`git ls-remote --heads origin`) and diffs the survivors two ways against the retained pre-batch snapshot: every ref in the **hold set** — the exact-`refs/heads/<ref>`-name complement of the delete list — must still be present, and a missing one is a **data-loss row** that prints the snapshot SHA and the ready-to-run restore `git push origin <snapshot-sha>:refs/heads/<ref>` (run only behind an explicit confirm interactively, printed but never executed under `--afk`); every delete-list ref must be gone, and a survivor is a **failed-delete row**, reported and never silently re-batched. The run withholds its clean verdict until that two-sided diff is empty or every discrepancy is already a report row — a non-empty unreported diff reads *verification-failed*, never *clean-with-a-footnote*. A set-difference construction advisory additionally heads off the recorded near-miss that motivated this: a `$STRANDED` exclusion match evaluated inside a piped `while read` subshell that silently matches nothing, letting hold-meant refs flow into the batch while `git push` prints success. ADR 0027's C3 deletion bar is byte-unchanged — the verification only adds evidence, opens no new deletion path, and grows no autonomous ref-creation authority under `--afk`. Recorded with a `residual-set verification` glossary entry, prose drift-pins in the Class-1 gate-evidence block, and an in-place amendment to the origin lesson.
