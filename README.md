...
**0.14.33** — Two consciously-ratified confinement residuals (ADR 0002) are narrowed without changing the ratified posture. The worker's **advisory-only Bash-write detector** (`warn-bash-write-scope.sh`) now resolves a relative redirect target against the payload's `.cwd` and scans `python`/`perl`/`ruby`/`node` `-c`/`-e` payloads for absolute write-target tokens, so a relative `> out` or an inline `open('/outside/x','w')` from outside a `.war-task` worktree is surfaced on stderr — still exit 0, never a block; the non-redirect extractors stay absolute-only as a documented ceiling (#809). The **servitor write glob** in `validate-worktree-scope.sh` is now `$HOME`-anchored (trailing-slash-normalized, with a fail-open shape-glob fallback when `HOME` is unset or empty), and all six `agent_type` case arms across the four scope hooks are suffix-anchored so a decorated agent type falls through to the fail-open default rather than silently matching a deny-side arm (#810). The advisory-only posture and the cross-project-under-`$HOME` residual are re-ratified; ADR 0002 gains a dated addendum, and `guard-conventions.test.sh` gains standing regression cases so a future hook edit can't silently un-anchor an arm or the servitor glob.
