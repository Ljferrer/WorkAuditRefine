...
**0.14.37** — Four memory tool-chain frictions closed, restoring MEMORY.md projection headroom without touching promotion semantics, the redaction lint, or the budget constants. `buildProjection` now dedups a promoted lesson's cross-root twin: when a slug appears in both the local and repo roots the repo copy wins and the local twin is dropped from the projection input only — indexing, `query`, `inboundCiters`, and promotion still see both copies — so one fact no longer costs two rows against the single hard cap; the **deduped** set feeds `archiveCandidates`, whose comparator gains one root clause (equal tier → `local` before `repo` → oldest) so a shadowed twin is never nominated (#821, #820). A reviewed, lint-gated `docs/learnings/` description diet plus explicit-slug archives — moves into `archive/` with a note, never deletions — recover ≥ 4,000 B of headroom under the hard cap; the steady-state `warn` verdict above `WARN_BYTES` is the designed post-remediation state, not a failed diet (#820). `/survey-corps` now mandates **slug-only** lesson citations in issue bodies and titles for local-root lessons, so a mining citation can no longer self-trip the fail-closed redaction lint by resolving a home path (#822). And `/lessons-learned migrate`'s pre-flight branches on `test -f .claude/war/config.json`: an absent config resolves to effective defaults (`commitLearnings: false`) instead of failing the `--fill-defaults` resolver, fixing the most common state (#823).
