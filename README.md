...
**0.21.11** — preset re-tier: the three shipped presets in `war-config.mjs` move to new agent tiers. `balanced` (= `DEFAULTS`) runs fable workers at `default` effort on the base, docs and fix tiers, with opus auditors on `high`; refiner, servitor, red-team and snipe keep their prior values. `thorough` inherits those worker tiers and pins its own auditor, refiner, servitor, red-team and snipe seats. `economy` pins its own base and docs worker tiers and its auditor, refiner, servitor and red-team seats, inherits the fix tier and snipe, and drops its `rosterPolicy: solo` and `roundLimit: 2` pins, so it now inherits `auto` and the 6-round budget; its four-lens pool and `redteamRoundLimit: 2` stay pinned. The per-role values for all three presets live in the `/war-room` preset bullets, written in a `role model/`effort`` grammar. Root-cause change for the prose that restates those values (#2097): every restating surface now either carries a machine pin or a pointer to `war-config.mjs`. `war-config.test.mjs` parses the three `/war-room` bullets whole against `presetConfig()`, binds the README defaults sentences through `DOC_TIER_PINS`, bans a model literal from re-entering any de-mirrored construct through `DEMIRRORED`, and pins the `thorough` and `economy` agents literals whole; `version-slots.test.mjs` pins this `## Status` block and its CHANGELOG twin byte-equal. The two `workflow-template.js` sandbox literals and the two agent-card frontmatter `model:` lines move with `DEFAULTS` under their existing drift guards.

Release scope: the window from the `master` head (`c913fc9`, PR #2065 and #2077 merged, still 0.21.10) to this release, bucketed whole. Config authority: `war-config.mjs`. Engine: `workflow-template.js` (two sandbox literals and their comments, no routing change). Agent cards: `agents/war-worker.md` and `agents/war-auditor.md` (the frontmatter `model:` lines). Docs: `README.md`, `CHANGELOG.md`, `CONTEXT.md`, `skills/war/SKILL.md`, `skills/war-room/SKILL.md`, `skills/snipe/SKILL.md`, `schemas.md`, `design.md`, `gastown-design-params.md`. Tests: `war-config.test.mjs`, `workflow-template.test.mjs`, `version-slots.test.mjs`. Slots: `plugin.json`, `marketplace.json`. No guard, hook, floor script, shell suite, or `docs/learnings/` file changed.
