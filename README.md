...
**0.14.54** — Hardens the `/lessons-learned tighten` eviction ladder. `effectiveDate` now validates the date tokens it scrapes from lesson prose: a token is kept only if it round-trips through a UTC parse (an invalid-shaped stamp such as `2026-13-45` is dropped) and lands no more than 48 h past the current clock (a stray far-future token such as `2099-01-01` no longer counts). The fail-safe direction is preserved — when **no** token survives, the lesson reads **undated** and stays protected from eviction, exactly as before. Separately, `tighten-plan`'s returned `verdict` is now the **stricter** of the advisory line and the effective `--target`: the preflight can now bind at a stricter `--target` (a sub-advisory target makes the pass trigger there), while a looser target never suppresses the advisory `warn` — it still moves only the pass's exit target, never the trigger. `buildProjection` is byte-untouched, and `render-index` and `archive --candidates` semantics are unchanged. Both origin lessons are amended in place with dated MITIGATED (#989) / RESOLVED (#992) notes.
