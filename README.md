...
**0.14.69** — Prompt-surface budgets and the hot/cold law (ADR 0042): the memory subsystem's proven size governance — advisory line, hard refusal, temperature-is-location — is ported to the prompt-bearing prose surfaces (which, unlike memory, cost context-window tokens every time a window loads them), together with the one-time shrink pass the new budgets ratchet against. **The governance:** a new standing suite, `skills/war/assets/prompt-surface-budgets.test.mjs`, carries one budget row per budgeted surface — every `agents/*.md` card via a live readdir census (default-deny: a new card without a budget row reds the census, and removing a row is a loud `deepEqual` failure, never silent), four fixed non-agent surfaces (`skills/war/SKILL.md`, `skills/lessons-learned/SKILL.md`, `CONTEXT.md`, `CLAUDE.md`), and the prompt-literal share of `workflow-template.js`, measured by a pinned nesting-aware extraction of top-level template literals ≥ 200 B so engine-code growth never trips a prose budget (the spec's draft 164,234 B / 238-block literal-share row was unreproducible — ~3× the file's total template-literal content — and is superseded by the pinned algorithm; the extraction is non-vacuous: zero extracted blocks reds, and a desynced scan fails closed). Each row reds **when** the surface's byte size exceeds its hard constant, and logs a warning — a log line, never a failure — **when** the size crosses its advisory constant. Formula (red-team adjudication D): hard = post-shrink size × 1.25 rounded up to the KB, advisory = post-shrink × 1.10 ceil-KB; every constant carries its derivation comment (measured bytes @ base). Ratchet-down semantics: lowering a constant is a normal PR; raising any hard or advisory constant requires citing ADR 0042's justification rule in the commit body. New doctrine now defaults to cold storage — the hot/cold law: only tier-1 (every-invocation) doctrine stays inline; anything rarer becomes a `when <trigger>, read references/<file>` pointer, and the trigger is the skeleton — a pointer without a trigger is a defect. Eviction is a byte-identical move into `references/` (`references/` files are deliberately unbudgeted — cold storage, the memory `archive/` analogue — and `README.md` is deliberately unbudgeted too, a human release surface); compression was allowed only for provable cross-surface redundancy, each such commit naming the surviving canonical copy. **The measured shrink,** plan base (`0dc004a`) → post-shrink (`c6a05fb` — the measurements the landed constants derive from, per their own comments): `skills/war/SKILL.md` 96,608 → 58,555 B (60.6% of the plan-base measurement, under End state 5's ≤ 65% ceiling); `agents/war-refiner.md` 31,957 → 27,109 B; `agents/war-worker.md` 10,664 → 9,199 B; `skills/lessons-learned/SKILL.md` 38,711 → 32,923 B; `agents/war-auditor.md` 23,823 → 22,216 B; the `workflow-template.js` prompt-literal share 52,636 → 49,864 B under the pinned extraction; `agents/war-servitor.md` 15,550 → 15,531 B; `agents/war-setup-scout.md` — no shrink task targeted it — ratchets to its own measured 8,989 B with no flag (unchanged is not failed). Two surfaces grew across the plan: `CLAUDE.md` 12,400 → 12,987 B, whose computed ceil-KB budgets land exactly on its Phase-1 placeholder values (no flag), and `CONTEXT.md` 100,984 → 101,769 B, whose computed hard would exceed its placeholder — ratchet-down only, so the placeholder pair (126,976 / 111,616 B) is retained and the growth was raised as a blocking done-report flag for Lead adjudication, never a silent note. Guards followed their text (ADR 0025): every doc-contract extraction region whose pinned prose moved in this plan re-anchored in the same task as the move — save one whole-file UNION key the plan's own file-disjointness law forced to the phase-6 close: `war-config.test.mjs`'s `RETIRED_CLAIM_SURFACES` widened over `skills/lessons-learned/references/tighten.md` at the phase-close sweep (the first tip where both tasks had merged), because that suite sat in same-wave Task 6.2's Files list while Task 6.1 performed the eviction — a presence key by relocating its read to the destination file, an OLD-absent / whole-file key as a UNION scan over origin plus every destination, never a relocated read — and the dispatched copies of the auditor calibration / doc-cascade / committed-tree-grounding clauses and the servitor admission disciplines are now token skeletons whose full texts live on the standing cards (`agents/war-auditor.md`, `agents/war-servitor.md`), with every registry anchor token those suites pin kept in place on both surfaces. A new `D29` row in `skill-doc-contracts.test.mjs` binds the doctrine's hand-synced mirror set — ADR 0042 against `CONTEXT.md`'s three glossary terms (**surface budget**, **prose temperature**, **trigger pointer**), `CLAUDE.md`'s `## Doctrine placement` summary, and the budget suite's header formula comment (the ×1.10/×1.25 numeric pair's only mirror block) — every key token-anchored and asserted on BOTH its mirror block and the ADR, with direction-paired ratchet keys (lowering↔normal-PR, raising↔commit-body) so a one-sided edit or a direction swap reds while sanctioned rewording latitude does not. **No engine control flow, exit code, status enum, schema, or floor script changed — a label covering exactly those surfaces:** this release moved prose and re-anchored tests; `workflow-template.js` itself changed only inside its string-built dispatched-prompt literals and their adjacent comments (verified against the plan base: every changed line of that file across this plan sits in a template literal or a comment).
