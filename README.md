...
- `vale-md.py` (`PostToolUse` on `Edit`/`Write`, harness-filtered to `.md` paths by per-handler `if` rules so non-Markdown edits never spawn a process) runs [Vale](https://vale.sh) over the edited file when its path ends in `.md`, using the self-contained profile beside the script (`.vale.ini` + `styles/ReplyStandard/` — no remote packages, no network, no `vale sync`). When Vale reports findings, the hook returns one `additionalContext` line naming the count and the top three rules. It never blocks, and the line itself cautions the agent never to reword ratified, byte-pinned, or drift-guarded literals to satisfy a style rule.
- The shipped rules are the Reply Standard card's, recast for docs: `SentenceLength` (over 25 words), `SlopWords`, `LatinAbbrev`, and `Dashes` (shipped but disabled in the profile — em dashes are this repo's own house style).
...
**Optional Google profile:** `{"hooks": {"valeGoogle": true}}` swaps the hook to `.vale-google.ini` — the house rules plus the Google developer-documentation style, vendored under `styles/Google/` ([errata-ai/Google](https://github.com/errata-ai/Google), MIT) and tuned: fourteen Google rules that judged entrenched repo voice in the prototype sweep are disabled (parens, em dash, semicolons, acronyms, and US spelling among them — the card mandates British, so `Google.Spelling` is off), and `Google.Latin` yields to the house `LatinAbbrev` twin. Vendoring rather than `vale sync` keeps lint time offline and makes rule updates deliberate commits. Subordinate to `valeMarkdown`: when that is `false`, nothing runs.
