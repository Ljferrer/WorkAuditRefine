...
Three more modes go beyond the routine pass above. `migrate` and `evict` are one-time moves that manage the two-root split itself; `tighten` is repeatable and manages the projection's size instead:
...
**0.14.53** — A test-floor `no-test` caused by a *pattern* mismatch now says so on the first round instead of after three add-test rounds. On its **exit-1 path only**, `assert-test-in-diff.sh` re-scans the same changed-file list against a fixed, documented set of test-shaped conventions (`*.test.*`, `*.spec.*`, `*_test.*`, plus basename-prefix `test_*`) and, **when that scan finds anything**, prints a **stderr** block naming the **active** pattern set — the custom `--pattern` tokens when supplied, else the two defaults printed from the existing pattern variables rather than re-stated as a third literal, plus the note that the unconditional `*.test.sh` union arm is always in force — followed by each near-miss path. The near-miss set is fixed and documented, never derived from the tree at floor time (that derivation stays rejected — the verdict must not depend on tree state outside the diff), and the scan is diagnostic-only: it can never block, change an exit code, or write to stdout. The byte-empty stdout summary the refiner reads is untouched, the 0/1/2 exit contract is unchanged (exit 2 still never collapses into `no-test`), and there is no scan on exit 0 or on the exit-2 die path. The refiner captures that stderr **verbatim** into the new optional `MergeResult.floor_diagnostic` at **every** dispatched merge-task floor site, instructed on both prompt surfaces — the template's dispatched prompts and `agents/war-refiner.md` step 4 — and the Workflow threads it into the ADD_TEST fix-worker prompt (quoted verbatim, with the instruction to reconcile the diff's test files against the active pattern and **report blocked naming the mismatch rather than adding a duplicate test**) and onto the `no-test` exhaustion escalation as `detail` on both the `escalated` entry and the `no-test:exhausted` auditLog entry. The field is fail-open advisory and **never routed on**: absent ⇒ every consumer is byte-/shape-identical to today. Alongside it, `overrides.testPattern` becomes **per-phase-resolved**: under `--afk`, a Setup proposal the sanity floor rejected is recorded in the ledger **verbatim** with its zero-match tokens, and the Lead **re-checks the Setup proposal per phase** — every token matching ≥ 1 file in the current tree **and** the proposal re-passing the config validator's charset/shape check adopts it **monotonically** (`null` → the Setup proposal, once, for that and every subsequent phase, with an adoption ledger note, never a fresh interactive ask); a validator-failing proposal is **closed** rather than re-checked, since a bad proposal string cannot self-heal as the tree grows. No `MergeResult` status value, `HARD_ESCALATION_REASONS` member, or `KNOWN_LAND_DECISIONS` member is added or changed — `land-decision.mjs` and both hand-mirrored enum blocks are byte-untouched (ADR 0005) — and the plugin-scoped `null` default stays byte-identical (ADR 0019). Guarded by the extended standalone testPattern both-surfaces drift-guard, which **discovers** each dispatched floor site from the template source and asserts the capture tokens per discovered site over a ≥ 3 non-vacuity floor rather than an exact site count that rots under stacking, plus new `assert-test-in-diff.test.sh` fixture cases and `workflow-template.test.mjs` coverage of the schema lock, the fixPrompt pair, and the exhaustion `detail`. Recorded with two new glossary terms (**near-miss diagnostic**, **pending testPattern proposal**) and a `floor_diagnostic` contract entry in `schemas.md`.
