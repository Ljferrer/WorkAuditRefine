...
**0.21.5** — `war-memory archive` now fails closed on an occupied `archive/` destination (#1924). `fs.renameSync` replaces its destination, and the repo arm's `git mv` (no `-f`) refuses and falls through to that same rename, so a slug already present in `archive/` had its archived copy deleted while the ordinary `archived <slug> → <dst>` success line still printed. That turned the pipeline's stated safe operation ("knowledge is archived, never deleted") into a silent delete, reachable whenever a lesson is re-captured after an earlier archive — a sanctioned shape, stamped `metadata.supersedes_repo_copy`. The guard tests the destination before the note append, so a refused slug keeps its hot file byte-intact: the batch skips that slug, names both paths on stderr, archives the rest, re-renders, then exits 1 with a count, so a caller running `archive` over dozens of slugs is not left reconciling file counts by hand. Four new tests cover the local root, the repo root's `git mv` arm, the untouched-hot-file guarantee, and the non-zero exit alongside a successful sibling move. No other engine, guard, or agent surface changed in this window.
