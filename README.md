...
**0.14.14** — Fail-loud ingest boundaries: two pipeline CLIs now refuse to falsely succeed on an unexpected input shape. `campaign init --roadmap` parses the ratified plan-index table (the first markdown-link `.md` target per row; bare bulleted/numbered lists kept for back-compat) and **throws on a 0-plan parse before any write**, so a mis-parsed roadmap can no longer seed an empty campaign at exit 0 — the throw names both accepted forms and the unaccepted bulleted-markdown-link form. The red-team gate gains an exported `unwrapEnvelope` that auto-unwraps the Workflow task-output envelope (one `.result` level; a top-level `probeResults` always wins) and an **unconditional zero-probe refusal** (exit 1, stderr diagnostic, no verdict on stdout) — closing the false-`CLEARED`/`probes: 0` fail-open where a verification that never ran reported a pass. Both fixes ship file-disjoint with fixture-truth tests; every exported gate function stays byte-untouched; `skills/red-team/SKILL.md` step 4 documents both accepted input shapes, the one-level unwrap, and the refusal.
