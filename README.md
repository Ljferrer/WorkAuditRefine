...
**0.21.3** — Optional Google profile for the vale-md hook: the Google developer-documentation style ([errata-ai/Google](https://github.com/errata-ai/Google), MIT) is now vendored under `hooks/vale-md/styles/Google/` — committed rather than `vale sync`-fetched, so lint time needs no network and rule updates are deliberate commits — and a second profile, `.vale-google.ini`, layers it on top of the house ReplyStandard rules, tuned: fourteen Google rules that judged entrenched repo voice in the 0.21.2 prototype sweep are disabled in the profile (parens, em/en dash, semicolons, acronyms, and US spelling among them — the Reply Standard card mandates British, so `Google.Spelling` is off), and `Google.Latin` yields to the house `LatinAbbrev` twin. Selection is a new validated `hooks.valeGoogle` key (`war-config.mjs` DEFAULTS + validation, default `false`, `null` = unset, subordinate to `hooks.valeMarkdown` — when that is `false` nothing runs): only an explicit `true` swaps vale-md.py's `--config` from `.vale.ini` to `.vale-google.ini`. The hook's advisory-only contract is unchanged: it still exits 0 on every path and blocks nothing. Beyond the vendored style directory, the new profile, the vale-md.py profile selector with three new test cases, the config key with its schema tests, the README, `/war-room`, and schemas doc lines, and the version slots, no engine, guard, or agent surface changed in this window.
