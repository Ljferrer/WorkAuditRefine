...
**0.14.20** — Stack-fragile literals die at authoring time and auditors stop re-litigating correct-but-stale-looking patterns. The `/war-strategy` §2 plan template gains a "reference the live artifact, never a stack-fragile literal" convention block — construct locators over line-range refs, `resolveGate` by name over suite enumerations and counts, "append to the canonical export" over restated array literals, dotted paths over flat-key abbreviations, "next free patch above the live base" over version literals, the "defined-but-not-yet-emitted; produced in Task N" annotation, and the grep-as-floor manual-survey rule — each locked verbatim by `war-strategy-structure.test.sh`. A new advisory `plan-literal-lint.mjs` (report-and-exit-0, `--strict` opt-in, never a CI gate) mechanically flags those cheap literals at conversion and in `/red-team`'s spine. `/war-machine` gains a next-free-patch release-drafting directive with no hardcoded version target. Four stale-looking-but-correct calibration rules land in `agents/war-auditor.md` and mirror into `auditPrompt()`, each gated on live-artifact confirmation, with a both-surfaces drift test. `/red-team`'s `executable-proof` probe now extracts and runs plan-authored `requiresTest:false` grep guards, and `/survey-corps` carries the grep-floor rule from the first authoring surface. ADR 0030 ratifies the conventions, the calibration floor, and the lint's advisory placement.
