...
**0.14.10** — Target-repo-agnostic execution: the test-floor pattern, docker-gate composition, gate-failure verdict, and provision base each now derive from the target repo and fall back to today's exact behavior when derivation is impossible, so an external campaign no longer burns full spend on an unsatisfiable floor, a doomed docker segment, a pre-existing red gate, or a stale phase base. A per-run `overrides.testPattern` threads end-to-end like `plan.gate` (never parsed from the gate command), and the floor always unions the gate's unconditional `*.test.sh` discovery so a shell suite always satisfies it. Setup probe-builds each discovered Dockerfile under a bounded timeout and auto-defers a platform-mismatched image to an auto-sourced backstop; every gate failure is classified by an on-failure re-run at its base — introduced, baseline, or environment — routing recovery without touching any status enum (environment soft-escalates with zero fix rounds, baseline proceeds with recorded debt, an absent class keeps today's routing as the permanent fail-safe). And `cmd_ensure_integration` fetches origin before cutting a phase base, resolving equal/behind/ahead/diverged — a true divergence halts loud with both SHAs and repair guidance, never a silent pick.
