...
**0.21.12** — emergency patch (#2099): the staged per-phase script is comment-stripped and size-floored, so a `/war` launch dispatches again. The shipped `workflow-template.js` reached 525,209 bytes at 0.21.11, over the Workflow tool's 524,288-byte `scriptPath` cap, so every 0.21.11 `/war` launch was refused at dispatch, before entry validation, with or without `--args`. `stage-workflow.mjs` now blanks every full-line `//` comment that sits in code state (never a line inside a string, template literal, regex literal or block comment; each blanked line becomes an empty line, so line numbers still match the shipped template) before the anchor substitutions and the `--args` prelude, and refuses with a named error, writing nothing, when the assembled staged copy would still exceed the cap. Write-if-absent gained the same floor: a pre-existing staged file over the cap (a 0.21.11-era stage) is refused with an error naming `--force` and left byte-untouched, never reused into the tool's own refusal. Nothing is opt-in: the strip runs inside the stager every launch already calls, with no new flag. The shipped template is unchanged (its comments and every drift guard over them stay); the strip lives only in the staged copy. Measured at this release: a staged copy of the shipped template is 274,663 bytes, 249,625 bytes under the cap (the measured over-size args class is ~104.5 KB). `stage-workflow.test.mjs` pins the strip on a fixture carrying each construct the scanner must not touch and on the shipped template (line count preserved, only `//`-led lines blanked, `node --check` green, the fallback-free interpolation census unchanged, at least 128 KiB of headroom under the cap), the cap refusal on an over-size fixture and on a stale over-cap staged file, and the live 0.21.11 shape: the shipped template plus a ~104.5 KB `--args` payload stages under the cap. `skills/war/SKILL.md`'s staging paragraph, ADR 0037 and `CONTEXT.md` record the strip and the floor.

Release scope: the window from the 0.21.11 `master` head (`6be322d`, PR #2081 merged) to this release, bucketed whole. Stager: `stage-workflow.mjs`. Tests: `stage-workflow.test.mjs`. Docs: `README.md`, `CHANGELOG.md`, `skills/war/SKILL.md`, `docs/adr/0037-run-scoped-staged-phase-scripts.md`, `CONTEXT.md`. Slots: `plugin.json`, `marketplace.json`. No change to `workflow-template.js`, any agent card, guard, hook, floor script, shell suite, or `docs/learnings/` file.
