...
The three presets move the whole profile at once: **`balanced`** *is* the defaults, **`thorough`** swaps in its own five deep lenses on fable auditor seats at session effort, lifts the refiner to `xhigh` and the servitor to opus on `high`, and puts red-team and snipe seats on fable, and **`economy`** puts sonnet on the auditor, refiner, servitor and red-team seats (auditors on `xhigh`, the rest on `high`) behind opus first-pass workers and the inherited fable fix tier, over its own four-lens pool, with roster policy and the 6-round budget inherited, while `run.ace` stays inherited on (no preset pins the ace ladder; it gates only the per-task ace ladder).
...
**0.21.11** — preset re-tier: the three shipped presets in `war-config.mjs` move to new agent tiers, and the named mirror surfaces below move with them. `balanced` (= `DEFAULTS`) runs fable workers at `default` effort on the base, docs and fix tiers, with opus auditors on `high`; refiner (sonnet/`high`), servitor (sonnet/`xhigh`), red-team (opus/`high`) and snipe (opus/`high`) keep their prior values. `thorough` inherits those worker tiers, runs fable auditors at `default` effort, sonnet refiner on `xhigh` (unchanged), opus servitor on `high`, and pins fable at `default` effort on both red-team and snipe. `economy` runs opus workers at `default` effort on base and docs, inherits the fable fix tier, sonnet auditors on `xhigh`, sonnet refiner and servitor on `high`, sonnet red-team at `default` effort, and drops its `rosterPolicy: solo` and `roundLimit: 2` pins, so it now inherits `auto` and the 6-round budget; its four-lens pool and `redteamRoundLimit: 2` stay pinned. Mirrors updated in this window: the `ROLE_MODEL` and `WORKER_TIER_DEFAULTS` sandbox literals in `workflow-template.js`, the `agents/war-worker.md` (fable) and `agents/war-auditor.md` (opus) frontmatter, the `/war-room` preset bullets and override rows, the README default-config paragraphs and roles table, the `skills/war/SKILL.md` defaults bullet and docs-tier step, `schemas.md`, `design.md` (sections 2 and 8), `gastown-design-params.md`'s role table, and the `CONTEXT.md` Docs tier and Fix bump terms. `war-config.test.mjs` and `workflow-template.test.mjs` pin the new values; the retired economy solo and roundLimit-2 assertions become inherit assertions.

Release scope: the window from the 0.21.10 head (`2b72e65`) to this release, bucketed whole. Config authority: `war-config.mjs`. Engine: `workflow-template.js` (two sandbox literals, no routing change). Agent cards: the two frontmatter `model:` lines. Docs: `README.md`, `CHANGELOG.md`, `CONTEXT.md`, `skills/war/SKILL.md`, `skills/war-room/SKILL.md`, `schemas.md`, `design.md`, `gastown-design-params.md`. Tests: `war-config.test.mjs` and `workflow-template.test.mjs`. No guard, hook, floor script, shell suite, or `docs/learnings/` file changed. The version slots and this blurb's CHANGELOG twin are part of the same window.
