...
**What "today's defaults" actually are.** With no config file WAR runs the built-in `DEFAULTS`: opus workers on `default` effort, sonnet auditors on `max`, `rosterPolicy: auto` (the Lead composes each task's roster), the pre-merge ace-fix on, and a 6-round fix budget (`run.roundLimit`).
...
The three presets move the whole profile at once: **`balanced`** *is* the defaults, **`thorough`** swaps in its own five deep lenses on opus auditor seats, lifts worker and auditor to `max` effort and refiner and red-team to `xhigh` (servitor alone moves to opus at `default` effort), and pumps tokens, and **`economy`** pins the cheaper knobs it always had — sonnet on every top-level role, a solo roster, and a 2-round budget — while `run.ace` stays inherited on (no preset pins the ace ladder; it gates only the per-task ace ladder).
...
**0.21.10** — in-band absorb default (plan `2026-09-03-in-band-absorb-default`): every fixable bug a seat finds in a run is fixed in that run, and the engine files an issue only on a seat-cited barrier tag, a logged engine demotion, or a logged floor-skip on a task whose diff probe failed. Nine mechanisms land.
