...
**0.14.49** — Auditor guard ergonomics: the read-only git guard's grammar now matches the auditor's idiomatic toolkit, and the guard contract is taught up front instead of re-learned by denial (auditor-guard-ergonomics; **additive — grammar only, the fail-closed posture is unmoved**). `hooks/validate-auditor-git.sh` admits `~` and `%` into its fail-closed character allowlist (the idiomatic `HEAD~1` and `--pretty=format:%H` read forms; quotes, globs, braces, `$`, parens, pipes, and redirects still deny) and **admits two read verbs** — `ls-tree`, plus `branch` in **read forms only**: every token must be an enumerated `=`-attached read flag (`--contains=<rev>`, `--merged=<rev>`, `--points-at=<rev>`, `--list`, `-a`, `-r`, `--show-current`, `-v`), so a bare name, a write flag, a combined short cluster, or a space-form value denies **by token shape** — default-deny, without enumerating the write flags. `git grep` **stays denied**: the Grep tool is the sweep channel for repo-wide search (ADR 0029 conserved), and the deny strings now micro-teach that sanctioned alternative in place of a bare refusal. The contract itself is taught on **both** auditor prompt surfaces — a `## Read-only git guard contract` section in `agents/war-auditor.md` and the dispatched `auditPrompt()` in `workflow-template.js` — pinned by a both-surfaces drift-registry row that also asserts the retired partial teach is absent from both. And `held:workflow-error` now **self-diagnoses**: the args-parse string arm re-throws a named error carrying the payload length, the engine's own parse message (position only when the engine supplies one — never fabricated), and a bounded head snippet, while every `workflowError` gains an **additive `recovery` field** naming the sanctioned retry — a fresh Recovery relaunch with a new `runId`, never `resumeFromRunId`. No new enum member (ADR 0005): `land-decision.mjs` and its hand-mirrored copies are byte-untouched. `version-slots.test.mjs` keeps all four version slots in lock-step.
