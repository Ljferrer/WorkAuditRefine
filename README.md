...
**0.15.0** — The three-question minimalism sweep (PR #1236): every `skills/*/SKILL.md` audited unit by unit (315 units across 11 files) against three questions — would a fresh model already do this, does it correct a weakness that no longer exists, does it conflict with anything else — with every cut and eviction adversarially verified against the drift-guard suites, the ADRs, and the `docs/learnings/` incident lessons before landing. Hot skill prose drops 29,228 → 27,638 words; seven doctrine blocks move cold, byte-identical, into six new `references/` files behind ADR 0042 trigger pointers — red-team’s backstop-legitimacy check, lessons-learned’s migrate pre-flight (into the existing `references/migration.md`) and its interruption-recovery table, war-campaign’s add-resolution protocol, war-machine’s `--afk` conversion block, and war-review’s offer-issue and scavenge sections — with `doc-cli-consistency.test.mjs`’s `EVICTION_DESTINATIONS` list extended over every destination in the same commit, closing the pre-existing `migration.md` gap while there. Nine verified cuts each name their surviving canonical copy in the commit body (ADR 0042 D3), alongside the approved cross-skill rationale dedups (main-checkout anchor, gh-preflight, `audit.autoEscalate`); one cut — red-team’s adjudication-channel tail — is operator-ratified (2026-08-02), reversing End state 6 of the audit-adjudication-threading plan (#1014). Five truth drifts fixed: war-room’s submodule guidance no longer says hand-edit-or-wait-for-Increment-2 (it landed — the warning now routes to `/war`’s submodule router), the `/war` Invariants fix-tier clause now matches `war-config.mjs` `DEFAULTS` (`fable`/`high`, not `opus`/`max`), lessons-learned Phase 8 re-runs `render-index` instead of hand-adding a `MEMORY.md` row, this README now names both `--erwin` gates, and war-strategy §5 describes `/survey-corps` as the memories + issues → specs step. Three surfaces were certified already-minimal and left byte-untouched beyond the above: survey-corps (its lone candidate trim no-opped as load-bearing), war (one-clause truth fix only), and war-help. **No engine, hook, floor script, schema, or status enum changed:** the release diff touches only `skills/` prose and its moved `references/` files, the three test re-anchors the moves mandated (`war-pipeline-structure.test.sh` criterion-10 pins, `lessons-learned-doc-contract.test.mjs` needles (8) and (12), the `EVICTION_DESTINATIONS` extension), and `README.md`.
