...
**What "today's defaults" actually are.** With no config file WAR runs the built-in `DEFAULTS`: fable workers on `default` effort (base, docs and fix tiers alike), opus auditors on `high`, `rosterPolicy: auto` (the Lead composes each task's roster), the pre-merge ace-fix on, and a 6-round fix budget (`run.roundLimit`). `default` effort is not the lowest effort: the seat inherits the effort knob of the chat session that launched the run, and a pinned effort overrides that knob for the one seat. Presets order every seat by model capability, fable seats stay on `default` by design, and the rule is recorded in [ADR 0050](docs/adr/0050-preset-tiers-order-by-model-capability.md).
...
The three presets move the whole profile at once: **`balanced`** *is* the defaults, **`thorough`** swaps in its own five deep lenses and pins its own review-side tiers (auditor, refiner, servitor, red-team, snipe) over inherited worker tiers, and **`economy`** pins every tier but the fix worker and snipe and shortens its fix budget (`roundLimit: 4`) over its own four-lens widening pool, with roster policy inherited — the per-role values for all three are the `/war-room` preset bullets, printed by `node skills/war/assets/war-config.mjs --preset <name>` — while `run.ace` stays inherited on (no preset pins the ace ladder; it gates only the per-task ace ladder).
...
**0.21.13** — verdict integrity: every seat finding is normalized before it is routed, a refiner-reported land, merge-gate or pin-transfer status whose own fields contradict it is refused, a split panel runs its rebuttal round first and then a fix round only when every surviving blocker has a concrete `suggested_fix`, and every dispatch death is tagged at its site and never reads as a content verdict. Verdict intake: `normalizeFinding` runs on every finding at every verdict-ingestion site (the roster seats, the rebuttal-successor re-audit, the ace re-audits, the gate-audit family and the endstate seat, each wrapped by `normalizeSeat`): it strips an auditor-supplied `seats` or `merged` field (a seat never corroborates itself and same-seat rows never collapse) and normalizes `file` through `aceRelPath`; its caller `normalizeSeat` demotes a finding with no title and no routable content (rationale, suggested_fix, ask.question) to a logged note, sparing a `scopeBreach`, `plan_ref` or bare Minor/Nit `disposition:'ask'` row; `remintKey` folds a content hash in when file and title are both absent, so two content-distinct empty-key findings both file; the auditor card and every auditor prompt build carry the `FINDING-PATH FORM` sentence. Ask records: a parked-ask collision's surviving record carries `corroborators[].file/title/fork` and the handoff `asks` projection includes `corroborators`; the internal ask key lives in a side `WeakMap`, so the top-level `asks[]` carries no `key`; a coordinate-less ruled-ask record refuses with `missing required planSlug coordinate` before the own-token scan. Land path: the pin-transfer consumer refuses an `already_upstream` whose fields contradict it (`rebased_tip` equal to `dispatch_base`, a non-empty post-rebase patch-id, or an empty `already_upstream_commits`) and routes by patch-ids instead; the #1973 probe replays verbatim as a fixture and the task merges. One `segmentedLand` helper serves the initial, `environment-proceed` and `baseline-proceed` land dispatches: continuation requires `status:'error'` with `land_segment:'incomplete'` under `run.roundLimit`, normal replies receive independent Git confirmation after continuation; marker-absent or exhausted errors enter bounded recovery before routing. Preflight prevents unaccounted local-only target history from piggybacking on a task push, and independent pin proof verifies approved content, actual patch IDs and already-upstream Git evidence. `budget-uncited` joins `HARD_ESCALATION_REASONS` in both hand-mirrored copies and every drift guard, so a budget-uncited floor exit at either re-merge site escalates under its own name. Gate segments: a per-task merge-task may background its gate (`run_in_background`) and return `gate_segment:'incomplete'`, which re-dispatches under the same bound instead of routing `error`; every gate-log artifact opens with `tip_sha:` and closes with `exit_code:`, and on re-dispatch a log whose first line is not `tip_sha:` of the gated sha or whose last line is not `exit_code:` is rerun from scratch, never read as a partial or stale result; an unthreaded `gate_log_path` renders an explicit missing-evidence marker and never falls back to a conventional path. Provision barrier: derive-and-skip requires integrated ancestry and a nonempty task-owned commit with the exact `WAR-Task: <task branch>` trailer; sibling-only, empty-tagged and untagged legacy branches dispatch work/audit instead of being recorded merged; a worktree-dialect `staleRemote` row classifies `env-blocked` with a logged drop; the provenance floor's foreign-plan-id scan runs over every row, a string adjudication row strips its `supersedes` path before the match, the canonical `schemas.md` adjudication shapes launch un-doped, and a slug deriving zero own-tokens falls back to the plan basename with a log; the publication-worktree die names the branch holder the way the refinery die does. Citations: membership is directional (an exact match, or the threaded row contains the cited text, with a 24-character floor); the matched row is what `aced.citation.row` carries; `recordAced` requires a unique key match before an `--afk` splice, so a title-coincident ask is never spliced; the refusal logs once per phase across waves; the sweep-path aced record carries its citation. Re-entry and the ace ladder: a re-entry batch's own approving re-audit births a third ace and never files it, `demote()` consults `filedKeys` before pushing to `minorsFiled`, `aceLabel` takes a per-site segment (`ace:subset:`, `ace:reentry:`, `ace:polish:`) so each `KEEP_GREEN_SITES` row matches exactly one site, and a fileless regress reaches blind-halving. Sweep and filing: the filing prompt row and `handoff.followUps[].drainCause` carry the drain cause; the discard-sweep arm stamps `floorSkipped` with a log; a discarded polish branch's audited findings become follow-ups tagged `demote:sweep-discarded`; the integrated-tip and end-state ask arms park with synthetic seat labels; one `dropDup` helper owns both duplicate-drop sites. Endstate attestation: `intake_lint:` and `cmd_bytes_mismatch:` are `unverified` triggers on the dispatched END-STATE CHECK build, the auditor card and `CONTEXT.md` (two record-only states, neither ever `unmet`), and the `.cmd` runner records one `cmd[i] exit:` line per statement (a statement boundary is `;` or a newline, an `&&` or `||` list is one statement whose status is the shell's own) with the artifact's `exit_code` the maximum over the statement statuses (operator ruling 2026-09-07). Audit boundary: on a split the one rebuttal round runs first; a survivor set in which every blocker has a concrete `suggested_fix` dispatches a fix round plus a full-roster re-audit at the new SHA under `run.roundLimit`, and approval stays unanimous on the post-fix `audit_sha`; a blocking seat that withdraws at the rebuttal approves the task with no fix round; a fix-less blocker, unchanged survivor or finding-less dissent escalates with its `escalate_reason` in `escalated[]`; a mechanical blocking finding with budget never escalates at round 0; an unresolved mandate conflict parks a seat-conflict ask while the phase holds pending a ruling and re-audit; fixable survivors take the bounded repair path, and ordinary lexical-scope prose cannot erase a blocker. The dispatch cost is stated, not hidden: per split task the worst case is one rebuttal seat plus `run.roundLimit` × (one fix worker plus the full roster re-audited), and `run.maxParallel` bounds how many run at once, never how many run. Release-slot eligibility is by literal, not by file: a `README.md` or `CHANGELOG.md` absorb rides the per-task ace when no version literal, CHANGELOG head heading or Status token moves, `plugin.json` and `marketplace.json` stay refused by basename, the ace prompt on a release task cites `version-slots.test.mjs` as the merge guard, and both sweep-family prompts say `never move a version literal`. Dispatch classification: `dispatchAgent`'s body is the one `await dispatch(` site in the engine, `dispatchSite` tags a post-spawn death per site, and each site classifies its own: a wave roster seat death classifies `env-died` soft before the shortfall check, an ace or re-audit seat death demotes the current subset with the existing abandon reason, a floor-fix, pin-transfer, endstate, gate-audit, wrap-up or filing death keeps its site classification. Uncertain task/polish/terminal merge and land outcomes enter bounded Git reconciliation on the separately configured `agents.refiner.recovery` tier (default Opus/high); recoverable state completes in-phase without human Git, while unresolved uncertainty holds before land. An already-published phase commit is reused only with exact source-parent and ancestry proof, and a provision-barrier death is tagged but stays `held:workflow-error`; a dead dispatch never reads as `audit-blocked` or `done-unmet`. Doctrine and config: `agents/war-refiner.md` evicts its tier-2 recovery and budget-raise prose byte-identical to `refiner-recovery.md` and `budget-raise-floor.md` (ADR 0042), remaining below its 36,864 B hard ceiling; `run.maxParallel: null` is accepted as unset; the new `skills/war/references/fix-round-doctrine.md` carries the ten fixer rules, mirrored byte-equal into the FIX_NEEDED, Ace-Subset and re-entry builds, with a trigger pointer on the worker card and on `/snipe` (which also carries the two-consecutive-all-approve loop bound); `run-manifest.md` gains the relaunch arm (`attempts[]`, `workflowRunId` and `transcriptDir` overwritten together), `/war-review` checks the transcript basename against `workflowRunId` first, the handoff `followUps` literal is `{ issue, reason, merged?, drainCause? }`, the ask-ruling overrule clause is ratify-or-record-dissent, and `CONTEXT.md`'s Clean-handoff row emits on `landed` and `held:escalation` only. The new [ADR 0051](docs/adr/0051-verdict-intake-normalization-and-fail-closed-refiner-enums.md) records that the engine owns the intake and enum invariants and the prompt is advisory; ADR 0013 is edited in place for the two-sided blocking boundary and eligibility by literal; ADRs 0012, 0024, 0035, 0041 and 0049 carry dated amendments or cross-references.

Release scope: the window from the 0.21.12 `master` head (`ba08a77f`, PR #2100 merged) to this release, bucketed whole. Engine: `workflow-template.js`. Config: `war-config.mjs`. Land decision: `land-decision.mjs`. Provision script: `provision-worktrees.sh` (the worktree-add die names the holder). Agent cards: `war-auditor.md`, `war-refiner.md`, `war-worker.md`. Skills: the `war`, `war-room`, `war-review` and `snipe` cards. References: `budget-raise-floor.md`, `design.md`, `disposition-eligibility.md`, `file-followups.md`, the new `fix-round-doctrine.md`, `refiner-recovery.md`, `resume-and-recovery.md`, `run-manifest.md`, `schemas.md`. Docs: `README.md`, `CHANGELOG.md`, `CLAUDE.md` (the floor-route enumeration gains `budget-uncited`; the pointer line is untouched), `CONTEXT.md`, the new ADR 0051 and the six edited ADRs named above, three plans (this plan patched in-run, and the two 2026-08-25 sibling plans edited in place under the living-plans ruling), and the new red-team report. Tests: `workflow-template.test.mjs`, `land-decision.test.mjs`, `war-config.test.mjs`, `reference-link-integrity.test.mjs`, `skill-doc-contracts.test.mjs`, `doc-cli-consistency.test.mjs`, `provision-worktrees.test.sh`. Lessons: `docs/learnings/` gains 24 new files across this window, edits five (two of them under `archive/`) and archives two more into `archive/`. Slots: `plugin.json`, `marketplace.json`. No file under `hooks/` changed, no floor script changed, and no shell suite other than `provision-worktrees.test.sh` changed.
