...
**0.14.36** — Three fidelity fixes on the land/provision shell path and its verification layer, with zero behavior change on the land path itself. `cmd_ensure_origin` no longer swallows git's push stderr behind a static guess: the push is wrapped in the file's own `_tmp_err` mktemp idiom (already used by `cmd_ensure_integration`'s fetch and both branch-create sites), so a failed push keeps the never-force guidance text **and** appends the captured real-git stderr — the `git push -u origin` refspec itself stays byte-identical (#801). The D9 enum-leak detector's `phase-token-in-status-equality` pattern no longer false-trips on equality-form MergeResult narration like `mr.status === 'landed'`: pattern 1 now captures an optional dotted receiver anchored on a non-dotted boundary and drops a hit only when the receiver is in a new `NARRATION_RECEIVERS` allowlist (`mr`, `mergeResult`), so a bare `status ===`, an unlisted receiver, and a chained path like `result.mr.status` all still flag (#813). And `cmd_land_advance`'s CLASSIFY exit-3-without-`[rejected]` push-error branch gains a dedicated exerciser — case **T2.9**, a pre-receive-hook rejection whose `ls-remote` succeeds but whose push is declined without the contiguous `[rejected]` token — pinning execution to the silent bare-`exit 3` route (#814).
