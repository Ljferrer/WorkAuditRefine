...
**0.14.45** — Per-phase retired-token sweep: a Lead-run backstop that catches stale command recipes in memory lessons when a phase retires a land/merge/escalation mechanism (learnings-recipe-drift-sweep). Lesson bodies under `docs/learnings/` carry hand-authored *operational recipes* — literal git commands a Lead is told to run — and unlike hand-mirrored code constants, **no drift-guard test, floor, or grep-parity check binds lesson prose**: when a phase retires, renames, or consolidates a mechanism, the stale recipe survives silently until a human happens to reread it. **#930** (memory-mined from `process-recipe-lesson-body-is-not-drift-guarded-by-any-test`) mechanizes what that lesson's own "How to apply" paragraph could only describe as manual process. New Lead-run prose step in `skills/war/SKILL.md` under `## Per phase (in DAG order)`: at each landed phase close — **including a phase the Lead completed manually** via the §4.3 escalation-completion recipe (held phases get no record line) — judge **from the landed phase diff** (mandatory input, so retirements emerging from ace commits or fix rounds are caught even though no plan prose names them), informed by the plan slice, whether the phase retired, renamed, or consolidated a land, merge, or escalation mechanism. Deliberately **not plan-declared** — a plan field would reopen the gap whenever the plan forgets, the exact failure mode being fixed. On a trigger, two nets run over the **hot** corpus only: a literal grep of both memory roots — the repo root read at the **just-landed tip** (`git grep … origin/<working>`, never the possibly-lagging session checkout), plus the local root — stated as a **completeness floor, not a ceiling**, then the fully-flagged ranked `war-memory query … --local … --repo …` (both flags load-bearing: the CLI has no cwd fallback, so a flagless query silently walks an *empty corpus*), archive rows discarded, then a bounded hand-scan. Each hit is adjudicated **load-bearing** (recipe-position, and the recipe as written is no longer the sanctioned path) or **exempt** (narration-position, or still-live-in-context — a token retired for one procedure can stay legitimate in another), which is why this can never be a fail-closed gate, floor, or hook: a hard grep would flag exactly the lessons that were correctly rewritten. Repo-root load-bearing hits file **one** dedup-checked consolidated `war-followup` issue inside the existing preflighted per-phase gh-write batch; local-root hits contribute **counts only** to every gh-mirrored surface, with slugs confined to the uncommitted ledger notes. Every landed phase carries a `retired-token sweep:` record line (`n/a` when untriggered). The sweep **never blocks or holds a land**, runs identically under `--afk`, and is not the ADR 0012 phase-close coherence sweep. `war-config.test.mjs` anchors the clause's trigger family, `war-followup` routing, and record line within the clause region — prose truth beyond those anchors stays the doc-contract family's known ceiling.
