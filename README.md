...
**0.14.50** — Audit adjudication threading: a ruling the Lead has already made no longer costs a phase — decompose-gate and escalation-time scope adjudications now reach every audit seat over the **existing** `args.adjudications` channel (audit-adjudication-threading; **additive — this widens the producers of an existing channel, not a new adjudication system**). The `/war` Lead is instructed at the two moments it was previously silent: a new decompose-gate step assembles the run's adjudication set from the red-team report's `## Adjudications` rows **and** from every scope finding the Lead adjudicates at that gate, threads it as `args.adjudications` into every per-phase Workflow, and records each row in the ledger; the held-partial-phase recovery runbook records the escalation ruling at the moment it is made and re-threads the **full accumulated set** — from the ledger record, alongside `args.recovery` — so a relaunched seat is never adjudication-blind. `adjudicationClause` gains one producer-agnostic **ADJUDICATION-MATCH** sentence: a finding whose substance matches an adjudicated row is a confirmation note citing that row, never an escalation, while a candidate deviating from **both** the plan and the row is judged normally. The clause is now also emitted at the **three gate-audit-family seats** (per-task `execution-evidence`, integrated-tip, end-state-only) immediately after their `intentClause`, so a gate-time ruling reaches the gate-audit pass and not only the work-wave roster seats. Everything under it is unmoved: the version-precedence sentence is **byte-identical**, `adjRow`'s string arm is reused untouched (no new interpolations, the prompt-tag census is unchanged), and empty/absent adjudications still resolve the clause to `''` — every prompt byte-identical to today. **No new D3 registry row** and the `REGISTRY.length` floor is untouched (only the registry's rationale comment is corrected, now that this one clause reaches the inline gate-audit seats directly), and **no new enum member** — `land-decision.mjs` and its hand-mirrored copies are byte-unchanged (ADR 0005). Docs follow the widening: `schemas.md` documents the channel's shape, both producers, run-long accumulation and recovery re-threading; `/red-team` step 5, `CONTEXT.md`'s **Adjudication** term and an ADR 0013 addendum name the second producer, and `references/lenses.md` widens only its destination phrase. The new rule is mirrored verbatim on the standing auditor card, locked by the extended both-surfaces test pair, alongside a scoped absence test that no sole-producer phrasing survives.
