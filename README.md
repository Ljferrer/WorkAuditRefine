...
**0.14.48** — `/lessons-learned tighten`: a bounded memory projection plus an operator-gated, usage-scored eviction pass (lessons-learned-tighten; **additive — no existing mode, format extractor, or behavior removed**). `MEMORY.md` is now a **two-column router** — `buildProjection` drops the phase column and caps each summary cell at `SUMMARY_CELL_BYTES` (160 B), truncating the description text **first** and appending the provenance/`[repo]` tags **second** so the trailing `[tier]`/`[repo]` markers the safe-swap repo-completeness gate and row classifiers key on are never cut; depth of recall moves entirely to the FTS query path, and the over-advisory render WARN now names its remedy. A new **read-only** `war-memory tighten-plan` verb (`--local` / `--repo` / `--target`, default 17,000 B) walks the corpus, counts per-slug query-log hits with per-entry dedupe, and emits a JSON eviction plan behind three hard floors — never `user-confirmed`, never a ≥2-inbound hub (`inboundCiters`), never a lesson whose effective date is within 14 days (undated ⇒ protected) — ranking eligible rows by ascending hits, tie-broken by tier then age, with cross-root dupes reported as both-copies-or-nothing units (a shadowed twin's single-copy `bytesFreed` is 0). The `/lessons-learned tighten` mode wraps that verb in a **preflight → plan → single strike-list gate → execute → report** shape (local archives via the staged `safe-swap.sh` swap; repo archives on a `dev/<date>-memory-tighten` branch by reviewed PR), with a **loud shortfall block** — bytes missing + next candidates — whenever strikes leave the file above target; Phase 0 and the render WARN both suggest it. `CONTEXT.md` gains **Tighten pass**, **Cell budget**, **Usage-scored eviction**, and **Advisory line** and sharpens **Index projection**; ADR 0038 records the index projection as a bounded two-column view. `version-slots.test.mjs` keeps all four version slots in lock-step.
