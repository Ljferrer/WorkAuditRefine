...
**0.14.52** — A gate failure the refiner has already classified as `environment` — a positive proof, since the classifier re-ran the same tip green in a fresh environment moments earlier — now earns one bounded recovery attempt instead of having that proof discarded. Both gate sites dispatch exactly **one** `environment-proceed` re-run — `merge:<taskId>:environment-proceed` and `land:phase-<id>:environment-proceed`, mirroring the existing `baseline-proceed` dispatch shape in a fresh TMPDIR/shell — and it carries the opposite gate discipline to that sibling: the re-run's gate must come back **fully green**, never a proceed-over, with no debt recorded and no failing-id carve-out. The bound is structural — one per gate site, never chained with another recovery dispatch — not a config knob. Exhaustion routes by site: at the merge site it **hard**-escalates on the existing reused `'escalate'` reason, so the phase holds rather than completing without an approved task whose merge merely flaked; at the land site it falls back to today's `held:land-failed`, with the retry provably spent. No `MergeResult` status and no `HARD_ESCALATION_REASONS` / `KNOWN_LAND_DECISIONS` member is added — both hand-mirrored enum copies stay byte-untouched, every route reuses an existing member (ADR 0005), and the `held:phase-incomplete` intentional gap stays ratified (ADR 0025). Alongside it, `land-advance` gains a pre-push precheck and now **refuses when `HEAD` is not the merge sha**: a self-explaining die on the catalogued `EX_WRONG_BRANCH` (exit 6, no new constant) naming both SHAs and the expected cwd, pushing and writing nothing. It sits after the land-truth guard's early-return arms on purpose, so the already-landed follower reconciliation stays idempotent from any cwd (ADR 0008). Exit-2 semantics are unchanged — and now unambiguous: a rejected push means only a real concurrent advance. Recorded in the new **ADR 0040**, an append-only **ADR 0023** amendment (the original Decision untouched), an `environment-proceed` glossary term, a new **D18** doc-contract row binding `SKILL.md`'s `environment` routing arm to the bounded mechanics, and an **ADR 0036** Decision 1 reworded to the gate-bearing-dispatch-site invariant rather than a member count that rots.
