...
**0.14.19** — Memory-store integrity moves from remembered discipline into the tooling. `war-memory` gains `inbound <slug>` — the mechanical inbound-`[[link]]` count across both roots (excluding the slug's own file) that the Phase-3 hub-check prose grep becomes — and `archive --candidates` is now non-destructive by default: it lists the ranked candidate set and mutates nothing, so archiving requires an explicit `--apply` or a slug list. Archiving a slug with two or more hot inbound refs emits an advisory concept-hub WARN to stderr and still exits 0 (archive is link-safe — cold links resolve). `safe-swap.sh verify`'s archive-aware dangling-link check is frozen by test as the sole authority on link removal, and the fan-out verifier prompt gains the HOT / COLD / MISSING trichotomy — a hot-only `ls` can no longer produce a removal verdict. The servitor's D3 re-confirms an audit-log-sourced finding against the landed tip before recording — matched becomes `code-verified` with a locate cue, unmatched (fixed in-flight) falls back to the generic pattern at `agent-unverified`. Auditor verify-and-close / already-done no-op claims ground on `git show <audit_sha>:<path>` (working-tree grep advisory only); the auditor git allowlist is unwidened. Every dispatched-prompt clause lands with its `agents/*.md` mirror, drift-guards pinning both surfaces. New ADRs 0028 (memory-store integrity, tool-enforced) and 0029 (capture grounds on the committed tip) ratify the two halves.
