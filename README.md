...
**0.14.47** — Comment/prose truth for the campaign-state-anchor idiom, plus a hook-suite hermeticity guard (campaign-anchor-comment-truth; **zero behavior change**). The `git rev-parse --path-format=absolute --git-common-dir` main-checkout anchor (landed in `hooks/inject-campaign-state.sh`, `resolveCampaignDir` in `skills/war-campaign/assets/campaign-ledger.mjs`, and recorded in ADR 0016) carried comments that wrongly enumerated a **bare** repo among the probe-**failure** cases that leave the caller's root untouched. That is false: the probe **succeeds** inside a bare repo (exit 0, printing the bare repo's own git dir — live-proved), so the anchor reassigns the root to the bare git dir's parent. Today's consumers degrade harmlessly (no `.claude/campaigns` markers there — fail-open one level down), but a future caller copying the idiom where a bare-dir resolution carries the expected marker would inherit a real mis-anchor. **#927** states each site's true contract (comment-only diffs — the hook's `common=` probe line, all of `resolveCampaignDir`'s logic, `is_active`, and ADR 0016's ratified decision are byte-unchanged; ADR 0016 is corrected by an **appended dated amendment**, its 2026-07-15 amendment left intact and only the Status line repointed). **#928** closes the sibling gap: the hook now git-probes **every** root it is handed, so `hooks/inject-campaign-state.test.sh`'s hermeticity (its `mktemp -d` workspace landing outside any repo) was an accident nothing asserted — an enclosing repo would silently re-root the scan and fail injection-path cases far from their cause. A **fatal setup guard** built from the hook's own two-step probe now runs immediately after the workspace `cd`, before case 1, aborting on any probe success and naming the workspace and the resolved git dir — making the hermeticity a structural guarantee (a probe **failure**, git-absent included, *is* hermeticity, and it auto-covers the bare-ancestor case `--is-inside-work-tree` cannot flag). Both cited memory lessons gain dated body notes; no existing test assertion changed.
