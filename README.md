...
**0.14.26** — WAR can now resume a half-finished phase instead of re-running it, and its destructive branch proofs fail loud on a git error rather than guessing. When a held or partially-merged phase relaunches, the provision barrier derives the set of tasks already merged into the integration branch and skips re-dispatching them — adopting the landed work through an `args.recovery`/`preMerged` handoff instead of redoing it. A `STALE_REMOTE` follower marker at that barrier is now classified as `env-blocked` so the run continues, in place of the blanket fail-loud-on-any-nonzero default; the carve-out is mirrored across the dispatched barrier prompt, the `war-refiner` standing card, and the worker sentence in the same commit. Held and escalated returns carry a `defectClass` token on the top-level `escalated` field, kept out of the canonical `HARD_ESCALATION_REASONS` per ADR 0005. The `--reclaim-empty-orphan` self-heal's first proof now captures a git command error instead of reading it as branch-absent, so a reclaim never deletes a branch on an unproven origin state. A held-partial-phase recovery runbook and sync-follower recipe join `skills/war/SKILL.md`, four recovery domain terms join `CONTEXT.md`, and ADR 0035 ratifies.
