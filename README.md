...
**0.14.7** — Doc-rot remediation: audited surfaces restored to code-truth and made rot-resistant. The spec-of-record and operator docs had drifted from what the code provably does, so this round re-grounds them and adds guards so they cannot silently drift again. `design.md`'s header loses its stale version literal and its per-role model prose now defers to `war-config.mjs` DEFAULTS as the authority (worker = opus/max); a new **drift-guard test pins every agent frontmatter `model:` to those DEFAULTS**, and `agents/war-worker.md` is corrected to match. The `war-config` CLI usage string documents the implemented `--resolve-gate` verb, and `resolveGate`'s `*.test.sh` discovery now **excludes `.claude/`** (a repo-root run no longer executes ~100 stale duplicate suites), with the merge-path floor's mirrored pattern verified aligned. ADR-0011 is marked **accepted** to match shipped operating doctrine, lessons-learned's Node < 24 wording is truthed (every verb exits non-zero; callers fail open), and hooks hygiene drops a dead variable and consolidates the two `Bash` matcher blocks into one. The clean-audit-series roadmap gains the shared `-roadmap` suffix with all inbound links updated and zero broken references repo-wide.
