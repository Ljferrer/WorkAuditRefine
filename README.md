...
**0.14.12** — Run-lifecycle robustness: WAR now fails in the *right class* at its three seams — dispatch, relaunch, launch. The refiner's standing card names the `provision` mode it already performs (ADR 0001 keeps the dispatch route), all three provision dispatches return the shared env-outcome shape with every result captured, and `env-blocked` is gated on **execution evidence** — a failed command matching a dispatched provision step plus a numeric non-zero exit code; anything else (a refusal, a foreign command, an incoherent zero exit) throws to `held:workflow-error`, and the old fabricated no-result escalation is deleted. Derived task and polish worktree paths are now **phase-scoped** (`p<phase>-<task>` / `p<phase>-polish`, mirroring the task branch), so a same-run cross-phase relaunch can never collide with a stale worktree; `_refinery` stays run-scoped by design. A single entry-validation check at args parse enumerates the missing launch trio (`planSlug`/`runId`/`worktreeRoot`) and, distinctly, a missing `phase.id` — dispatching zero agents on a violation. The **recovery-relaunch** playbook ships as Lead prose with two entry points (single-task retry; whole-phase relaunch for `held:workflow-error` and retries-exhausted `held:phase-incomplete`) — fresh run, new runId, same plan slug and numeric phase id, owned-file continuity, never `resumeFromRunId`. Every dispatched-prompt change lands with its `agents/war-refiner.md` mirror; ADR 0021 and CONTEXT.md ratify the run-lifecycle contract, the polish-path rename, and the run-scoped-vs-phase-scoped naming convention.
