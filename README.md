...
- `vale-md.py` (`PostToolUse` on `Edit`/`Write`) runs [Vale](https://vale.sh) over the edited file when its path ends in `.md`, using the self-contained profile beside the script (`.vale.ini` + `styles/ReplyStandard/` — no remote packages, no network, no `vale sync`). When Vale reports findings, the hook returns one `additionalContext` line naming the count and the top three rules. It never blocks, and the line itself cautions the agent never to reword ratified, byte-pinned, or drift-guarded literals to satisfy a style rule.
