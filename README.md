...
**0.14.34** — `campaign-ledger.mjs init --roadmap` now ingests only a roadmap's **plan-index table** — the document's first table, read code-fence-blind so a fenced example table can't stand in for it — and reads each plan's footprint from **backticked `Files:` paths**, falling back to comma-separated path-shaped tokens only when a `Files:` block carries no backtick at all (any backtick present keeps extraction backtick-precise); block-scoping stops the `Files:` block at the next list item so a following `- Plan slice:` no longer bleeds its prose backticks in (#857). The producer side is pinned so the two can't silently drift: the war-strategy §2 plan template states the backticked-`Files:` contract (structure-locked), a new advisory `bare-files-path` lint runs in the `/war-machine` drafter step, war-machine §3 mandates plan-index-table-first (locked), and both fail-loud throws now self-diagnose — the empty-ingest throw naming first-table-only extraction and the `--plans` escape hatch, the unparseable-footprint throw naming the backtick-and-comma authoring rule (#858).
