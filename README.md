...
**0.14.39** — Engine-owned gate composition (#894), plus three coupled prose/doc-truth corrections the move re-baselines. The WAR engine now composes the self-discovering `*.test.sh` gate loop idempotently at a single point — `resolveGate(plan.gate)` runs right after entry validation in `workflow-template.js`, so a Lead who skips the `--resolve-gate` Setup pre-resolution can no longer dispatch a shell-blind gate to any of the nine gate-bearing sites; the pre-resolution stays as the belt, the engine is the suspenders (ADR 0036). `resolveGate` detects its own discovery-clause token, so double composition is a no-op, and the hand-mirrored inline copy in `workflow-template.js` is drift-guarded by its D2 registry row. Riding along: the Setup "Daemon reachable" docker bullet now names the **refiner** — not `classOf` — as the performer of the classification-base gate re-run, with `classOf` a pure reader of the refiner-computed `gate_failure_class`, locked by new spec-truth guard rows in `skill-doc-contracts.test.mjs` (#887); the entry-validation phase-field check is documented as defensive fail-fast on both mirrored surfaces (`workflow-template.js` and `schemas.md`), dropping the false "even a zero-task phase builds the Provision-barrier prompt" justification (#892); and both #815 code-literal bullets in the landed 2026-07-12 audit-gate-evidence-fidelity plan now match the non-backtick land-cwd form the shipped count test actually carries (#893).
