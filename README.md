...
**0.21.12** — emergency patch (#2099): the staged per-phase script is comment-stripped and size-floored, so a `/war` launch dispatches again. The shipped `workflow-template.js` reached 525,209 bytes at 0.21.11, over the Workflow tool's 524,288-byte `scriptPath` cap, so every 0.21.11 launch that staged a fresh copy was refused at dispatch, before entry validation, with or without `--args` (a resume that reused an under-cap file staged earlier was not). `stage-workflow.mjs` now blanks every full-line `//` comment that sits in code state (never a line inside a string, template literal, regex literal or block comment, and never a trailing comment; one known scanner residual is recorded in the reference) before the anchor substitutions and the `--args` prelude; each blanked line becomes an empty line, so the line count is preserved and an `--args` stage adds only its two prelude lines after `meta`. When the assembled copy would still exceed the cap, the stager refuses with a named error that reports the stripped-template and args sizes separately, and writes nothing. Write-if-absent gained the same floor: a pre-existing staged file over the cap is refused with an error naming `--force`, never reused, and left byte-untouched; the refusal also says that a restage forfeits any injected stage and byte identity with a recorded journal. Nothing is opt-in: the strip runs inside the stager every launch already calls, with no new flag. The shipped template is unchanged; the strip lives only in the staged copy. Measured at this release: a comment-stripped copy of the shipped template is 274,663 bytes, 249,625 bytes under the cap (the measured over-size args class is ~104.5 KB). `assert-args-complete.mjs` gains `ptSpanRanges`, the offset-and-expression view of its existing `pt` span scanner. `stage-workflow.test.mjs` pins the strip on a fixture with one or more lines per scanner arm (each arm proven red under deletion), on the shipped template through three oracles that do not share the scanner (only `//`-led lines blank; no blanked line inside a `pt` span's prose, nested spans included, via `ptSpanRanges`; `node --check` plus an unchanged interpolation census with counts), a 128 KiB headroom floor, the cap boundary (exactly the cap stages, one byte over refuses), an over-cap `--args` payload, the stale over-cap staged file, and the live 0.21.11 shape (the shipped template plus a ~104.5 KB payload stages under the cap); arm (r) binds every prose restatement of the cap in `CONTEXT.md`, ADR 0037 and the new reference to `SCRIPT_BYTE_CAP`. Doctrine: `skills/war/SKILL.md`'s staging step evicts its resume, restage and stage-failure sentences byte-identical to the new `skills/war/references/staged-script.md` (ADR 0042; the same file carries the comment-strip section) behind one trigger pointer, which also lifts the card's headroom under its hard line from 258 B at 0.21.11 to 927 B; ADR 0037 decisions 2, 3 and 6 each carry a dated amendment; `CONTEXT.md`'s staged-script entry records the strip and the reuse floor.

Release scope: the window from the 0.21.11 `master` head (`6be322d`, PR #2081 merged) to this release, bucketed whole. Stager: `stage-workflow.mjs`. Preflight: `assert-args-complete.mjs` (a new export, no behavior change to its CLI). Tests: `stage-workflow.test.mjs`, `assert-args-complete.test.mjs`, `reference-link-integrity.test.mjs` and `doc-cli-consistency.test.mjs` (registry rows for the new reference). Docs: `README.md`, `CHANGELOG.md`, `skills/war/SKILL.md`, the new `skills/war/references/staged-script.md`, `docs/adr/0037-run-scoped-staged-phase-scripts.md`, `CONTEXT.md`, `skills/lessons-learned/references/seeding.md`. Lesson: the new `docs/learnings/fix-the-finding-class-not-the-site-and-prove-every-new-guard-red-before-commit.md` (the fix discipline this patch's review round used), also packed into the portable seed corpus (`docs/seed/`, 41 members). Slots: `plugin.json`, `marketplace.json`. No change to `workflow-template.js`, any agent card, guard, hook, floor script, or shell suite.
