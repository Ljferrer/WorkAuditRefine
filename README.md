...
**0.14.62** — Three verified holes that let a false "all green" survive the exact checkpoint built to catch it are closed, each by riding a mechanism that already existed. **(1) The redaction lint is now gate evidence (#1081).** `war-memory.mjs lint docs/learnings/` ran only in post-push CI, so a plan End state citing it was a structurally SOFT cannot-confirm at every gate-audit — no captured gate artifact could ever carry the evidence. A new thin wrapper at `skills/_shared/war-memory-lint.test.sh` sits beside the CLI and exists to be **discovered**: the refiner-dispatched gate's own `-name '*.test.sh'` sweep picks it up with **zero engine change** — `resolveGate` and its `GATE_DISCOVERY_TOKEN` pairing are byte-untouched, and no floor script was pressed into service as a vehicle — so the lint now lands in every captured `.war/gate-<taskId>.log` under a `== gate(bash): … ==` banner, pre-merge. The wrapper adds exactly one guard beyond path resolution: an absent or unreadable target exits **2** — deliberately distinct from the CLI's own exit 1 (lint hits) — naming the path on stderr, because `cmdLint`'s `catch { continue }` otherwise prints `lint: clean` and exits 0 on a missing directory, which would make this evidence silently vacuous forever the moment `docs/learnings/` moved or was renamed; new meta-tests in `skills/_shared/war-memory.test.mjs` drive the red, clean, and missing-target paths against temp fixtures, and deleting the wrapper's CLI invocation reds the violating-fixture case. `.github/workflows/memory-audit.yml` is now documented as the **post-push backstop** to that pre-merge enforcement — **header comment only: its triggers, steps, and permissions are byte-identical, and CI behavior is unchanged.** **(2) The gate-audit End-state ownership exemption names both owners (#1082).** `endStateBlock`'s case (3) exempted only a condition "owned by a LATER phase", so a condition owned by a not-yet-landed `deps`-chained **sibling task in the same phase** read as provably-UNMET-HARD against an upstream task that had fully met its own slice — the recorded incident dodged the false hold only through auditor judgment. Case (3) now names both owners and directs the seat to map each numbered condition to the task slice that owns it before scoring, citing the plan file through a **guarded** `${(plan && plan.file) ?? '<unset>'}` interpolation (`plan` is destructured with no default and this const is built at top-level scope, so a bare interpolation would throw phase-wide on a plan-less claims-bearing phase). The exemption is carried on **both** prompt surfaces in one commit — the dispatched clause in `skills/war/assets/workflow-template.js` and a new `End-state ownership mapping` bullet in `agents/war-auditor.md` — and reuses the existing `out-of-scope` finding-title token, so **no status enum, no routing table, and no handoff `endState` derivation moved**; `schemas.md`, `design.md`, and both `workflow-template.js` header comments that restated the single-owner rule now carry the sibling clause. **(3) The version guard is no longer lock-step-only, and two stale-worktree paths refuse (#1083).** A coherent all-four-slots *revert* moves every slot together and therefore passes the lock-step test — exactly how a Gate-2 `docs(learnings)` commit staged from a stale publication worktree silently reverted a landed release. Three layers close it. `skills/war/assets/version-slots.test.mjs` gains a **monotonic floor**: `plugin.json#version` must be `>=` the highest version in a bounded 50-commit first-parent window of slot-touching history, read in ONE `git log` spawn with `--diff-merges=first-parent` **pinned explicitly** — on a campaign branch effectively every slot-touching commit in that window is a merge (50 of 50 at the implementation base), so the whole signal comes from merge diffs, which git shows today only because `--first-parent` **implies** that flag; the explicit pin keeps the window from collapsing toward zero versions if that display default ever shifts, and this suite runs in every dispatched gate. It fails open **only** on an unusable git context (spawn failure, non-zero git exit, or a genuinely empty log); a non-empty log whose parse finds zero versions is **RED**, never a vacuous pass, and the monotonic failure names both versions and the Lead/operator restore remedy. The existing extraction, lock-step, and README-prose tests are byte-untouched. `skills/war/SKILL.md`'s Gate-2 promotion flow gains a **pre-push staged-file-list check** between the commit and the `ensure-origin` push: the docs commit's `git show --name-only --format= HEAD` set may contain only paths under the resolved promotion destination (default `docs/learnings/`, honoring a non-null `overrides.learningsTarget`) plus `CLAUDE.md` for the pointer duty — any other path means stale tracked files were staged, so do **not** push; `remove-publication-worktree`, re-provision, re-commit. That is the root-cause check: it catches every stale-staged tracked file, not merely the four version slots. And `cmd_ensure_publication_worktree`'s behavior (b) — registered, present, HEAD already on the working branch — now runs the same `status --porcelain -uno` tracked-file probe behaviors (c)/(d) already use and **refuses a dirty reuse loudly** through the existing catalogued `$EX_WRONG_BRANCH`, naming the stale-staging hazard and the remove-then-re-provision remedy. It only ever refuses — never resets, never cleans, never switches away — and introduces **no new exit constant and no bare numeric literal**, leaving the ADR 0034 exit catalogue unchanged; untracked files (the `.war-task` marker) still never count as dirty, clean reuse and fresh-create are untouched, and the refinery counterpart deliberately keeps its six behaviors, since extending the refusal there would interact with the serial merge queue's legitimate in-flight state (a recorded non-goal). CLAUDE.md's Releasing sentence now names the monotonic floor beside lock-step, and the three origin lessons under `docs/learnings/` carry `MITIGATED (#1081)` / `RESOLVED (#1082)` / `MITIGATED (#1083)` description prefixes with the closing mechanisms recorded in their bodies.
