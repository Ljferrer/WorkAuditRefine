...
The three presets move the whole profile at once: **`balanced`** *is* the defaults, **`thorough`** swaps in its own five deep lenses on fable auditor seats at session effort, lifts the refiner to `xhigh` and the servitor to opus on `high`, and puts red-team and snipe seats on fable, and **`economy`** pins the cheaper agent tiers — opus workers, sonnet auditors on `xhigh`, sonnet refiner and servitor on `high`, sonnet red-team — over its own four-lens pool, with roster policy and the 6-round budget inherited, while `run.ace` stays inherited on (no preset pins the ace ladder; it gates only the per-task ace ladder).
...
**0.21.11** — preset re-tier: the three shipped presets in `war-config.mjs` move to new agent tiers, and every mirror moves with them. `balanced` (= `DEFAULTS`) runs fable workers at `default` effort on the base, docs and fix tiers, with opus auditors on `high`; refiner (sonnet/`high`), servitor (sonnet/`xhigh`), red-team (opus/`high`) and snipe (opus/`high`) keep their prior values. `thorough` inherits those worker tiers, runs fable auditors at `default` effort, sonnet refiner on `xhigh` (unchanged), opus servitor on `high`, and pins fable at `default` effort on both red-team and snipe. `economy` runs opus workers at `default` effort on base and docs, inherits the fable fix tier, sonnet auditors on `xhigh`, sonnet refiner and servitor on `high`, sonnet red-team at `default` effort, and drops its `rosterPolicy: solo` and `roundLimit: 2` pins, so it now inherits `auto` and the 6-round budget; its four-lens pool and `redteamRoundLimit: 2` stay pinned. Mirrors updated in the same commit: the `ROLE_MODEL` and `WORKER_TIER_DEFAULTS` sandbox literals in `workflow-template.js`, the `agents/war-worker.md` (fable) and `agents/war-auditor.md` (opus) frontmatter, the `/war-room` preset bullets and override rows, the README default-config paragraphs, `schemas.md`, `design.md`, and the `CONTEXT.md` fix-bump term. `war-config.test.mjs` and `workflow-template.test.mjs` pin the new values; the retired economy solo and roundLimit-2 assertions become inherit assertions. No engine routing, guard, hook, floor, or shell suite changed in this window; the version slots and the CHANGELOG head entry are this commit.

- **Absorb budget (#2017, #2021, #2022)** — `run.absorbRounds` joins `DEFAULTS.run` (6), validated exactly like `run.roundLimit` (integer ≥ 1, null rejected, same message shape) and inherited by every shipped preset; the engine charges `r.task.absorbRounds` at the batch ace, each re-entry, and each bisect-subset commit, and every ace-side commit prompt (reverts excluded) carries an `Ace-Charge: <task>:<n>` trailer, which the phase-start git-topology barrier reads back as `absorbCharges` so a resume seeds the counter from the branch (0 with a loud log on error or absence). `fixRounds` now counts blocking fix rounds and floor retries only, and the `roundLimit − 2` reserve is retired: a budget-spent bisect ladder sends its untested subsets to the sweep instead of demoting them. ADR 0013 carries the supersession amendment, and the refiner card's `## Gate-failure classification` body moves byte-identical to `references/gate-failure-classification.md` behind a trigger pointer, with the card measured back under its hard line.
- **Economy preset flip (#2017, #2019)** — the `economy` preset no longer pins `run.ace` and inherits it from `DEFAULTS`, so all three shipped presets resolve `run.ace === true`; `run.ace` gates the per-task ace ladder only. The `README.md` `--ace` row and defaults paragraph, the `skills/war/SKILL.md` `--ace` bullet, the `schemas.md` comment, and the `/war-room` rows state the new fact, with `economy-ace-flip` OLD-absent and NEW-present rows over those four surfaces plus `war-config.test.mjs`.
- **Barrier list and default flip (#2044, #2045)** — `BARRIER_TOKENS` is canonical in `land-decision.mjs` and hand-mirrored inline: four members, `barrier:release-slot`, `barrier:underspecified`, and `barrier:rationale-comment` as follow-up barriers and `barrier:trade-off` as an ask route; a scope argument is never a barrier. A fully specified in-diff Minor/Nit now defaults to `absorb` and an out-of-diff one to `absorb` + `phaseClose:true`; `follow-up` needs a cited `barrier`, carried in the new optional AuditVerdict field. The dispatched DISPOSITION RULE byte-mirrors the auditor card, the retired "never a default" conjunction is absent from the eight living surfaces after normalization, the card's `execution-evidence` checklist body moves byte-identical to `references/gate-audit-checklist.md`, and ADR 0013 carries the Phase 3 amendment.
- **Exclusion set (#2018, #2048, #2075)** — `campaign-ledger.mjs`'s `extractFiles` unions every `- Files:` block with the backtick-absence fallback evaluated per block, and `extractFilesFromPlanFile` is exported so the Lead can thread `args.sweepExclude` per the new `references/sweep-exclusion.md` duty. An absorb whose file sits in another campaign plan's footprint demotes naming the plan slug, an in-phase collision demotes naming the task id, and `RELEASE_SLOT_FILES` (the two version-slot JSONs, canonical in `land-decision.mjs`) is refused at birth from the per-task ace, the sweep, and the terminal pass by basename, so `sub/dir/plugin.json` is refused and `plugin.json.bak` is not. The run manifest records `sweepExcludeCount` and the threaded `finalPhase`.
- **Filing floor (#2048)** — a per-task `diff-probe` refiner dispatch runs before the seats convene and hands git-derived `diff_files` to `dispositionOf`, so the intake floor is deterministic: an omitted-disposition in-diff Minor with a `suggested_fix` defaults `absorb`, a barrierless `follow-up` reroutes to `absorb` (in-diff into the task's own ace batch, out-of-diff with `phaseClose:true`), `barrier:trade-off` with the `ask` fields reroutes to `ask`, and an in-diff `note` naming a fix is applied. A failed probe keeps the old default, logs the skip, and stamps `demote:floor-skipped` on that task's filed rows; the probe is idempotent on resume.
- **`DEMOTE_REASONS` census (#2048)** — the twelve-member closed prefix enum is canonical in `land-decision.mjs` and mirrored inline; `demote()` validates every `follow-up` reason against it at runtime and on a miss prepends `demote:unclassified` with a loud log, so an engine-filed issue body's prefix line is never blank. A census in `workflow-template.test.mjs` pins every `demote` call site whose disposition can evaluate to `follow-up`, carries one unwired negative reference, and asserts zero shipped sites carry `demote:unclassified`.
- **Ace-off and gate-audit routing (#2048, #2049)** — with `run.ace` off a seat absorb and a floor reroute route `phaseClose:true` to the sweep instead of demoting; the "absorb requires --ace" arm and the "every absorb demotes" Residual-rule wording are gone. The three gate-audit seats' (`execution-evidence`, integrated-tip, end-state) fully specified Minor/Nits route into the sweep queue stamped with the seat label under the exclusion set, a barrierless gate-audit `follow-up` reroutes, and the post-merge evidence dispatch carries `phase_diff_files` so a gate-audit `note` naming a fix in a touched file reroutes too; the `auditLog`-only sink is retired. `CONTEXT.md` gains the demote reason prefix term and ADR 0013 the Phase 4 amendment.
- **Terminal pass (#2063, #2064)** — one hop per phase after the polish merge: the polish panel's absorbs plus any queued absorb the sweep could not land are filtered through `aceEligible`, committed once at the post-polish tip with an `Ace-Charge` trailer through Refine, and re-audited by one seat (`correctness` when present, else the roster's first, logged). The pass convenes with `run.ace` off; a re-audit regression forward-reverts the terminal commit and then carries (non-final phase) or demotes with `demote:absorb-regressed` (final phase, `args.finalPhase`, boolean-validated at entry), while an absorb the terminal seat freshly raises demotes with "raised on the terminal pass" there; the merged-path sweep-raised demote arm is retired while the discard-path arm stays. `CONTEXT.md` gains the terminal pass and carried queue terms and ADR 0012 the successor cross-reference.
- **Held carry (#2063, #2075, #2076)** — every Workflow-emitted hold class (`held:escalation`, `held:nothing-merged`, `held:land-failed`, `held:submodule-pr`, `held:workflow-error`) returns its undrained absorbs as `carriedPhaseClose` (`[]` when nothing was carried; a landed non-final phase can still carry the terminal pass's leftovers), and a relaunch with `args.seededPhaseClose` drains them into the next sweep. `resume-and-recovery.md` carries the relaunch steps (seed the queue, thread `finalPhase`, override `absorbCharges` only at a sanctioned relaunch), and `skills/war-review/SKILL.md` gains rows for `sweepExcludeCount`, the `terminal-pass` seat and outcome, `absorbRounds` spent per task, follow-ups per `DEMOTE_REASONS` prefix, and `demote:unclassified` as a defect signal.

Release scope: the 47-file window from this run's launch base to this commit, bucketed whole (44 plan files below plus the three release files this commit adds). Engine: `workflow-template.js`, `land-decision.mjs`, `war-config.mjs`, and `campaign-ledger.mjs`. Prompt and doctrine surfaces: `agents/war-auditor.md`, `agents/war-refiner.md`, the `war`, `war-room`, `war-campaign`, and `war-review` `SKILL.md` files, and twelve `skills/war/references/` files, three of them new (`gate-audit-checklist.md`, `gate-failure-classification.md`, `sweep-exclusion.md`). Tests: the seven JS suites that pin the above (`workflow-template`, `war-config`, `land-decision`, `campaign-ledger`, `skill-doc-contracts`, `reference-link-integrity`, `doc-cli-consistency`). Docs: `CLAUDE.md`, `CONTEXT.md`, `README.md`, ADR 0012, and ADR 0013. Learnings: nine `docs/learnings/` files, six new and three amended. Config: `.claude/war/config.json`, this run's own `/war-room` commit (fable workers, opus auditors, a `snipe` tier, `redteamRoundLimit: 3`), which rode along and is not plan work. The plan and its red-team report both predate the base and are not in the window. No hook, floor script, or shell suite changed, and the version slots and the CHANGELOG head entry are this commit.
