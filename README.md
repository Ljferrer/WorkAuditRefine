...
The plugin ships a pair of session-level output-discipline hooks (`hooks/reply-standard/`), created by [@kem_glitch](https://www.instagram.com/kem_glitch) on Instagram ([original Claude artifact](https://claude.ai/code/artifact/7e14d97c-7d12-421e-ad68-1f1f4c22b954)) and included byte-for-byte per their instructions:
...
Both events fire in the main conversation only: WAR's phase subagents (workers, auditors, refiners, servitors) see neither hook — `UserPromptSubmit` does not fire for a subagent's spawn prompt, and a subagent's turn-end is `SubagentStop`, not `Stop`. The WAR **Lead** does run in the main conversation, so the gate also skips the card for any prompt whose first token is a WAR command (`/war`, the `/war-*` family, `/red-team`, `/survey-corps`, `/aftermath`, `/lessons-learned`) — orchestration turns are never style-pressured; the meter still scores them.
...
- `vale-md.py` (`PostToolUse` on `Edit`/`Write`, harness-filtered to `.md` paths by per-handler `if` rules so non-Markdown edits never spawn a process) runs [Vale](https://vale.sh) over the edited file when its path ends in `.md`, using a self-contained profile beside the script, chosen by `hooks.valeStyle` (default `googleFork`); every style is vendored in the plugin, so no synced packages and no network at lint time. When Vale reports findings, the hook returns one `additionalContext` line naming the count and the top three rules. It never blocks, and the line itself cautions the agent never to reword ratified, byte-pinned, or drift-guarded literals to satisfy a style rule.
- The house rules are the Reply Standard card's, recast for docs: `SentenceLength` (over 25 words), `SlopWords`, `LatinAbbrev`, and `Dashes` (shipped but disabled in both profiles — em dashes are this repo's own house style).
...
**Styles (`hooks.valeStyle`):** eight profiles ship vendored — `house` (the four ReplyStandard rules alone), `googleFork` (the default: house rules plus a tuned cut of the Google developer-documentation style — sixteen Google rules disabled: thirteen that judged entrenched repo voice in the prototype sweep, plus `Google.Spelling` since the card mandates British, `Google.Latin` as a house twin, and `Google.WordList`, a substitution list that fires on ratified vocabulary like "CLI"), `microsoftFork` (house rules plus a Microsoft Writing Style Guide cut tuned on the same rationale — including `Microsoft.Contractions`, which recommends the contractions the card bans), and `writeGood`, `proselint`, `alex`, `readability`, `redhat` (house rules plus that package, near-stock). Each profile header pins its upstream source, version, and license (`styles/<Name>/LICENSE`). A ninth value, `custom`, reads the project's own `.claude/war/vale/.vale.ini` (written by the `/war-room` interview) and fail-opens to `googleFork` when absent. Unknown values also mean the default; subordinate to `valeMarkdown` — when that is `false`, nothing runs.
...
**0.21.4** — Vale style library for the vale-md hook: seven prose standards are now vendored beside the house rules under `hooks/vale-md/styles/` — Google v0.7.1, Microsoft v0.15.1, write-good v0.4.1, proselint v0.3.4, alex v0.2.3, Readability v0.1.1 (all errata-ai, MIT except proselint's BSD-3), and Red Hat v677 (redhat-documentation, MIT) — each with its upstream LICENSE committed and its version pinned in the selecting profile's header, fetched once at vendoring time so lint time needs no network and rule updates are deliberate commits. Selection is a new validated `hooks.valeStyle` enum (`war-config.mjs` DEFAULTS + `VALE_STYLES` export + validation, default `googleFork`, `null` = unset, subordinate to `hooks.valeMarkdown`): `house | googleFork | microsoftFork | writeGood | proselint | alex | readability | redhat | custom`, where the two Fork profiles carry documented per-rule tuning (voice rules off; British spelling wins; substitution lists off), the supplement profiles run near-stock, and `custom` reads the project-side `.claude/war/vale/.vale.ini` — written by the `/war-room` custom-style interview (doctrine at `skills/war-room/references/vale-custom-interview.md`, fronted by a non-blocking repo pattern-mining subagent) — failing open to the default when absent. This supersedes the short-lived `hooks.valeGoogleFork` boolean from earlier on this unreleased branch; no released config key changes shape. Also in this window: the committed repo run config `.claude/war/config.json`, deleted by a stray smoke-test cleanup in an earlier commit on this branch and caught by a two-seat audit, is restored with the `hooks.*` keys added explicitly. Beyond the vendored style directories, the eight profiles, the vale-md.py style selector with its reworked test cases, the config key with its schema tests, the restored run config, the README, `/war-room`, and schemas doc lines, and the version slots, no engine, guard, or agent surface changed in this window.
